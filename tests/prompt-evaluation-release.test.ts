...
try {
  ...
  ).enabled, true, "未来 verified 编辑发布应允许 UI 应用精确匹配的参考图配置");

  promotePromptVariantForTest(maskVariant, "recommended");
  assert.equal(getRuntimePromptVariantAvailability(
    {
    ...
    { currentCodeSha: TEST_PROMPT_RELEASE_CODE_SHA },
  ).enabled, true, "未来 recommended 蒙版发布应允许 UI 应用精确运行时参考图配置");
} finally {
