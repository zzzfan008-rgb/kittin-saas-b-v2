...
- Do not install or update dependencies: worktrees intentionally share `node_modules`. Escalate dependency or lockfile changes to the user; they are part of the reviewed delivery batch, not a worktree edit.
- Keep all changes local. Do not push, open pull requests, merge, rebase, cherry-pick, reset hard, or clean.
- Finish with the structured handoff required by `AGENTS.md`.
