    ...
    if has_jq; then
        ...
        if [[ -n "$TEMPLATE_NAME" ]]; then
            jq -cn \
                ...
                '{FEATURE_DIR:$feature_dir,TASKS:$tasks,AVAILABLE_DOCS:$docs,TEMPLATE_CONTENT:$template_content}'
        else
            jq -cn \
                --arg feature_dir "$FEATURE_DIR" \
                --arg tasks "$TASKS" \
                --argjson docs "$json_docs" \
                '{FEATURE_DIR:$feature_dir,TASKS:$tasks,AVAILABLE_DOCS:$docs}'
        fi
    else
        if [[ ${#docs[@]} -eq 0 ]]; then
            json_docs="[]"
        ...
        if [[ -n "$TEMPLATE_NAME" ]]; then
            printf '{"FEATURE_DIR":"%s","TASKS":"%s","AVAILABLE_DOCS":%s,"TEMPLATE_CONTENT":"%s"}\n' \
                "$(json_escape "$FEATURE_DIR")" "$(json_escape "$TASKS")" "$json_docs" "$(json_escape "$TEMPLATE_CONTENT")"
        else
