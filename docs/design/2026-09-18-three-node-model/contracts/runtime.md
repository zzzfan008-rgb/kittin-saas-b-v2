...
- v3.2（R-39）：§5b 改写为「参考图序号：拉线自动定序 + 节点徽标」——取代 B1/B2 的悬浮窗口手动排序口径；序号 = （选中目标， 源图片） 派生视图，选中态切换重算，永不持久化。
...
## 5b. 参考图序号：拉线自动定序 + 节点徽标（R-39，取代 B1/B2）

**本节为 R-39 改写段**（2026-09-18 晚，用户裁定经 orchestrator 两次中继）：原 B1/B2「边顺序编辑归悬浮窗口（上移/下移手动调序）」**作废**。用户裁定的形态是：**拉线时自动给图片节点排序打标，并把顺序在节点上直接显示**；补充裁定：徽标跟随**当前选中的目标**显示，未选中目标时不渲染。

运行时口径（数据侧完整定义在 graph-invariants.md §2b，本节是运行时消费侧的承诺）：

- **序号永不持久化**：节点 `data`、`DocumentSnapshot`、flow_json、run 记录中均无 ordinal 字段；显示序号 ≡ 目标节点 image 入边数组下标 + 1，由派生选择器 `selectReferenceOrdinals(state, selectedTargetId)` 现算（契约签名见 graph-invariants.md §2b.3）。
- **自动定序**：新建 image 边追加到目标入边数组末尾（现有 `addEdge` 语义，无改动）；删除补位（shift-up），序号恒连续 1..N。
- **选中态驱动渲染**：选择器输入含 `selectedTargetId`；`null` → 空 Map → 画布无徽标。切换选中目标时按新目标入边数组重算，**禁止缓存跨目标的「节点 → 序号」映射**（防串味）。选中态是 React Flow UI 运行时状态，不进文档数据。
- **运行路径零影响**：提示词组装（§1 第 4/6 步）与 `resolveReferenceInputs` 的参考图顺序仍按边数组顺序——徽标序号只是该顺序的人类可读投影，不进入任何 Provider 请求。
- **`maskSourceRef` 不失配**：maskSourceRef 绑定引用值而非序号（§1 第 5 步检查不变）；补位重排不改变引用值，只有蒙版源边本身被删/替换才触发既有 fail-closed 拒绝。删除导致序号补位后，悬浮窗口展示一次性提示「顺序已变更，角色对应关系以新顺序为准」（i18n 键 `inspector.orderChanged`，graph-invariants.md §5 清单）。
- **手动调序能力取消**：悬浮窗口的参考图列表改为**只读**（展示当前生效顺序 + 断线入口）；R-39 裁定明确用户不要手动调序。若未来恢复，走新裁定。
- **text 边不编号**：序号只覆盖 image 参考边；text 串联顺序维持悬浮窗口列表可见性，无徽标。
