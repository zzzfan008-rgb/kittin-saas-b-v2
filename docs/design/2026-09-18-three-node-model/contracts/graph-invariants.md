...
- image 边数量上限：image 节点 ≤ 8；video 节点 ≤ 1（首帧，待 Q2 裁定）。
- 不再使用 v6 `fabric-recolor` 的 `targetHandle="fabric"` 特例（Q3，默认抹平）。
...
- `tests/workflow-schema.test.ts`：INV-1 正例/反例（无 text 边、text 边来自错误 kind、handle 错误）。
- `tests/dag.test.ts`：INV-2 正例/反例（text 空、text 全空白、多 text 上游拼接顺序）。
- `tests/canvas-connection`（或并入现有 e2e）：非法连线 UI 反馈 + 自动补 text 节点行为。
