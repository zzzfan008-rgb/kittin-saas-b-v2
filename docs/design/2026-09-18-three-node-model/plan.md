...
**参考图序号的展示语义**（R-39 裁定，2026-09-18 晚，取代 v3 的 B1/B2「边顺序编辑归悬浮窗口」口径）：序号**不是持久化字段**，而是**边数组顺序的派生视图**——节点徽标上显示的序号 = 该源节点在**当前选中目标**入边数组中的下标（1-based）；目标未选中时不渲染。数据侧口径：

- **事实来源唯一**：`edges` 数组顺序仍是唯一事实；序号（ordinal）是纯视图概念，永不写入节点 `data`、永不进 `DocumentSnapshot`、永不进 flow_json（避免陈旧序号与边数组漂移的双源真相）。
- **拉线自动定序**：新建 image 边连入即追加到目标节点入边数组**末尾**（序号 = 当前最大 +1），用户无需手动排序；这与现有 `addEdge` 追加语义一致，画布层零改动。
- **断线/删除重排：补位（shift-up）**。删除中间一条边后，后续边上移补位（数组原生语义）。理由：序号必须始终连续（1..N 无空洞），否则「参考图 2 = 面料」的提示词角色声明会在空洞处歧义（指 2 还是指第 2 条现存边？）；补位后「边数组顺序 == 显示序号」的不变量恒成立，心智成本最低。代价：删除会导致后续序号变化，由 R-38 徽标的即时刷新 + `inspector.orderChanged` 一次性提示兜底（沿用 v3.1 已有键）。
- **与 `maskSourceRef` 的交互**（沿用 v6 语义 + 补位后果的明文口径）：`maskSourceRef` 存的是**引用值**（图片 ref 字符串），不是序号；因此**补位重排不会令 maskSourceRef 漂移**——它仍指向同一张图。会失配的只有两种情况：(a) 第一条图片入边本身被删除/替换（引用值变了）；(b) 蒙版源图不再是第一条（用户删除排在它前面的边，蒙版源图补位成了新的第一条——此时 maskSourceRef 仍匹配，语义反而自洽）。真正需要防的是 (a)：运行前检查 `maskSourceRef === 第一条图片入边的当前引用`（runtime.md §1 第 5 步），不匹配即运行拒绝，文案已有。
- **多目标与选中态**（R-39 补充裁定，2026-09-18 晚，orchestrator 中继）：序号 = **(当前选中目标, 源图片)** 的二元派生视图——徽标只在「该 image/video 目标节点处于选中态」时，为其入边源节点渲染；**未选中目标时不渲染任何徽标**。切换选中目标时同一源图的序号随之变化（选择器按选中目标重算，不得缓存串味）。一张源图连多个目标时，各目标入边数组独立，同一源图在不同目标下序号天然不同，选中哪个目标就显示哪个序号。视觉细节（徽标位置、样式、多选/框选时的呈现）归 R-38 视觉规范，数据侧只承诺「任一 (selectedTargetId, sourceId) 对的序号可 O(入边数) 派生」，见 contracts/graph-invariants.md §2b。

详细契约（含展示数据流选择器口径）见 contracts/graph-invariants.md §2b 与 contracts/runtime.md §5b。
...
>
> **R10 最终裁定（2026-09-19，用户拍板，orchestrator 中继；§5 提案清单就此关闭）**：
>
> - **文本三实名**：`gpt-4o`（默认/主力；用户原话「gimini-4o」经确认为笔误）+ `claude-sonnet-4-5`（横线形式）+ `gemini-3-pro-preview`。原 P2-a 落地的 `gpt-5.3-chat-latest` / `gemini-3.6-flash` / `deepseek-v4-flash-ga-260731` **作废**。
> - **视频只要 `doubao-seedance-2-5-260628`**：原提案品质档 **Veo 3.1 取消**（`veo-3.1-*` 从清单移除），Seedance 2.0 三档（`2-0-260128`/`-fast`/`-mini`）一并移除。
> - **令牌变更**：用户已**新建专用令牌**调用 Seedance → §5.2 的「需勾选 SeeDance2 分组（0.18x）」运营动作**取消**；口径改为「支持配置专用令牌」，具体配置形态归 P2-e。
> - 与原提案的差异总结：文本从「gpt-5.3 系旗舰 + flash 轻量档」换为「gpt-4o 默认 + claude/gemini 备选」；视频从「三档梯度 + 品质档」收敛为单模型（接受 2.5 约 1.5 倍于 2.0 标准版的单价，换取 30s/30 参考图/1080p/mov 能力）；分组令牌依赖消除。
> - §5.1/§5.2 的推荐正文保留为历史提案记录（不回写），现行有效口径以 contracts/data-model.md v4（§5/§5b）与 contracts/model-proposals.md §6 为准。
> - 待确认登记：`claude-sonnet-4-5` 横线形式在计费目录（models.md）登记的是 `-20250929` / `-thinking` 变体，纯横线形式仅见于 chathub 场景页示例；按用户裁定落横线形式，网关实际可调性由 P2-b 联调与评估链 campaign 证明。
...
| `contracts/graph-invariants.md` | R3 不变量与三处实现的统一语义（§1.3）+ 参考图序号展示语义（§2b，R-39） |
