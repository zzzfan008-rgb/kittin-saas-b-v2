...
> GLM-5.3-Flash 模型详情：输入 $0.15 / 输出 $0.5 每 1M tokens，1,000,000 上下文、128,000 最大输出，2 个分组可用。

GLM-5 系列首个原生多模态，320B 总参数、18B 激活，Terminal-Bench 2.1 84.3 逼近 Claude Opus 4.8，成本约为旗舰的十分之一；MIT 开源。
...
以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `glm-5.3-flash`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  <Card title="上线公告" icon="megaphone" href="/news/glm-5-3-launch">
    GLM-5.3-Flash 的发布背景、实测表现与迁移建议
  </Card>

  <Card title="GLM-5.3" icon="git-compare" href="/models/glm-5-3">
    同系列模型详情
  </Card>
