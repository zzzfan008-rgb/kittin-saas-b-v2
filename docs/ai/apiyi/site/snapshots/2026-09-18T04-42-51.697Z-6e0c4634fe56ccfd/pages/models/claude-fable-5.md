...
> Claude Fable 5 模型详情：输入 $10 / 输出 $50 每 1M tokens，4 个分组可用。

Mythos 级旗舰，软件工程与视觉理解近乎全面 SOTA；输入输出保留 30 天用于滥用检测。
...
| ------------------------- | ------------------------------------------------ |
| `claude-fable-5`          | 标准调用                                             |
| `claude-fable-5-thinking` | 强制思考变体，价格与端点与标准版完全相同；分组比标准版少一个 ClaudeCodeReverse |

## 调用示例

以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `claude-fable-5`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  <Card title="上线公告" icon="megaphone" href="/news/claude-fable-5-launch">
    Claude Fable 5 的发布背景、实测表现与迁移建议
  </Card>
