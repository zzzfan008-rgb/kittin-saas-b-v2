...
> GLM-5.2 模型详情：输入 $1.142 / 输出 $3.997 每 1M tokens，1,000,000 上下文，3 个分组可用。

MoE 架构约 744B 总参数、40B 激活，AA 智能指数开源第一，适合项目级编程与长程智能体。
...
以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `glm-5.2`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  <Card title="上线公告" icon="megaphone" href="/news/glm-5-2-launch">
    GLM-5.2 的发布背景、实测表现与迁移建议
  </Card>
