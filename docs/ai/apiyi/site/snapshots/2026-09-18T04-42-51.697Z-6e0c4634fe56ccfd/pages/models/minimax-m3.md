...
> MiniMax-M3 模型详情：输入 $0.3 / 输出 $1.2 每 1M tokens，1,000,000 上下文，3 个分组可用。

首个集前沿 Agent 编程、百万上下文与原生多模态于一身的开源权重模型，MSA 稀疏注意力把长上下文推理成本降至前代 1/20。
...
* 0 – 524,288 tokens 输入 \$0.3/1M
* 超过 524,288 tokens 输入 \$0.6/1M
...
以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `MiniMax-M3`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  <Card title="上线公告" icon="megaphone" href="/news/minimax-m3-launch">
    MiniMax-M3 的发布背景、实测表现与迁移建议
  </Card>
