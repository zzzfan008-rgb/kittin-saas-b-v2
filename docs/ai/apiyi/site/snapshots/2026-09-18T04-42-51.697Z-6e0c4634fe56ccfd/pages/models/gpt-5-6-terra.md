...
> GPT-5.6 Terra 模型详情：输入 $2 / 输出 $12 每 1M tokens，1,000,000 上下文，4 个分组可用。

GPT-5.6 系列主力档，对标 GPT-5.5 的性能但价格减半，存量迁移首选。
...
* 0 – 272,000 tokens 输入 \$2/1M
* 超过 272,000 tokens 输入 \$4/1M
...
以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `gpt-5.6-terra`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
