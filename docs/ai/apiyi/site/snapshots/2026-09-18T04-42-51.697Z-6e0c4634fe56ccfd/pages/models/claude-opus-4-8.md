...
> Claude Opus 4.8 模型详情：输入 $5 / 输出 $25 每 1M tokens，200,000 上下文，4 个分组可用。

上一代 Opus 旗舰，Agentic 编程 69.2%，支持 effort 五档与动态工作流。
...
| 模型 ID                      | 说明                   |
| -------------------------- | -------------------- |
| `claude-opus-4-8`          | 标准调用                 |
| `claude-opus-4-8-thinking` | 强制思考变体，价格与端点与标准版完全相同 |

## 调用示例

以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `claude-opus-4-8`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  <Card title="上线公告" icon="megaphone" href="/news/claude-opus-4-8-launch">
    Claude Opus 4.8 的发布背景、实测表现与迁移建议
  </Card>
