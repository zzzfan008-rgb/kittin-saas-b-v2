...
> Qwen3.7-Flash 模型详情：输入 $0.16 / 输出 $0.64 每 1M tokens，1,000,000 上下文、65,536 最大输出，2 个分组可用。

上一代低价多模态推理款，1M 上下文、65K 最大输出、256K 思考预算，主打多模态 Agent 与视觉编码。
...
以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `qwen3.7-flash`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  <Card title="Qwen3.8-Flash" icon="git-compare" href="/models/qwen3-8-flash">
    同系列模型详情
  </Card>
