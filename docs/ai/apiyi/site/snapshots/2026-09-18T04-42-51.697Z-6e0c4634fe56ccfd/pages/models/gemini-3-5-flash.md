...
> Gemini 3.5 Flash 模型详情：输入 $1.5 / 输出 $9 每 1M tokens，1,000,000 上下文、64,000 最大输出，3 个分组可用。

Terminal-Bench 2.1 76.2%，思维链带加密推理上下文可跨调用保留；不支持 Computer Use。
...
以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `gemini-3.5-flash`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  <Card title="上线公告" icon="megaphone" href="/news/gemini-3-5-flash-launch">
    Gemini 3.5 Flash 的发布背景、实测表现与迁移建议
  </Card>
