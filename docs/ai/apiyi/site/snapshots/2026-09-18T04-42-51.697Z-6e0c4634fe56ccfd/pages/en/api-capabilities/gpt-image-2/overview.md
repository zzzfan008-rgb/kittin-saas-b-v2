...
# GPT-Image-2.5 / 2 Image Generation/Edit

> Official-relay guide to OpenAI's GPT-Image 2.5 / 2 series: gpt-image-2.5-flare (speed-first), gpt-image-2.5-sunburst (editing-precision-first) and gpt-image-2, all at the same price and parameters. Native any 2K/4K resolution, auto high-fidelity reference images, text-to-image, reference editing, multi-image fusion, mask inpainting.
...
This page covers OpenAI's **GPT-Image 2.5 / 2 series** via APIYI's official relay: **`gpt-image-2.5-flare`** (speed-first), **`gpt-image-2.5-sunburst`** (quality- and editing-precision-first) and the previous-generation **`gpt-image-2`**. The 2.5 models shipped on 2026-09-08 with higher quality than gpt-image-2, more precise editing and two new `quality` tiers (`xhigh` / `max`), at **exactly the same price and parameters as gpt-image-2**. Shared across the series: **any valid resolution (incl. 2K / 3840×2160 4K)**, **auto high-fidelity on reference images**, **per-token billing**. APIYI's gateway is fully compatible with the OpenAI Images API — point the official OpenAI SDK's `base_url` here for zero-code direct connection.

<Note>
  **🎨 Key highlights**: Native support for any valid resolution (max 3840×2160 4K) + auto high-fidelity on reference image edits + native Chinese prompt support + new `xhigh` / `max` quality tiers in 2.5. **Best for production scenarios that need precise size/quality control, must match the OpenAI official API exactly, or require 4K output**; default text-to-image to `gpt-image-2.5-flare` and edits to `gpt-image-2.5-sunburst`.
</Note>
...
<Prompt description="Have a coding agent integrate or troubleshoot GPT-Image 2.5 / 2 series text-to-image and image editing. Copy and paste into Codex, Claude Code, Cursor and similar tools." icon="bot" actions={["copy"]}>
  Integrate / troubleshoot GPT-Image 2.5 series text-to-image and image editing in this project: use `gpt-image-2.5-flare` for text-to-image and `gpt-image-2.5-sunburst` for image editing (both share price and parameters with `gpt-image-2`; make the model name a config value so it is easy to switch).
  ...
  4. Quality tier: always send `quality`, defaulting to `medium`, and add a quality dropdown to the UI (`low` / `medium` / `high` / `xhigh` / `max`; the last two are accepted by 2.5 models only and cost more). Note that 2.5 re-graded the tiers: 2.5 `high` only equals `gpt-image-2` `medium`, so do not carry `quality` over verbatim when migrating from `gpt-image-2`; send `max` for the same budget. Do not send `auto` — it is a dynamic reasoning tier, so cost and latency drift.

  5. In production, pin the model name to the dated snapshots `gpt-image-2.5-flare-2026-09-08` / `gpt-image-2.5-sunburst-2026-09-08` so an alias change never moves you.
...
<Accordion title="What this prompt saves you from">
  ...
  | Do not carry `quality` over when migrating | 2.5 `high` = `gpt-image-2` `medium`; copying the value drops you one tier                                                                                                                                                                                              |
  | Pin dated snapshots                        | Behavior can change when an alias moves to a new snapshot; production should not follow passively                                                                                                                                                                      |
</Accordion>
...
## Model Selection: flare / sunburst / gpt-image-2

Since 2026-09-08 this doc group covers three OpenAI GPT-Image 2.5 / 2 models. **All three share the same price, parameters, groups and endpoints**; switching is a one-field `model` change:

| Item                                 | `gpt-image-2.5-flare`                                                              | `gpt-image-2.5-sunburst`                                                         | `gpt-image-2`                                        |
| ------------------------------------ | ---------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------- |
| Positioning                          | Speed-first small model, OpenAI's recommended everyday default                     | Quality-first base model, OpenAI's most capable generation / editing model today | Previous-generation single flagship, still available |
| Quality                              | Higher than gpt-image-2                                                            | Highest of the three                                                             | Baseline                                             |
| Latency (1024², measured 2026-09-09) | `low` 10 s / `high` 21 s / `max` 45 s; up to 50% lower than gpt-image-2 per OpenAI | `low` 14 s / `high` 42 s / `max` 142 s                                           | `low` 21 s / `high` 224 s                            |
| Editing and multi-turn consistency   | Improved                                                                           | Improved further: better subject preservation and multi-turn memory              | —                                                    |
| `quality` tiers                      | Six, adds `xhigh` / `max`                                                          | Six, adds `xhigh` / `max`                                                        | `low` / `medium` / `high` / `auto`                   |
| Current snapshot                     | `gpt-image-2.5-flare-2026-09-08`                                                   | `gpt-image-2.5-sunburst-2026-09-08`                                              | `gpt-image-2-2026-04-21`                             |
| Best for                             | Batch generation, social content, product images, latency-sensitive work           | Multi-turn retouching, production-grade campaign creative, multi-image fusion    | Existing integrations you do not want to touch       |

<Tip>
  **How to choose**: default text-to-image to `gpt-image-2.5-flare`, use `gpt-image-2.5-sunburst` for edits and multi-image fusion, and pin dated snapshots in production so an alias change never moves you. Existing `gpt-image-2` code upgrades with a single model-name change; every other parameter, price and timeout lesson carries over. Launch write-up: [GPT-image-2.5 Launches: Flare Is Faster, Sunburst Is Sharper](/en/news/gpt-image-2-5-launch).
</Tip>
...
APIYI's `gpt-image-2.5-flare` / `gpt-image-2.5-sunburst` / `gpt-image-2` (Default group) **share one price list that matches OpenAI's official rates exactly** — the discount comes from our top-up bonus instead: **top up \$100 and get a 10% bonus, up to 20%**. 📖 [Learn about top-up promotions](/en/faq/recharge-promotions).
...
### Per-image cost reference (official table, gpt-image-2)

Typical per-image cost for `gpt-image-2` at 1K preset sizes (the 2.5 models use different token counts for the same tier names; see the next section):
...
### Quality tiers and measured cost on the 2.5 models (measured 2026-09-09)

**The same tier name does not mean the same token count on 2.5 and gpt-image-2**: 2.5 re-grades the quality ladder. `low` is unchanged, 2.5's `high` equals gpt-image-2's `medium`, and 2.5's `max` is what equals gpt-image-2's `high`. The table shows `usage.output_tokens` and the output cost at \$30 / 1M for 1024×1024 text-to-image, same prompt, one serial run each (flare and sunburst produce identical token counts per tier; only latency differs):
...
<Warning>
  **Do not carry `quality` over verbatim when migrating from gpt-image-2**: the same `high` yields a quarter of the output tokens on 2.5 and maps to a lower rung of the ladder; to match gpt-image-2 `high` in token budget, send `max` on 2.5. Conversely, 2.5's `high` / `xhigh` give you two cheaper intermediate steps at the same budget. Run your own prompts once and check `usage.output_tokens` before production; extrapolate 2K / 4K by pixel ratio.
</Warning>
...
All three GPT-Image 2.5 / 2 models share the same official-relay groups. Switch in dashboard → **Token Settings → Group**:
...
| `svip`             | See console | Also carries all three models; the rate is shown on the token page in the console                                    |
...
| Dimension                  | Value                                                                                                                                                                                                          |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Model name**             | `gpt-image-2.5-flare` / `gpt-image-2.5-sunburst` / `gpt-image-2` (same parameters; see the Model Selection section above)                                                                                      |
| **Speed**                  | 1024² measured (2026-09-09): `gpt-image-2.5-flare` `low` 10 s / `high` 21 s / `max` 45 s; `gpt-image-2.5-sunburst` 14 / 42 / 142 s; `gpt-image-2` `low` 21 s / `high` 224 s. High quality + 2K/4K takes longer |
| **Output resolution**      | Any valid size (1K/2K/4K, max 3840×2160)                                                                                                                                                                       |
| **Quality tiers**          | `auto` / `low` / `medium` / `high` / `xhigh` / `max` (`xhigh` / `max` accepted by the two 2.5 models only)                                                                                                     |
| **Output formats**         | `png` (default) / `jpeg` / `webp`                                                                                                                                                                              |
| **Chinese prompts**        | ✅ Native                                                                                                                                                                                                       |
| **Prompt limit**           | 32,000 characters (provider limit, counted in characters), see [Long Prompts](/en/api-capabilities/image-long-prompt)                                                                                          |
...
| **Transparent background** | ✅ Supported (`background: "transparent"` with `png` / `webp`; `jpeg` has no alpha channel and is mutually exclusive with transparency; keep scenes out of the prompt or the parameter cannot override them)    |
...
| `high`   | High quality                        | Text, fine textures, print; the top tier for `gpt-image-2`                                 |
| `xhigh`  | Extra-high quality (**new in 2.5**) | Accepted by `gpt-image-2.5-flare` / `sunburst` only; more output tokens than `high`        |
| `max`    | Maximum quality (**new in 2.5**)    | Accepted by the two 2.5 models only; highest cost and latency, for production-grade finals |

<Warning>
  **The default is `auto`, not `medium`.** Omitting `quality` is equivalent to passing `"quality": "auto"` — the model auto-selects a quality tier, and **OpenAI does not guarantee it maps to `medium`**. The tier `auto` resolves to is unpredictable and directly affects cost, latency, and billing stability. **When you need cost control and predictability, pass `low` / `medium` / `high` / `xhigh` / `max` explicitly instead of relying on `auto`.**
</Warning>

<Warning>
  **Do not pass the legacy DALL·E values `standard` / `hd`.** `quality` only accepts the six official enum values `low` / `medium` / `high` / `xhigh` / `max` / `auto`, and `xhigh` / `max` are accepted by the two 2.5 models only. The legacy DALL·E 3 values `standard` / `hd` behave inconsistently across backend channels: sometimes they fail immediately with a 400 (`invalid_value`), and sometimes they are silently ignored and the request runs at `auto` (unpredictable cost). Always pass one of the official values explicitly.
</Warning>

<Info>
  **`quality` has the largest impact on price — more than `size`.** Output image token count is driven by `quality × size`, but `quality` carries far more weight: at the same size, going from `low` to `high` can change per-image cost by **30×+** (see the "per-image cost" table above: gpt-image-2 at 1024×1024 ranges from `low` \$0.006 to `high` \$0.211; the 2.5 models range from `low` \$0.006 to `max` \$0.211). Estimate cost by `quality` first, then layer in the effect of `size`.
</Info>
...
<Steps>
  ...
  <Step title="Transparent background: pass the parameter, and keep scenes out of the prompt">
    `background: "transparent"` with `png` / `webp` only guarantees an alpha channel. **If the prompt or the reference image describes a full scene (a forest, a room, a street), the model paints that scene** and the parameter will not stop it — this is the main reason transparency "sometimes works", and it has nothing to do with `quality` (measured 2026-09-11: subject-only prompts 16/16 transparent, scene prompts 0/3, medium and high alike). End the prompt with `isolated subject on a transparent background, no scenery, no ground`; when the reference image has a background, say `remove the background entirely, keep only the subject` in the edit prompt. See the [transparent background FAQ](/en/faq/image-transparent-background).
  </Step>
  ...
  <Step title="Tier your client timeout (high → 600s safety net)">
    ...
    | `high`             | ≥ **600 seconds** (safety net) | gpt-image-2 2K/4K runs 3–5 minutes, long tail observed at 235+ seconds; the 2.5 models measured 21–42 s at 1024² |
    | `xhigh` (2.5 only) | ≥ **300 seconds**              | 1024² measured 31–60 s (2026-09-09); longer at 2K/4K                                                             |
    | `max` (2.5 only)   | ≥ **600 seconds** (safety net) | 1024² measured 45–142 s; same token count as gpt-image-2 `high`, so budget 2K/4K like gpt-image-2 `high`         |
...
| `413`                             | Request body too large (multi-image edits / oversized PNGs; observed from roughly 80MB request bodies upward, and not on every call of that size) | The body reads `message: openai_error, code: bad_response_status_code`; it comes from the provider-side chain and the gateway does not retry it. Compress reference images to under 10MB each and under 50MB in total (the provider's per-image limit is 50MB) — this also cuts upload waits from minutes to seconds                                                                                                   |
| `401`                             | Invalid token                                                                                                                                     | Check Bearer Token                                                                                                                                                                                                                                                                                                                                                                                                     |
| `400` + `safety_violations=[...]` | Content moderation block (observed status is 400, not 403)                                                                                        | Usually happens after the image is rendered, so the same prompt passes sometimes and fails sometimes; `moderation: low` only affects prompt-side moderation. Add explicit clothing for the character first, then change one word at a time. See [Safety Rejections](/en/api-capabilities/image-safety-troubleshooting)                                                                                                 |
...
<AccordionGroup>
  ...
  <Accordion title="What if I need a transparent background?">
    Just pass `background: "transparent"` with `output_format` set to `png` or `webp`. What comes back is a real alpha-channel image — no cutout post-processing needed. Both text-to-image and image editing support it.
    ...
    The third boundary is the one people miss: **if the prompt or the reference image describes a scene, the model paints the scene** — the parameter cannot override it, and raising `quality` does not help. Describe the subject only and end with `isolated subject on a transparent background, no scenery, no ground`.
  ...
  <Accordion title="gpt-image-2 vs gpt-image-2-all: which to pick?">
    | Pick                                                        | When                                                                                                                                                                             |
    | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | **gpt-image-2.5-flare / sunburst / gpt-image-2** (Official) | Need precise size/quality control, must match OpenAI official exactly, want 4K output, need mask inpainting; flare is the fastest of the three, sunburst the most precise editor |
  ...
  <Accordion title="Is there a prompt length limit? Why can I paste more into the ChatGPT web app?">
    Yes: **32,000 characters**, the OpenAI Images API provider limit, counted in characters not tokens; APIYI's official relay does not tighten it. The web app only looks unlimited because the chat model reads what you paste and then writes its own short prompt for the image tool, so the image model never sees the material. For long material such as brand books or packaging specs, distill it with a text model into a 1K to 3K character structured prompt first, see [Long Prompts](/en/api-capabilities/image-long-prompt).
  </Accordion>

  <Accordion title="400 with safety_violations=[sexual] on a prompt that is not explicit. What now?">
    Most of these blocks happen **after the image is rendered**: the image is generated, then rejected by an output-side classifier, so a failed call takes as long as a successful one and the same prompt passes sometimes and fails sometimes. `moderation: low` only lowers prompt-side moderation and does not help here. The most common cause is a character described by mood but not by clothing, so the model improvises and crosses the line: **add one explicit clothing phrase first** (leave everything else untouched), and only then ablate one word at a time. Full case and the 20-call log: [Safety Rejections](/en/api-capabilities/image-safety-troubleshooting).
  </Accordion>
...
* [Deep Dive: GPT-image-2.5 Launches, Flare Is Faster, Sunburst Is Sharper](/en/news/gpt-image-2-5-launch) - News article
...
* [Community: Luck GPT-Image 2 ComfyUI Nodes](/en/scenarios/ecosystem/luckgpt2-comfyui) - Call `gpt-image-2` / `gpt-image-2.5-flare` / `gpt-image-2.5-sunburst` directly in ComfyUI (mask / 16 reference images / six quality tiers / custom sizes)
...
<Info>
  `gpt-image-2.5-flare` / `gpt-image-2.5-sunburst` / `gpt-image-2` are OpenAI's official models, billed by token. If you prioritize flat pricing (\$0.03/image) and faster generation (30–60s), see [gpt-image-2-all](/en/api-capabilities/gpt-image-2-all/overview).
</Info>
