...
| **Model registry**      | `https://docs.apiyi.com/model-registry.json` | Machine-readable model IDs, endpoints, groups and list prices, so the agent stops guessing |

The skill, the CLI and the contract-plus-registry path for coding agents are all described in the [AI Developer Kit](/en/developer-kit).
