...
> Complete guide to Grok Imagine 2, xAI's latest-generation image model (grok-imagine-image / grok-imagine-image-quality) on APIYI — 5 aspect ratios, 1K/2K tiers, up to 10 images per call, true reference-image editing, flat $0.02 / $0.045 per image regardless of resolution — about 64% of list price at 2K. Not open by default — access requires the dedicated Grok_imagine group.
...
<Warning>
  **🔒 This family is not open by default — access requires the dedicated `Grok_imagine` group**

  Grok Imagine 2 is fully integrated and stable, but it does **not** sit in the `Default` group. Its content-safety policy differs substantially from the other models on the platform — **some categories are not filtered** — so to limit compliance risk we grant access selectively:

  * **Existing customers with \$1,000+ in cumulative spend**: contact support, describe your use case, and we enable it after verification
  * **Everyone else**: apply through [WeCom support](https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec), describing your **use case** and your **content-moderation controls**; we enable it for your account once the request is approved

  Calling the models with a Token that lacks the `Grok_imagine` group returns **`503`** — a permissions problem, not an outage. See [Group Setup](#group-setup) below for how to apply and configure.
</Warning>
...
<Warning>
  **Grok Imagine 2 is not in the `Default` group and is not open by default.** It lives in its own **`Grok_imagine` group (1.0x rate, matching the pricing table above)**, and access must be requested.
</Warning>

**Why a separate group**: this family's content-safety policy differs substantially from the other models on the platform, and some categories are not filtered. To limit compliance risk we keep it out of the default group that every account can reach, and grant access selectively instead.

### Who can get access

| Customer                                              | How access is granted                                                                            |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| **Existing customers with \$1,000+ cumulative spend** | Contact support, describe your use case, enabled after verification                              |
| **Everyone else**                                     | Submit a request describing your use case and content-moderation controls; enabled once approved |

### How to apply

<Steps>
  <Step title="Contact WeCom support">
    Apply through [WeCom support](https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec), or email [hi@apiyi.com](mailto:hi@apiyi.com).
  </Step>

  <Step title="Describe your use case and moderation controls">
    Cover three things: what product the images are for, who the end users are, and what moderation and human review you run on your side. The more specific the request, the faster the review.
  </Step>

  <Step title="Switch your Token to the group">
    Once approved, we enable `Grok_imagine` on your account. Go to the [Token page in the console](https://api.apiyi.com/token) and switch the Token you use for this family to `Grok_imagine`, with the billing model set to `Pay-as-you-go Priority` or `Pay-per-request`.
  </Step>
</Steps>

<Info>
  **What it looks like before access is granted**: a Token without the `Grok_imagine` group returns **`503` (no available channel in the current group)**. Retrying will not help — the group has to be enabled first.
...
<Note>
  **Compliance note**: once access is granted, responsibility for the generated content sits with the caller. Follow the laws that apply to you; do not generate illegal content, content that infringes someone's likeness or intellectual property, or material inappropriate for minors. For consumer-facing distribution we recommend an additional moderation layer on your side. Background reading: [compliance when calling overseas models](/en/faq/overseas-model-compliance). Abuse results in the group being revoked.
</Note>
...
The table uses GPT-Image-2 as the baseline; `gpt-image-2.5-flare` / `gpt-image-2.5-sunburst` share its parameters, so the mapping applies to them as well.
...
| `503` | —                           | Token lacks the `Grok_imagine` group                   | **Permissions, not an outage** — retrying will not help; apply via [Group Setup](#group-setup) |
...
<AccordionGroup>
  <Accordion title="Every call returns 503 — is the channel down? How do I get access?">
    First work out which 503 it is. Passing `resolution: "4k"` means the tier is unsupported (see the entry below). If the parameters are fine and 503 is constant, the Token almost certainly **lacks the `Grok_imagine` group**.

    This family is **not open by default**: its content-safety policy differs substantially from the other models on the platform and some categories are not filtered, so to limit compliance risk we keep it in a separate `Grok_imagine` group and grant access selectively. Existing customers with \$1,000+ in cumulative spend can get it enabled by describing their use case to support; everyone else applies through [WeCom support](https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec) with a description of the use case and the content-moderation controls in place.

    Full process in [Group Setup](#group-setup) above.
  </Accordion>
