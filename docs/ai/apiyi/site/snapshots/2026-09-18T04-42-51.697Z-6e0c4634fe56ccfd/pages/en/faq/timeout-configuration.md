...
<AccordionGroup>
  ...
  <Accordion title="② Reasoning / long-output models: non-streaming can't hold, use streaming">
    Regular text models return in seconds, which makes it easy to assume text calls never need timeout tuning. **Reasoning models — and any workload that produces tens of thousands of characters in one call — are the exception:**

    * `gemini-3.1-pro-preview`
    * `gpt-5.6-sol`
    * `gpt-5.5-pro` (pricier and slower)
    * `claude-opus-5` and other models running deep thinking / high effort
    * Long-output tasks: drama scripts, fiction, long translations, large code generation

    These calls reason for a long time, then emit a long body token by token, and **ten-plus minutes of total latency is normal**.

    **Key point: non-streaming can't hold, but streaming with a sensible read timeout solves it.** Non-streaming buffers the whole generation and sends it at once, so your read timeout has to race the entire generation and any hiccup breaks it. With streaming the first byte arrives within seconds, and after that a data event arrives every few tens of seconds (measured max silent gap during thinking \~42s, with keepalive pings in between), so your read timeout only has to cover the *inter-event gap*, not the total time.

    **Recommendation**:

    * **Prefer streaming for long output** (`stream=True`), with a read timeout of **90-120 seconds** sized to the inter-event gap — not one huge value covering the whole generation.
    * If you insist on non-streaming, set the timeout to **300-600 seconds** and accept the instability; the longer it runs the more likely you disconnect before the result arrives.
    * Match your thinking tier (`reasoning_effort` / `thinking`) to the time you have allowed — a higher tier needs more headroom.

    For the full long-output recipe (streaming code, max\_tokens, stop\_reason, retries) see [Long-form Output Practices](/en/api-capabilities/long-form-output-practices).
  </Accordion>
