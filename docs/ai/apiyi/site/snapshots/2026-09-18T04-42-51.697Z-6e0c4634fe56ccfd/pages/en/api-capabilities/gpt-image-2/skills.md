...
# GPT-Image-2.5 / 2 Series Agent Skill

> Package gpt-image-2.5-flare / gpt-image-2.5-sunburst / gpt-image-2 (official) plus gpt-image-2.5-all / gpt-image-2-all / gpt-image-2-vip (reverse) into one ready-to-use Agent Skill. Drop it into Codex, OpenClaw, hermes-agent, Claude Code, or any coding Agent and switch channels with --model for text-to-image, multi-image fusion, and inpainting.

<Note>
  This page provides a **ready-to-use Agent Skill**: one script covers six models — **gpt-image-2.5-flare / gpt-image-2.5-sunburst / gpt-image-2 (official)** and **gpt-image-2.5-all / gpt-image-2-all / gpt-image-2-vip** (reverse). They all use the same OpenAI Images API; only `--model` differs. Drop it into the coding Agent you already use and generate images with a single prompt — just two files.
</Note>
...
All six models are **called identically**; they differ only in source channel, price/speed, and which params are honored. The three official models (2.5-flare / 2.5-sunburst / 2) share price and parameters, as do the three reverse models. The script handles these differences automatically by `--model`:

| Model (`--model`)                                        | Channel                   | Price                        | Speed                                        | `size`                  | `quality` / `mask`                                                                           | Best for                                                                |
| -------------------------------------------------------- | ------------------------- | ---------------------------- | -------------------------------------------- | ----------------------- | -------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| `gpt-image-2.5-flare` (default)                          | Official passthrough      | token-based \~\$0.03–0.2/img | **fastest official** (1K low measured \~10s) | ✅ any preset            | ✅ yes, incl. `xhigh` / `max`                                                                 | everyday text-to-image default, quality tiers, transparent backgrounds  |
| `gpt-image-2.5-sunburst`                                 | Official passthrough      | token-based, same as flare   | slower than flare                            | ✅ any preset            | ✅ yes, incl. `xhigh` / `max`                                                                 | edits / multi-image fusion / mask inpainting, highest editing precision |
| `gpt-image-2`                                            | Official passthrough      | token-based, same as flare   | \~100–120s                                   | ✅ any preset            | ✅ yes (up to `high`)                                                                         | previous generation, existing integrations                              |
| `gpt-image-2.5-all`                                      | Reverse (ChatGPT web 2.5) | flat \$0.03/img              | \~30–90s                                     | ❌ write into prompt     | ❌                                                                                            | volume, speed, 2.5 quality, sizing via prompt text                      |
| `gpt-image-2-all`                                        | Reverse (ChatGPT line)    | flat \$0.03/img              | **fastest \~30–60s**                         | ❌ write into prompt     | ❌                                                                                            | volume, speed, sizing via prompt text                                   |
| `gpt-image-2-vip`                                        | Reverse (Adobe line)      | flat \$0.03/img              | \~90–150s                                    | ✅ 30 tiers incl. **4K** | ❌                                                                                            | locked output size / 4K at a low price                                  |
| `gpt-image-2.5-flare-vip` / `gpt-image-2.5-sunburst-vip` | Reverse (Adobe line, 2.5) | flat \$0.03/img              | \~20–140s                                    | ✅ 30 tiers incl. **4K** | all six `quality` tiers (incl. `xhigh` / `max`), transparent bg ✅; `mask` whole-image only ❌ | 2.5 quality at the flat price, lockable size                            |

<Tip>
  Quick rule: **everyday text-to-image** → `gpt-image-2.5-flare`; **edits / mask inpainting** → `gpt-image-2.5-sunburst`; **fast & cheap** → `gpt-image-2.5-all` / `gpt-image-2-all`; **locked size/4K & cheap** → `gpt-image-2-vip`.
</Tip>
...
```bash
# Text to image (default gpt-image-2.5-flare official, with quality)
...
# Inpainting (mask, official models only; gpt-image-2.5-sunburst recommended)
...
Switching model is **just `--model`**, one of six values:
...
| --------------------------------------------- | ----------------------------------------------------- |
| "Draw a cinematic cat"                        | Default gpt-image-2.5-flare, 1 png                    |
