...
> GLM-5.3-Flash on APIYI: $0.15 input / $0.5 output per 1M tokens, 1,000,000 context, 128,000 max output, available in 2 billing groups.

The first natively multimodal GLM-5: 320B total / 18B active, 84.3 on Terminal-Bench 2.1 within reach of Claude Opus 4.8, at roughly a tenth of the flagship's cost; MIT-licensed.
...
The example below calls `glm-5.3-flash` through OpenAI Chat Completions (`/v1/chat/completions`). Point base\_url at `https://api.apiyi.com/v1` — everything else matches the official API.
...
<CardGroup cols={2}>
  <Card title="Launch announcement" icon="megaphone" href="/en/news/glm-5-3-launch">
    Background, benchmarks and migration notes for GLM-5.3-Flash
  </Card>

  <Card title="GLM-5.3" icon="git-compare" href="/en/models/glm-5-3">
