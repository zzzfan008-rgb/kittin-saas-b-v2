...
<Warning>
  **Update 2026-09-16 (UTC+8): access to this family is now granted selectively.** Grok Imagine 2 now lives in its own **`Grok_imagine` group** and is **no longer in the `Default` group** — access must be requested. The "callable from the Default Group" statement below reflects the launch date (2026-08-12) and no longer holds.

  Its content-safety policy differs substantially from the other models on the platform and some categories are not filtered, so to limit compliance risk we narrowed access: existing customers with \$1,000+ cumulative spend can get it enabled by describing their use case to support; everyone else applies through [WeCom support](https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec) with a description of the use case and the content-moderation controls in place. Full process: [Grok Imagine 2 Overview - Group Setup](/en/api-capabilities/grok-imagine-image/overview#group-setup). Everything else below — parameters, billing and test results — is unaffected.
</Warning>

## Key Takeaways

* **Two models live**: `grok-imagine-image` (**\$0.02/image**) and `grok-imagine-image-quality` (**\$0.045/image**), requiring the dedicated `Grok_imagine` group (**selective access since 2026-09-16**, see the notice above)
...
**Group**: the dedicated `Grok_imagine` group at 1.0x — **not open by default, access must be requested** (since 2026-09-16, see the notice above). Once enabled, switch your Token to `Grok_imagine` and set the billing model to `Pay-as-you-go Priority`.
