...
# GPT-Image-2.5-All Agent Skill

> Use gpt-image-2.5-all (reverse, ChatGPT line — fastest, flat $0.03/image; gpt-image-2-all uses the same call) inside your Agent. It shares the one gpt-image-2 skill with gpt-image-2 and gpt-image-2-vip; just set --model to gpt-image-2.5-all.
...
After installing the [gpt-image-2 series skill](/en/api-capabilities/gpt-image-2/skills), set `--model` to `gpt-image-2.5-all` (or `gpt-image-2-all`; same price and behavior):
