...
<Info>
  **🔥 Limited-time discount — through 2026-10-07 23:59 (UTC+8)**: two new single-model groups, `SD2Mini` (0.10x) and `SD2Fast` (0.15x), cut the rate by **44.4% for `mini` and 16.7% for `fast`**. Swap in one new Token to get it — no code changes. See "Limited-time discount groups" and "Group Setup" below.
</Info>
...
<Info>
  **Pricing in one line — precise token billing, tier-by-tier pegged to the Volcengine official site.** The four models are **priced differently**: mini \< fast \< standard \< **2.5** (same direction as the official site; mini runs at about half the standard model's unit price and 2.5 at about 1.5× it — **they are NOT the same price level**). Volcengine offers **no discount** on this series, and this channel is a **pure official relay run to secure supply**, not for profit. Pricing is set against the "large-deposit customer with 20% bonus" baseline: the nominal charge is 1.26× the official price (0.18 rate × fixed 1:7 exchange), which lands at **about 1.05× the official price** once the 20% [top-up bonus](/en/faq/recharge-promotions) is applied — that 5% premium mainly covers the 6% VAT cost of issuing special invoices. General customers with the 10% bonus land at roughly 1.15×. Billing is by area×duration, so a **±5% deviation is normal** — you're welcome to test, reconcile, and reach out anytime.

  Beyond unit price, this channel adds two things: **whitelisted accounts can generate real human faces** (blocked by default officially), and the [asset library](/en/api-capabilities/seedance2/asset-library) required for character-consistent video is **included free on APIYI** (officially a separately purchased add-on — a six-figure CNY annual contract for customers without a framework agreement). Neither is reflected in the unit-price comparison above.
</Info>
...
<Note>
  **Every price in table ② is still for a 5-second output**; the range only reflects the input video length (2-15 s) — CNY 30.13 is "5 s output + 15 s input", **not** a 15-second output. With input video, billed duration = **input video duration + output duration**, and both sides are converted to frames at the output resolution; a minimum-token floor also applies (very short inputs bill at the floor). The authoritative usage is the returned `usage.completion_tokens`.

  **Worked example** (standard model, 1080p, 12 s output + 12 s reference video): 1080p is 1920 × 1080 ÷ 1024 = 2025 tokens per frame; output 24 × 12 + 1 = 289 frames plus 288 input frames = 577 × 2025 = 1,168,425 tokens. At the official with-video-input rate of about CNY 0.031 per 1K tokens that is roughly **CNY 36.2**; APIYI's nominal charge is \$6.52 (CNY 45.6), about CNY 38.0 for a large-deposit customer after the 20% bonus. This job matches no cell in table ② because the output is not 5 seconds.
</Note>
...
**Measured on-platform (with input video)** (tested 2026-09-12, standard model 1080p / 9:16 / no audio / 12 s reference video; CNY at the fixed 1:7 rate):

| Model                        | Resolution | Output | Input video | Tokens    | APIYI cost | CNY    | General ÷1.1 (¥) | Large customer ÷1.2 (¥) | Official ref (¥) |
| ---------------------------- | ---------- | ------ | ----------- | --------- | ---------- | ------ | ---------------- | ----------------------- | ---------------- |
| `doubao-seedance-2-0-260128` | 1080p      | 12s    | 12s         | 1,168,425 | \$6.5198   | ¥45.64 | ¥41.49           | ¥38.03                  | ≈ ¥36.2          |

Input video is billed at a **separate, lower token rate** (this 1080p run measured \$5.58 per million tokens versus \$8.28 for 1080p without video), but every second of the input video is converted to tokens at the output resolution and billed alongside the output, so the total is usually higher. **Trim the reference video first** before dropping resolution if you want to save.
...
Note: "CNY" is the on-platform list charge; "General ÷1.1" and "Large customer ÷1.2" are the effective prices after a 10% / 20% [top-up bonus](/en/faq/recharge-promotions) — after the bonus, large-deposit customers pay about 1.05× the official reference price and general customers about 1.15×. The authoritative usage is the returned `usage.completion_tokens`.
...
<Warning>
  **The three easiest ways to overspend on 2.5**:
  ...
  3. **The reference video's duration is billed too.** With `video_url`, every second of input video is converted to tokens at the output resolution and settled together with the output — on both 2.0 and 2.5. The pre-charge only counts the output duration, so the settlement on these jobs is **often several times the pre-charge**; that is expected.
</Warning>
...
<Warning>
  **Nothing gets cut off when the offer ends**: after 2026-10-07 23:59 (UTC+8), `SD2Mini` and `SD2Fast` **stay online** — their rate simply reverts to 0.18x, matching the regular `SeeDance2` group. Your Tokens keep working and no code changes are needed. If you have batch production planned, schedule it inside the discount window.
</Warning>
...
### Looking up one video's cost by task ID: use the task API

For programmatic reconciliation, **do not try to pair log entries one by one**: neither of a video's two log entries carries the `task_id`, and the settlement entry has an empty `request_id` (this is how it works on the documented `/seedance/api/v3/...` path). Use the **task API** and query by `task_id` directly, authenticated with the same system token as the [Log Query API](/en/api-capabilities/log-query):
...
The returned **`quota` is the video's total cost** (pre-charge + settlement, ÷ 500,000 = USD). It is the same number you get by adding the two log entries, and the same `quota` shown in the task detail on the "Async tasks" page. Interpret it by `status`:
...
| `failed`                    | **Still shows the pre-charge** (it is not zeroed) | **0**: the pre-charge is refunded in full, with a matching negative `type=11` refund entry in the logs |

Note the two vocabularies differ: the video query endpoint `/seedance/api/v3/.../tasks/{id}` reports success as `succeeded`, while the task API `/api/task/self` reports it as `completed`.

<Warning>
  **The task API's paging parameters are the opposite of the Log Query API's**: here it is snake\_case `page_size` and the page number `p` starts at **1**; the log API uses camelCase `pageSize` with `p` starting at 0. Getting it wrong does not raise an error, you just get the default page. Verified filters: `task_id`, `model_name`, `start_timestamp` / `end_timestamp` (Unix seconds).
</Warning>

If you must cross-check manually in the Log Query API: the settlement entry's timestamp equals the task's `finish_time` (within 1 second), its `completion_tokens` equals the task's `usage.completion_tokens`, `other.final_quota` equals the task's `quota`, and `other.original_quota` is the pre-charge. When several tasks are submitted in the same second their pre-charge entries collide, so this only works for manual checks, not for automated pairing. Full walkthrough: [How to look up a video's real cost by task\_id](/en/faq/seedance-task-cost-lookup).
...
| `SD2Mini`   | **0.10x** | `doubao-seedance-2-0-mini-260615` only             | 🔥 Limited-time, **44.4% below** the original rate, through 2026-10-07 23:59 (UTC+8)                           |
| `SD2Fast`   | **0.15x** | `doubao-seedance-2-0-fast-260128` only             | 🔥 Limited-time, **16.7% below** the original rate, through 2026-10-07 23:59 (UTC+8)                           |
...
<Note>
  ...
  **No cutoff when the offer ends**: after 2026-10-07 23:59 (UTC+8) both discount groups stay online with the rate reverting to 0.18x — no Token or code changes required.
</Note>
...
| **C (existing · main)** | `SeeDance2`   | 2.5 and the 2.0 standard model; the whole 2.0 family falls back here after October 7 |
...
<Tip>
  ...
  * **You won't miss the deadline** — billing is split per Token, so what you used and saved during the offer is visible at a glance, and it is easier to decide whether to pull batch work forward as October 7 approaches
...
<Tip>
  **Domains**: `api.apiyi.com` is the primary gateway; `vip.apiyi.com` and other platform domains behave identically. The path prefix is `/seedance/api/v3` — **do not drop the `/api` segment**.
</Tip>

<Warning>
  **Do not submit Seedance jobs through the generic video endpoints.** `/v1/videos`, `/v1/video/generations` and `/v2/videos/generations` currently do not pass the Seedance `resolution` parameter through completely — in production we measured **2.5 requests for 480p rendering at 720p, and 2.0-family requests for 1080p rendering at 720p**, with billing settled on the tokens actually rendered (2.25× more in the first case). The issue is identified and being fixed (2026-09-14); until then use only the `/seedance/api/v3/...` endpoints on this page, where resolution is honored. If you were charged for a mismatched resolution on a generic endpoint, contact support for a refund of the difference based on the requested resolution.
</Warning>
...
<Info>
  ...
  * Log the task `id` and the `X-Shellapi-Request-Id` response header (the APIYI request ID, searchable in the logs; `X-Request-Id` is the provider-side one) for troubleshooting
</Info>
