...
<Info>
  **Three sentences:**
  ...
  3. **Everything else is orchestration**: a chat model plus a standalone image endpoint (the Images API) working together. The OpenAI Responses native `image_generation` tool is **not recommended on APIYI** — it can only be billed at a fixed per-call rate, which is not a reasonable pricing model, and its stability cannot be guaranteed.
</Info>
...
| **C. Responses native tool**                   | `gpt-5.5` + `tools: [{"type": "image_generation"}]` | ✅ Yes                                          | **Not recommended on APIYI**: fixed per-call fee, stability not guaranteed — use A |
...
<AccordionGroup>
  ...
  <Accordion title="C. Responses native image_generation tool — not recommended on APIYI">
    This route calls `POST /v1/responses` with `gpt-5.5` and attaches the native image tool:
    ...
    In shape it is the closest thing to a "chat model that draws" on the OpenAI side — **but APIYI does not recommend it**.

    <Warning>
      **Why not:** on APIYI the image tool inside Responses can **only be billed per call** — a fixed tool-call fee
      of roughly \$0.20 per image, with no usage-based option like route A — which is not a reasonable pricing model.
      And because of supply constraints, we **cannot guarantee the stability** of this path.

      For image generation always use route A's Images API (`/v1/images/generations` / `/v1/images/edits`), billed by usage.
      If you want "the agent decides whether to draw", build it with the "chat and draw" orchestration below —
      same result, with clear billing.
    </Warning>
  </Accordion>
...
<AccordionGroup>
  <Accordion title="1. A multimodal model can generate images">
    **False.** In an API context, multimodal defaults to **input-side** capability. `gpt-5.5` can read the design
    mockup you send it, but it cannot emit an image on its own — to get one you need a separate call to an image
    endpoint (route A); the Responses native image tool (route C) is not recommended on APIYI.
  </Accordion>
