...
# GPT-Image-2.5-VIP Agent Skill

> Use gpt-image-2.5-vip (reverse, Adobe line — lockable size/4K, flat $0.03/image; gpt-image-2-vip uses the same call) inside your Agent. It shares the one gpt-image-2 skill with gpt-image-2 and gpt-image-2-all; just set --model to gpt-image-2.5-vip.
...
<CardGroup cols={3}>
  <Card title="Lockable size / 4K" icon="expand">
    Reverse Adobe line (Firefly), supports 30 `size` tiers (incl. 4K like 3840×2160) — controllable dimensions.
  </Card>
...
After installing the [gpt-image-2 series skill](/en/api-capabilities/gpt-image-2/skills), set `--model` to `gpt-image-2.5-vip` (or `gpt-image-2.5-flare-vip` / `gpt-image-2-vip`) and lock the size with `--size`:
...
<Warning>
  **Red lines**: `quality` tiers depend on the model — the two 2.5 models take all six (`xhigh` / `max` opened 2026-09-10), `gpt-image-2-vip` stops at `high` and rejects `xhigh` / `max`; 2.5 `high` only equals `gpt-image-2-vip` `medium`, and 2.5 `max` equals its `high`. **never send `n`** (passing `n>1` still returns 1 image but bills per count), and **mask is whole-image regeneration** (use the official models for precise inpainting). The skill script gates this automatically, so normal `--model gpt-image-2.5-vip` use is safe.
