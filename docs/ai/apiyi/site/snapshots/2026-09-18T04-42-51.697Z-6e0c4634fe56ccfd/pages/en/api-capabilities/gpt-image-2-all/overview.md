...
# GPT-Image-2.5-All Image Gen/Editing
...
**gpt-image-2.5-all** and **gpt-image-2-all** are APIYI's **GPT image generation reverse-engineered models** (ChatGPT web line). The ChatGPT web app has moved to Images 2.5 as a whole, so `gpt-image-2-all` already produces 2.5 images; the new `gpt-image-2.5-all` name simply makes that upgrade explicit — **both names share the same price and behavior and are interchangeable**; new projects should use `gpt-image-2.5-all`. With extremely competitive pricing at **\$0.03/image** per call, it generates images in **about 30–60 seconds** and supports **text-to-image / single-image editing / multi-image fusion / natural-language editing** — with high text-rendering fidelity and native support for Chinese prompts.
...
| `gpt-image-2.5-all` | Per-call | **\$0.03 / image** (same as -all) | 1 image per call, backed by GPT-Image 2.5 |
...
<Tip>
  **Same-price sister model**: [`gpt-image-2-vip`](/en/api-capabilities/gpt-image-2-vip/overview) (Adobe reverse line (Firefly)) — same \$0.03/image, supports 30 explicit sizes (incl. 4K), same call format. Switch when you need locked output dimensions.
</Tip>
...
| `gpt-image-2.5-all` | `Default`    | GPT-Image 2.5 version of the same line, flat \$0.03/img, same groups as -all                                                 |
