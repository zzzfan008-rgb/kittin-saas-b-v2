...
| On Fable 5 / 5.1, one turn suddenly shows `read = 0` and the response `model` is `claude-opus-4-8` (or `claude-opus-5`), or `stop_reason` is `refusal` | Content-safety fallback / refusal fired. A fallback switches the model and therefore the cache key, so that turn cannot read the earlier cache; a cache written by a refused turn is not read back later. See the note below |
...
<Warning>
  **Safety fallback on the Fable family breaks the cache.** `claude-fable-5` / `claude-fable-5-1` ship with built-in safety classifiers. When a request hits high-risk content, the model either **refuses** (HTTP 200 with `stop_reason: "refusal"`) or **falls back** to the Opus family, and the top-level `model` in the response faithfully echoes the model that actually answered. This is normal model-side behavior, not a gateway issue.

  Effect on caching: a fallback switches the model and therefore the cache key, so that turn cannot read what earlier turns wrote; hits resume once the next turn lands back on Fable. A refused turn may still report `cache_creation_input_tokens`, but that write is never read back later. In multi-turn agent sessions this shows up as **isolated turns where `read` drops to 0 and `write` jumps back up**.

  What to do: check the response `model` and `stop_reason` before judging a miss from `usage` alone; adjust a refused input before resending instead of retrying it verbatim; keeping the conversation content within policy is what minimizes these misses. Details on refusals, fallback and billing are in the [Fable 5.1 launch notes](/en/news/claude-fable-5-1-launch).
</Warning>
