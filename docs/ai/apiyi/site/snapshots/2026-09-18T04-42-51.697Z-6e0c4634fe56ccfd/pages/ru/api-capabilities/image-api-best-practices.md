...
## Краткий справочник по сериям моделей

Рекомендуемые тайм-ауты, форматы вывода и поддержка URL для каждой серии моделей генерации изображений:

| Серия моделей                                                                                      | Эндпоинт                                                                     | Рекомендуемый тайм-аут                                                                                           | Формат вывода                                                                                                                     | Вывод URL и срок действия                                                                                        |
| -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| [GPT-Image-2.5 / 2 (Официальный: flare / sunburst / 2)](/ru/api-capabilities/gpt-image-2/overview) | `/v1/images/generations`, `/v1/images/edits`                                 | **360s** (для gpt-image-2 high + 2K/4K измерено 3–5 минут; 2.5 быстрее, но оставляйте запас для `xhigh` / `max`) | Сырой b64\_json (**без префикса `data:`**)                                                                                        | ❌ Не поддерживается (`response_format` возвращает 400; группа `image2_OSS` пока не охватывает официальный канал) |
| [GPT-Image-2-All (Обратный)](/ru/api-capabilities/gpt-image-2-all/overview)                        | То же, что выше                                                              | **300s**                                                                                                         | По умолчанию `b64_json` (**без префикса `data:`**, проверено 2026-07); можно переключить явным указанием `response_format: "url"` | ✅ Явный `url`: R2 CDN, \~24 часа; используйте группу `image2_OSS`, если вы зависите от URL                       |
| [GPT-Image-2-VIP](/ru/api-capabilities/gpt-image-2-vip/overview)                                   | То же, что выше                                                              | **300s**                                                                                                         | То же, что All (по умолчанию `b64_json`, без префикса, проверено 2026-07)                                                         | ✅ То же, что All (явный `url` или группа `image2_OSS`)                                                           |
| [Nano Banana Pro](/ru/api-capabilities/nano-banana-image/overview)                                 | Нативный Gemini `:generateContent`                                           | 1K/2K **300s**, 4K **600s**, несколько изображений — 5+ минут                                                    | Сырой base64 в `inlineData.data`                                                                                                  | Бета-группа `NB_OSS` (см. ниже)                                                                                  |
| [Nano Banana 2](/ru/api-capabilities/nano-banana-2-image/overview)                                 | То же, что выше                                                              | **360s**                                                                                                         | То же, что выше                                                                                                                   | Покрытие `NB_OSS`: обратитесь в поддержку                                                                        |
| [Nano Banana Lite](/ru/api-capabilities/nano-banana-lite-image/overview)                           | То же, что выше                                                              | **300s** (\~4s обычно, запас на пиковую нагрузку)                                                                | То же, что выше                                                                                                                   | Покрытие `NB_OSS`: обратитесь в поддержку                                                                        |
| [FLUX](/ru/api-capabilities/flux/overview)                                                         | `/v1/images/generations`                                                     | **60-120s**, 180s для flex-моделей                                                                               | Только `data[0].url` (**URL используется upstream по умолчанию**)                                                                 | ⚠️ Действителен лишь \~**10 минут**, без CORS — немедленно разместите на своём сервере                           |
| [Seedream](/ru/api-capabilities/seedream-image/overview)                                           | `/v1/images/generations` (унифицированный эндпоинт генерации/редактирования) | **60s** (4K + hd около 30-60s)                                                                                   | По умолчанию `url` (**URL используется upstream по умолчанию**); опционально `b64_json` (сырой base64, без префикса)              | ✅ BytePlus TOS, \~24 часа                                                                                        |

<Tip>
  `response_format` имеет **ограниченную поддержку**: его принимают только GPT-Image-2-All / VIP и Seedream; официальный канал GPT-Image-2 возвращает 400 `unknown_parameter`, если вы передаёте его. Там, где это поддерживается, **всегда передавайте его явно**, а не полагайтесь на значение по умолчанию — исторически оно различалось между группами и в зависимости от условий нагрузки.
</Tip>
...
### Почему тайм-ауты по умолчанию приводят к сбоям

Большинство HTTP-клиентов по умолчанию используют тайм-ауты в 30–60 секунд (у `requests` собственного ограничения нет, но фреймворки часто оборачивают его, добавляя около 30 секунд), тогда как генерация изображений — это действительно долгий запрос:

* GPT-Image-2 с качеством `high` и разрешением 2K/4K выполняется **3–5 минут** от начала до конца;
* Изображения Nano Banana серии 4K начинают генерироваться примерно за 50 секунд, а в часы пик это занимает больше времени;
* Запросы на объединение нескольких изображений и редактирование изображений обычно выполняются дольше, чем преобразование текста в изображение.

При настройках по умолчанию клиент разрывает соединение, пока сервер всё ещё выполняет генерацию в штатном режиме. В результате вы видите множество «тайм-аутов», которые на самом деле являются **успешными запросами, соединение с которыми вы разорвали самостоятельно**, и каждый из них тарифицируется.

### Уровни тайм-аутов для отдельных моделей
...
Не каждый сбой заслуживает повторной попытки — сначала определите, как тарифицируется каждый случай:

| Сбой                                                        | Тарифицируется?                             | Рекомендация                                                                                           |
| ----------------------------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| 429 / 503 (лимит запросов, перегрузка вышестоящего сервиса) | Не тарифицируется                           | Повторите запрос с экспоненциальной задержкой (например, через 5, 15 и 45 секунд)                      |
| Тайм-аут клиента / преждевременный разрыв соединения        | **Тарифицируется**                          | Сначала увеличьте тайм-аут; если повторная попытка необходима, тщательно ограничьте количество попыток |
| 400 / 403 (ошибки параметров или разрешений)                | Не тарифицируется                           | Исправьте запрос перед повторной отправкой — бездумные повторные попытки бессмысленны                  |
| Блокировка модерацией контента                              | **Зависит от модели** (см. примечание ниже) | Измените промпт; повторная отправка без изменений, скорее всего, снова будет заблокирована             |

<Info>
  **Тарификация при блокировке модерацией зависит от модели**: модели с тарификацией по token (официальные GPT-Image-2 и другие) обычно возвращают ошибку 400 при срабатывании модерации — **тарификация не производится**. Только **Nano Banana Pro с тарификацией за изображение** сталкивается с блокировкой на стороне Google по сценарию «HTTP 200, но генерация не выполнена», и такой вызов **тарифицируется**. APIYI компенсирует эти сбои, не зависящие от пользователя, в рамках [Плана компенсации кредитов за неудачную генерацию](/ru/api-capabilities/nano-banana-pro-guarantee), возмещая кредиты на основе подсчёта изображений.
</Info>
