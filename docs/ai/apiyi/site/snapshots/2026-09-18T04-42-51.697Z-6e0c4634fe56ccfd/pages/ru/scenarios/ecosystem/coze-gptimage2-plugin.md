...
[APIYI](https://api.apiyi.com) — это шлюз для GPT Image 2 с прямым подключением из материкового Китая, предлагающий три маршрута с единым API Key:
...
| ID модели         | Канал                    | Тарификация          | Скорость генерации | Основные особенности                                                                                   |
| ----------------- | ------------------------ | -------------------- | ------------------ | ------------------------------------------------------------------------------------------------------ |
| `gpt-image-2`     | Официальный релей        | Тарификация за token | \~120 с            | Полная совместимость с официальным API OpenAI, поддержка качества, размера и 4K                        |
| `gpt-image-2-all` | Обратный (ChatGPT)       | \$0.03/изображение   | 30–60 с            | Удобен для пользователей из Китая, вызывается через эндпоинт Chat, напрямую возвращает URL изображений |
| `gpt-image-2-vip` | Обратный (линейка Adobe) | \$0.03/изображение   | 90–150 с           | 30 фиксированных пресетов размеров, включая 4K                                                         |

> Этот плагин по умолчанию использует **`gpt-image-2` (официальный релей)**, полностью совместим с официальным API OpenAI и поддерживает полный контроль параметров. Если вам нужна более высокая скорость генерации, переключитесь в режим `gpt-image-2-all` (см. ниже).

<Tip>
  Запросите API Key (начинающийся с `sk-`) в [консоли APIYI](https://api.apiyi.com/token). Рекомендуем установить дневной лимит квоты (например, ¥20–50), чтобы контролировать расходы.
</Tip>
