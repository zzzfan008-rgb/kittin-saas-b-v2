...
**Режим b64\_json (возвращает данные изображения в формате base64)**:
...
<Tip>
  Для импорта `undici` выше требуется отдельная установка (`npm i undici`) — он обеспечивает встроенный `fetch`, но не предоставляется под этим именем модуля, а установленная вами копия по-прежнему обращается к встроенному `fetch` через `setGlobalDispatcher`.

  `maxRetries`, `connectTimeout` и общий тайм-аут запроса находятся на разных уровнях, и настройка неправильного параметра — самая распространённая проблема на стороне Node. При сбросах соединения, `UND_ERR_CONNECT_TIMEOUT` или усечённых больших ответах за прокси см. [Устранение разрывов соединения с Image API](/ru/api-capabilities/image-connection-drops).
</Tip>
...
| ----------------- | ------ | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `model`           | строка | Да           | `gpt-image-2.5-all` или `gpt-image-2-all` (одинаковые цена и поведение)                                                                                                                    |
| `prompt`          | строка | Да           | Промпт. Укажите здесь размер/соотношение сторон/стиль                                                                                                                                      |
| `size`            | строка | Нет          | Допускается только `auto` (одинаковый промпт приводит к разным размерам). Для строгой фиксации размера используйте [`gpt-image-2-vip`](/ru/api-capabilities/gpt-image-2-vip/text-to-image) |
| `response_format` | строка | Нет          | `b64_json` (по умолчанию, необработанный base64) или `url` (возвращает ссылку CDN R2); всегда передавайте явно                                                                             |

<Tip>
  Подробные ограничения параметров и допустимые значения отображаются в расположенной справа песочнице. Поле `response_format` поддерживает выбор из раскрывающегося списка.
</Tip>
