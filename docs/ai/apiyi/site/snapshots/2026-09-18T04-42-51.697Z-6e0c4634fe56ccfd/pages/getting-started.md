...
| **模型注册表**  | `https://docs.apiyi.com/model-registry.json` | 让 Agent 查模型 ID、端点、分组、标价的机器可读源，不再凭记忆编 |

Skills 技能包、命令行工具、给编程 Agent 的契约与注册表，三条路的完整说明见 [AI 开发者套件](/developer-kit)。
