...
**gpt-image-2.5-all** 与 **gpt-image-2-all** 是 API易 平台的 **GPT 图像生成官逆模型**（逆向 ChatGPT 网页版）。ChatGPT 网页版已整体升级到 Images 2.5，所以 `gpt-image-2-all` 现在出的就是 2.5 的图；新增的 `gpt-image-2.5-all` 只是把这次升级表达在模型名上，**两个名字同价、同行为、可互换**，新项目直接用 `gpt-image-2.5-all`。以 **\$0.03/张** 的极具竞争力的按次计费定价，**约 30–60 秒出图**，支持 **文生图 / 单图编辑 / 多图融合 / 自然语言改图**，文字还原度高、原生支持中文提示词。
...
| `gpt-image-2.5-all` | 按次计费 | **\$0.03 / 张**（与 -all 相同） | 单次返回 1 张图片，模型为 GPT-Image 2.5 |
...
<Tip>
  **同价姐妹模型**：[`gpt-image-2-vip`](/api-capabilities/gpt-image-2-vip/overview)（Adobe 官逆线路（Firefly））同样 \$0.03/张，支持 30 档常见 size（含 4K），调用方式与本模型完全一致——需要锁定输出尺寸时切过去即可。
</Tip>
...
| `gpt-image-2.5-all` | `Default`    | 同上线路的 GPT-Image 2.5 版本，统一 \$0.03/张，分组与 -all 相同   |
...
<Warning>
  本模型为**自适应输出尺寸**，不等同于官转 `gpt-image-2` API。需要严格锁定输出尺寸或 4K 时，请改用 [`gpt-image-2-vip`](/api-capabilities/gpt-image-2-vip/overview)（Adobe 官逆线路（Firefly），30 档 size 含 4K）；需要官方完全一致字段时，请使用 [`gpt-image-2`](/api-capabilities/gpt-image-2/overview)（官转）。
</Warning>
