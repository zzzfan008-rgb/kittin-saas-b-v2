...
> 把 Nano Banana Pro（gemini-3-pro-image）封装成开箱即用的 Agent Skill，丢进 Codex、OpenClaw、hermes-agent、Claude Code 等任意编码 Agent，一句话即可调用 API易 平台完成文生图与图片编辑。

<Note>
  本页提供一个**开箱即用的 Agent 技能（Skill）**：把它放进你正在用的编码 Agent，就能用自然语言（或显式命令）直接调用 API易 平台的 **Nano Banana Pro**（`gemini-3-pro-image`）完成生图与改图。整套东西就两个文件，复制即用。
</Note>
...
在 `nano-banana-pro/.env` 里写上你的 **API易 API Key**（在 `api.apiyi.com` 控制台创建）：
...
# Nano Banana Pro 出图技能

通过 API易 平台调用 Nano Banana Pro（`gemini-3-pro-image`）生成或编辑图片。
...
```bash
# 文生图（默认出 1 张）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "一只戴着宇航头盔的柴犬，电影感打光" -o dog.png --size 2K --aspect 16:9

# 图片编辑（传 1 张）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "把背景换成赛博朋克城市夜景" -i input.jpg -o edited.png

# 多图合成（传多张，重复 -i）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "把这两个人合到同一张办公室合影里" -i a.png -i b.png -o merged.png

# 一次出多张同主题变体（最多 5 张，并发生成）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "国风山水插画" -o shanshui.png -n 3 --aspect 16:9
```
...
- `--aspect`：宽高比，10 选 1（`1:1` `2:3` `3:2` `3:4` `4:3` `4:5` `5:4` `9:16` `16:9` `21:9`），默认 `1:1`。
...
<Tip>
  `name` 必须是小写字母 + 连字符，且**不能含 `claude` / `anthropic`** 等保留词。在支持斜杠命令的 Agent 里，目录名就是命令名——叫 `nano-banana-pro` 即 `/nano-banana-pro`。`${CLAUDE_SKILL_DIR}` 是 Claude Code 提供的技能目录变量；其他 Agent 直接用脚本的实际路径即可。
</Tip>
...
```python theme={null}
#!/usr/bin/env python3
"""通过 API易 调用 Nano Banana Pro（gemini-3-pro-image）生成 / 编辑图片。纯标准库，零依赖。"""
import argparse
import base64
import json
import os
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Nano Banana Pro 出图")
    ...
    parser.add_argument("--aspect", default="1:1", help="宽高比，如 16:9")
    parser.add_argument("--size", default="2K", help="分辨率 1K / 2K / 4K")
...
| 你说                               | 技能行为                              |
| -------------------------------- | --------------------------------- |
| "用 nano banana 画一张 16:9 的雪山日出海报" | 调脚本（无 `-i`），出 1 张 png             |
| "把 photo.jpg 的背景虚化，突出人物"         | 调脚本 `-i photo.jpg`，出 1 张 png      |
| "国风山水来 3 张不同的"                   | 调脚本 `-n 3`，并发出 3 张 png            |
| "把 a.png 和 b.png 里的人合成一张合影"      | 调脚本 `-i a.png -i b.png`，出 1 张 png |
...
* **支持斜杠命令的 Agent**（如 Claude Code）：
  ...
  ```text theme={null}
  /nano-banana-pro 一只在花园里打盹的橘猫，油画风格 --size 4K --aspect 3:2
  ```

* **任意 Agent / 直接命令它跑脚本**（最通用）：

  ```text theme={null}
  运行 python3 nano-banana-pro/scripts/nano_banana.py "一只在花园里打盹的橘猫，油画风格" --size 4K --aspect 3:2
  ```
...
<Info>
  Nano Banana Pro 有严格的内容安全管控。若脚本提示 `finishReason` 非 `STOP` 或返回的是拒绝说明文本，请按提示调整内容，不要对同一违规提示词反复重试。详见 [生图接口的错误处理指南](/api-capabilities/nano-banana-image/overview#api-错误处理指南)。
</Info>

## 相关文档

* [Nano Banana Pro 图片生成总览](/api-capabilities/nano-banana-image/overview)
