...
<Warning>
  **🔒 本系列默认不对外开放**：Grok Imagine 2 不在 `Default` 默认分组，独立放在 **`Grok_imagine` 专属分组**，需申请开通后才能调用（含本页 Playground）。未开通时调用固定返回 `503`。

  该系列的内容安全策略与平台其它模型差异较大，部分类别不作过滤，为避免合规风险我们采取定向开放：累计消费满 \$1,000 的存量客户联系客服说明用途即可开通，其他客户请通过[企业微信客服](https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec)提交申请，说明使用场景与内容管控措施。申请流程见 [Grok Imagine 2 概览 · 分组介绍](/api-capabilities/grok-imagine-image/overview#分组介绍)。
</Warning>
