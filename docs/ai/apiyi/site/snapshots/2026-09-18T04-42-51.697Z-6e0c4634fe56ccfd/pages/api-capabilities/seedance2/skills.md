...
<Tip>
  简单记：**日常与 Agent 场景就用默认 `mini`**；**要 1080p 或最高质量** → `--model std`；**要 30 秒长片、30 张参考图或 mov 输出** → 只能 `--model sd25`（约为 `std` 的 1.5 倍价，分组与 2.0 系相同）。同档位全比例同价，时长按秒线性计费，帧率固定 24fps。特价分组 `SD2Mini` / `SD2Fast` 截至 **2026 年 10 月 7 日 23:59 (UTC+8)**，到期分组不下线、倍率恢复 0.18x，详见 [概览页的分组说明](/api-capabilities/seedance2/overview)。
</Tip>
