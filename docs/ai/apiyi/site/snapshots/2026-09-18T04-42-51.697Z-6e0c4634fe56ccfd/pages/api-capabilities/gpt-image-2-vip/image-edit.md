...
<Warning>
  ...
  **推荐做法**：想要 base64 或要传超大参考图时，**复制下方"代码示例"到本地运行**，代码会自动处理上传与解码。
</Warning>
...
| 字段       | 类型   | 必填 | 说明                                                                                                                            |
| -------- | ---- | -- | ----------------------------------------------------------------------------------------------------------------------------- |
| `model`  | text | 是  | `gpt-image-2.5-vip`（= `gpt-image-2.5-sunburst-vip`）/ `gpt-image-2.5-flare-vip` / 上一代 `gpt-image-2-vip`，同价同调用；`mask` 三款都只做整图重绘 |
...
与文生图接口一致：**默认返回 base64**（`data[0].b64_json`，纯 base64 无前缀，2026-07 实测）。如需 **图片 URL**：显式传 `response_format: "url"` 即可；**强依赖 URL 输出**的业务建议把令牌分组切到 **`image2_OSS`**，稳定输出 URL、不降级为 base64。`data[0]` 中只会出现 `url` 或 `b64_json` 之一，不会两者都返回。
...
<CardGroup cols={2}>
  ...
  <Card title="姐妹模型 gpt-image-2-all" icon="copy" href="/api-capabilities/gpt-image-2-all/image-edit">
    不需要锁尺寸时调用方式一致，出图更快
  </Card>
</CardGroup>
