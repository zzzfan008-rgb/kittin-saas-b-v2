...
> 在 Agent 里用 gpt-image-2.5-all（官逆·ChatGPT 线，最快、flat $0.03/张；gpt-image-2-all 同调用）出图——与 gpt-image-2、gpt-image-2-vip 共用同一个 gpt-image-2 技能，只需把 --model 设为 gpt-image-2.5-all。
...
装好 [gpt-image-2 系列技能](/api-capabilities/gpt-image-2/skills) 后，把 `--model` 设为 `gpt-image-2.5-all`（或 `gpt-image-2-all`，两者同价同行为）即可：

```bash theme={null}
python3 gpt-image-2/scripts/gpt_image.py "扁平插画风节日海报，竖版 2:3" -o poster.png --model gpt-image-2.5-all
```
