...
> 在 Agent 里用 gpt-image-2.5-vip（官逆·Adobe 线，可锁尺寸/4K，flat $0.03/张；gpt-image-2-vip 同调用）出图——与 gpt-image-2、gpt-image-2-all 共用同一个 gpt-image-2 技能，只需把 --model 设为 gpt-image-2.5-vip。
...
<CardGroup cols={3}>
  <Card title="能锁尺寸 / 4K" icon="expand">
    官逆 Adobe 线，支持 30 档 `size`（含 3840×2160 等 4K），尺寸可控。
  </Card>
...
装好 [gpt-image-2 系列技能](/api-capabilities/gpt-image-2/skills) 后，把 `--model` 设为 `gpt-image-2.5-vip`（或 `gpt-image-2.5-flare-vip` / `gpt-image-2-vip`），并用 `--size` 锁定尺寸：
...
<Warning>
  **红线**：`quality` 按模型选档——2.5 两款六档全开（`xhigh` / `max` 2026-09-10 放开），`gpt-image-2-vip` 只到 `high`，传 `xhigh` / `max` 会被拒；2.5 的 `high` 只等于 `gpt-image-2-vip` 的 `medium`、2.5 的 `max` 才等于它的 `high`。**不要传 `n`**（传 `n>1` 仍只出 1 张但会按张数扣费），**掩码只是整图重绘**（要精确局部重绘请用官转）。技能脚本已对本模型自动门控，正常 `--model gpt-image-2.5-vip` 不会踩坑。
