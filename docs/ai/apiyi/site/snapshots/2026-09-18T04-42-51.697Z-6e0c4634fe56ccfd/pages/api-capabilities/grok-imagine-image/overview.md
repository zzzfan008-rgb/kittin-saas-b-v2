...
> xAI 最新一代图像生成模型 Grok Imagine 2（grok-imagine-image / grok-imagine-image-quality）完整指南，支持 5 种宽高比、1K/2K 双档分辨率、单次最多 10 张、真参考图编辑，按次计费 $0.02 / $0.045 一张，不区分分辨率，出 2K 约合官网 6.4 折。本系列默认不对外开放，需申请开通 Grok_imagine 专属分组。
...
<Warning>
  **🔒 本系列默认不对外开放，需申请开通 `Grok_imagine` 专属分组**

  Grok Imagine 2 已完成接入并稳定可用，但**不在 `Default` 默认分组**里。该系列的内容安全策略与平台其它模型差异较大，**部分类别不作过滤**，为避免合规风险，我们采取**定向开放**：
  ...
  * **其他客户**：通过[企业微信客服](https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec)提交申请，说明**使用场景**与**内容管控措施**，审核通过后我们为你单独开通
  ...
  令牌分组不含 `Grok_imagine` 时调用会返回 **`503`**，这是权限问题、不是服务故障。申请方式与开通后的配置见下方 [分组介绍](#分组介绍)。
</Warning>
...
<Warning>
  **Grok Imagine 2 不在 `Default` 默认分组，默认不对外开放。** 本系列独立放在 **`Grok_imagine` 专属分组（1.0x 倍率，与上方定价表一致）**，需申请开通后才能调用。
</Warning>

**为什么单独开一个分组**：该系列的内容安全策略与平台其它模型差异较大，部分类别不作过滤。为避免合规风险，我们不把它放进面向所有用户的默认分组，而是采取定向开放。
...
| ----------------------- | ---------------------------- |
| **累计消费满 \$1,000 的存量客户** | 联系客服说明用途，核验后开通               |
| **其他客户**                | 提交申请，说明使用场景与内容管控措施，审核通过后单独开通 |

### 怎么申请

<Steps>
  <Step title="联系企业微信客服">
    通过[企业微信客服](https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec)提交申请，也可发邮件到 [hi@apiyi.com](mailto:hi@apiyi.com)。
  </Step>

  <Step title="说明使用场景与内容管控措施">
    请写清楚三件事：出图用于什么业务、面向什么终端用户、你这一侧有哪些内容审核与人工复核措施。信息越具体，审核越快。
  </Step>

  <Step title="开通后切换令牌分组">
    审核通过后我们为你的账号开通 `Grok_imagine` 分组。请到[控制台令牌页](https://api.apiyi.com/token)把调用本系列的令牌分组切到 `Grok_imagine`，计费模式选 `按量优先` 或 `按次计费`。
  </Step>
</Steps>

<Info>
  **未开通时的表现**：令牌分组不含 `Grok_imagine` 时调用返回 **`503`（当前分组无可用渠道）**，重试无效，需要先完成开通。

  **令牌「计费模式」推荐**：选 `按量优先`（Pay-as-you-go Priority）—— 本系列是按次计费模型，按量优先与按次计费都能正常路由，选按量优先可以让同一把令牌兼容站内其它按 token 计费的模型。
</Info>

<Note>
  **合规提醒**：开通后生成内容的合规责任在调用方。请遵守当地法律法规，不得用于生成违法内容、侵犯他人肖像权与知识产权的内容，或面向未成年人的不当内容；面向 C 端分发的场景建议在你这一侧再加一层审核。相关背景见 [国内产品调用海外模型的合规要点](/faq/overseas-model-compliance)。发现滥用我们会收回该分组权限。
</Note>
...
下表以 GPT-Image-2 为基准；`gpt-image-2.5-flare` / `gpt-image-2.5-sunburst` 参数与之相同，对照同样适用。
...
| `503` | —                           | 令牌未开通 `Grok_imagine` 分组     | **权限问题不是故障**，重试无效，先按[分组介绍](#分组介绍)申请开通 |
...
<AccordionGroup>
  <Accordion title="调用一直返回 503，是渠道挂了吗？怎么申请开通？">
    先看是哪一种 503。传了 `resolution: "4k"` 是参数不支持（见下一条）；参数没问题却固定 503，基本就是**令牌没有 `Grok_imagine` 分组权限**。

    本系列**默认不对外开放**：它的内容安全策略与平台其它模型差异较大，部分类别不作过滤，我们为避免合规风险把它单独放在 `Grok_imagine` 分组，采取定向开放。累计消费满 \$1,000 的存量客户联系客服说明用途即可开通；其他客户通过[企业微信客服](https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec)提交申请，说明使用场景与内容管控措施，审核通过后我们为你单独开通。
