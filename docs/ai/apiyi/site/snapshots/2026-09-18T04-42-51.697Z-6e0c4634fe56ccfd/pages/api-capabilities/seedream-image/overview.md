...
<AccordionGroup>
  ...
  <Accordion title="image 字段接受 base64 吗？">
    **接受**（已实测验证）。格式必须是 data URI：`data:image/<格式>;base64,<base64编码>`，注意 `<格式>` 小写（如 `data:image/jpeg;base64,...`），URL 与 base64 也可以混在同一个数组里。**但请优先传公网 URL**：URL 由 BytePlus 直接从新加坡下载，不经过 API易 网关；base64 请求体要先整体跨境上传，20 MB 以上的请求体在跨境链路慢时会撞原厂 600 秒请求体超时而失败，网关不会替你把 base64 转成 URL。细节见图片编辑页顶部的提示。
  </Accordion>
