...
<AccordionGroup>
  <Accordion title="① 동기식 이미지 엔드포인트: 타임아웃을 360초로 설정">
    모든 APIYI 이미지 모델은 **동기식**입니다. 요청을 보내고 연결을 유지하면 결과가 응답 본문으로 반환됩니다. 비동기 작업 ID나 폴링 엔드포인트는 없으므로 연결을 끊으면 결과가 사라집니다.

    **기본값이 문제가 되는 이유**: 주류 HTTP 클라이언트의 기본값은 30\~60초인 반면, 이미지 생성은 실제로 상당히 오래 걸리는 요청입니다.

    * `high` 품질의 GPT-Image-2는 2K/4K에서 실제로 3\~5분이 걸립니다.
    * Nano Banana 4K 생성은 약 50초부터 시작하며, 사용량이 많은 시간대에는 더 오래 걸립니다.
    * 여러 이미지를 참조하는 작업은 5분을 넘는 경우가 많습니다.

    **권장 사항**: 모델의 지연 시간을 확신할 수 없다면 **360초**를 기준값으로 사용하고, 4K 및 여러 이미지 참조와 같은 무거운 작업에는 **600초**를 지정합니다. 모델별 값은 [이미지 API 모범 사례](/ko/api-capabilities/image-api-best-practices)에 나와 있습니다.

    <Tip>
      로그에는 이미지가 30초 만에 완료된 것으로 표시되지만 클라이언트는 5분을 기다리는 경우가 있습니다. 로그의 소요 시간은 게이트웨이가 처리를 완료하는 시점까지만 측정하는 반면, 클라이언트는 본문이 모두 도착하고 연결 종료 신호가 전달될 때까지 기다리기 때문입니다. **타임아웃을 늘리는 것이 항상 도움이 되지는 않습니다**. 느린 다운로드 유형에는 도움이 되지만, 데이터는 이미 완전히 도착했고 종료 신호만 누락된 경우에는 도움이 되지 않습니다. [로그에는 완료로 표시되지만 클라이언트에는 아무것도 도착하지 않음](/ko/faq/log-duration-vs-client-wait)의 방법에 따라 한 번 측정한 후 결정하시기 바랍니다.
    </Tip>
  </Accordion>

  <Accordion title="② 추론 / 장문 출력 모델: 비스트리밍은 유지하기 어려우므로 스트리밍 사용">
    일반 텍스트 모델은 몇 초 만에 응답하므로 텍스트 호출에는 타임아웃 조정이 필요하지 않다고 생각하기 쉽습니다. **추론 모델과 한 번의 호출에서 수만 자를 생성하는 모든 작업은 예외입니다.**

    * `gemini-3.1-pro-preview`
    * `gpt-5.6-sol`
    * `gpt-5.5-pro` (더 비싸고 느림)
    * `claude-opus-5` 및 심층 사고 / 높은 추론 수준으로 실행되는 기타 모델
    * 장문 출력 작업: 드라마 대본, 소설, 긴 번역, 대규모 코드 생성

    이러한 호출은 오랫동안 추론한 다음 긴 본문을 token 단위로 출력하므로, **전체 지연 시간이 10분을 넘는 것도 정상입니다.**

    **핵심: 비스트리밍은 유지하기 어렵지만, 적절한 읽기 타임아웃을 적용한 스트리밍으로 해결할 수 있습니다.** 비스트리밍은 전체 생성을 버퍼링한 후 한 번에 전송하므로 읽기 타임아웃이 전체 생성 시간과 경쟁해야 하며, 중간에 문제가 한 번만 발생해도 실패합니다. 스트리밍에서는 첫 바이트가 몇 초 안에 도착하고, 그 후에는 수십 초마다 데이터 이벤트가 도착합니다(추론 중 측정된 최대 무응답 간격은 약 42초이며, 그 사이에 연결 유지 핑이 전송됨). 따라서 읽기 타임아웃은 전체 시간이 아니라 *이벤트 간 간격*만 감당하면 됩니다.

    **권장 사항**:

    * **장문 출력에는 스트리밍을 우선 사용합니다**(`stream=True`). 이벤트 간 간격에 맞춰 읽기 타임아웃을 **90\~120초**로 설정하며, 전체 생성 시간을 아우르는 지나치게 큰 값을 사용하지 않습니다.
    * 비스트리밍을 고집한다면 타임아웃을 **300\~600초**로 설정하고 불안정성을 감수해야 합니다. 실행 시간이 길어질수록 결과가 도착하기 전에 연결이 끊길 가능성이 커집니다.
    * 허용한 시간에 맞춰 사고 단계(`reasoning_effort` / `thinking`)를 선택하시기 바랍니다. 단계가 높을수록 더 많은 여유 시간이 필요합니다.

    전체 장문 출력 설정(streaming 코드, max\_tokens, stop\_reason, 재시도)은 [장문 출력 모범 사례](/ko/api-capabilities/long-form-output-practices)를 참조하시기 바랍니다.
  </Accordion>

  <Accordion title="③ Base URL 선택: CDN 노드는 장시간 요청을 처리할 수 없음">
    APIYI의 `api-cf.apiyi.com`는 **Cloudflare 글로벌 CDN**을 통해 제공됩니다. 전 세계 가속과 해외에서의 낮은 지연 시간을 제공하지만, **약 100초의 요청 타임아웃**이 있어 이를 초과하면 `524` 오류가 발생합니다.

    ⚠️ **이는 이미지 엔드포인트에만 해당하지 않습니다.** 100초를 초과할 수 있는 모든 호출에 적합하지 않으며, 다음이 포함됩니다.

    * ❌ 이미지 생성 / 편집
    * ❌ 동영상 생성
    * ❌ 장문 텍스트 출력(긴 문서, 대규모 번역, 대규모 코드 생성)
    * ❌ 추론 모델의 심층 사고 작업

    ✅ **적합한 작업**: 100초 이내에 완료되는 일반 채팅 및 짧은 생성 작업입니다.

    **권장 사항**: 장시간 요청에는 `api.apiyi.com`(중국 본토에서 권장) 또는 `vip.apiyi.com`(해외에서 권장)을 사용하시기 바랍니다. 전체 노드 비교는 [Base URL 가이드](/ko/faq/base-url-config)에서 확인할 수 있습니다.
  </Accordion>

  <Accordion title="④ 429 동시 실행 수 제한에 도달한 경우: 지원팀에 문의">
    타임아웃과 함께 `429 Too Many Requests`가 자주 발생한다면, 문제는 대개 타임아웃이 아니라 **동시 실행 수 쿼터**입니다.

    동시 실행 수 제한은 계정 전체가 아니라 **모델별로** 적용됩니다. 특정 모델, 특히 새로 출시되었거나 공급이 제한된 모델은 더 낮은 쿼터를 가질 수 있습니다.

    **조치 방법:**

    1. 짧은 시간에 제한을 초과하지 않도록 지수 백오프를 구현합니다.
    2. 429 오류가 계속되면 **APIYI 지원팀에 문의**하시기 바랍니다. 해당 모델의 실제 쿼터를 확인하고 조정을 도와드릴 수 있습니다.

    자세한 규칙은 [얼마나 많은 동시 실행 수를 사용할 수 있나요?](/ko/faq/api-concurrency)를 참조하시기 바랍니다.
  </Accordion>
</AccordionGroup>
