...
<AccordionGroup>
  <Accordion title="502 발생 중 요청에도 과금되나요?">
    **아니요.** 502는 요청이 실제로 백엔드 서비스에 도달하지 않았음을 의미합니다. 모델 사용이 발생하지 않았으므로 **과금 기록에 아무것도 표시되지 않습니다**.

    이는 유용한 진단 방법이기도 합니다. 실패한 요청에 [호출 로그](/ko/faq/call-logs)의 과금 항목이 없다면 서버 측에서 처리되지 않은 것이므로 안전하게 다시 전송할 수 있습니다.
  </Accordion>

  <Accordion title="502는 timeout, 429 또는 524와 어떻게 다른가요?">
    * **`502`**: 백엔드 서비스를 일시적으로 사용할 수 없습니다(컨테이너 재시작). 30\~60초 기다린 후 재시도하십시오. 과금되지 않습니다.
    * **Timeout / 연결 끊김**: 클라이언트 timeout이 너무 짧습니다. 서버는 여전히 요청을 실행 중일 수 있으며 정상적으로 과금될 수 있습니다. [API Timeout 방지 방법](/ko/faq/timeout-configuration)을 참조하십시오.
    * **`429`**: 동시 실행 수 또는 요청 제한에 도달한 경우이며 서비스 가용성과는 관련이 없습니다. [API 동시 실행 수 제한](/ko/faq/api-concurrency)을 참조하십시오.
    * **`524`**: 약 100초를 초과하는 요청으로 CDN 엔드포인트(`api-cf.apiyi.com`)를 사용 중입니다. 엔드포인트를 전환하십시오.

    이 경우에는 대응 방법이 완전히 다릅니다. **직접 재시도해야 하는 것은 502/503뿐입니다**.
  </Accordion>

  <Accordion title="웹 콘솔과 API가 동시에 실패하는 이유는 무엇인가요?">
    웹 콘솔과 API는 동일한 서비스를 기반으로 합니다. 컨테이너 재시작 중에는 둘 다 **함께 사용할 수 없게 되고 함께 복구됩니다**. 따라서 웹사이트도 다운되었다는 현상은 클라이언트 구성 문제가 아니라 일시적인 플랫폼 측 이벤트임을 정확히 확인해 줍니다.
  </Accordion>

  <Accordion title="이 문제가 자주 발생하나요?">
    아니요. 정기적으로 발생하는 현상이 아닙니다. 일시적인 502는 일반적으로 갑작스러운 트래픽 급증과 관련되며 산발적으로 발생합니다.

    **2026년 8월 기준으로 백엔드 서버를 업그레이드하고 확장하고 있으며**, 이를 통해 이러한 일시적인 502의 발생 빈도가 크게 줄어들 것입니다. 플랫폼 측 장애가 발생할 때마다 [실시간 상태 피드](/en/live)에 상태 업데이트와 복구 진행 상황을 즉시 게시합니다.
  </Accordion>

  <Accordion title="플랫폼 문제인지 자체 네트워크 문제인지 어떻게 알 수 있나요?">
    빠르게 확인하는 두 가지 방법이 있습니다.

    1. **웹 콘솔 열기**: `api.apiyi.com`에서 502가 반환되고 콘솔도 로드되지 않으면, 거의 확실히 일시적인 플랫폼 측 재시작입니다. 1분 정도 기다리십시오.
    2. **네트워크 전환**: 모바일 데이터(다른 통신사)를 사용해 콘솔을 로드하거나 아래 명령을 실행해 보십시오. 해당 환경에서 작동한다면 문제는 로컬 네트워크 또는 프록시에 있습니다.
    ...
    다른 네트워크에서도 5분 이상 계속 502가 반환되면 지원팀에 문의하십시오.

    또한 웹 콘솔은 정상적으로 작동하지만 스크립트에서 빈 본문과 함께 간헐적인 502가 발생하고 해당 요청이 호출 로그에 없다면, 502는 로컬 프록시 소프트웨어에서 발생했을 가능성이 높습니다. [스크립트에서 502가 발생하지만 호출 로그에는 아무것도 없나요?](/ko/faq/proxy-empty-502)를 참조하십시오.
  </Accordion>
</AccordionGroup>
