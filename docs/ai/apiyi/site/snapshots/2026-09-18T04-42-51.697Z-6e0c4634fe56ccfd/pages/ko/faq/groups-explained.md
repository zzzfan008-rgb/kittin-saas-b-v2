...
## “사용자 그룹” ≠ “토큰 그룹” — 혼동하지 마십시오

처음에는 흔히 다음과 같이 생각할 수 있습니다. “내 계정에 어딘가 전환해야 하는 그룹이 있습니까?”

* 계정 수준에는 **기본 권한 범위**(SVIP 모델이 표시되는지, 엔터프라이즈 폴백 그룹이 잠금 해제되는지 등)를 결정하는 “사용자 그룹” 개념이 있습니다.
* 그러나 **각 API 호출의 라우팅, 요율 배수 및 모델 사용 가능 여부는 토큰에서 선택한 그룹에 따라 결정됩니다**.

<Tip>
  문제를 해결할 때는 먼저 토큰의 “그룹 선택” 및 “폴백 그룹” 설정을 확인하십시오. “내 계정의 그룹”을 찾으러 다니지 마십시오. [토큰 및 그룹](/ko/faq/token-and-groups)을 참조하십시오.
</Tip>
...
콘솔에 표시되는 “그룹 요율 배수”는 직접적인 USD 할인율이 아니라 **RMB로 책정된 상대적 값**입니다. 따라서 `0.14x`는 **“86% 할인”을 의미하지 않습니다**. 일반적으로 **이 값을 자세히 살펴볼 필요는 없으며**, 사용 중인 모델에 맞는 그룹을 선택하면 됩니다. 요율 배수와 가격 변환을 이해하려면 [모델의 요율 배수란 무엇입니까?](/ko/faq/model-multiplier)를 참조하십시오.

## 관련 문서

<CardGroup cols={2}>
  <Card title="Tokens 및 그룹" icon="key" href="/ko/faq/token-and-groups">
    Token 역할, 생성/편집, 코드 예시 보기, 그룹 개요입니다.
  </Card>

  <Card title="Token 과금 모드" icon="calculator" href="/ko/faq/token-billing-modes">
    사용량별 결제 모드와 호출별 결제 모드의 차이입니다.
  </Card>

  <Card title="모델 요율 배수" icon="percent" href="/ko/faq/model-multiplier">
    요율 배수의 의미, RMB 가격 단위 및 USD 가격 변환입니다.
  </Card>

  <Card title="모델 사용 가능 여부" icon="list" href="/ko/faq/model-availability">
    모델 등급 및 사용자 그룹별 액세스입니다.
  </Card>
</CardGroup>
