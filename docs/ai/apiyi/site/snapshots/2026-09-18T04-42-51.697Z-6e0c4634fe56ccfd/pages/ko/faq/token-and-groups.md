...
<Steps>
  <Step title="Token 페이지 열기">
    상단 내비게이션에서 “Token” 페이지를 엽니다: [https://api.apiyi.com/token](https://api.apiyi.com/token)
  </Step>

  <Step title="‘새로 만들기’ 클릭">
    오른쪽 상단의 “새로 만들기” 버튼을 클릭하여 token 생성 대화 상자를 엽니다.
  </Step>

  <Step title="token 세부 정보 입력">
    token 이름, 쿼터(선택 사항으로 무제한 설정 가능), 만료일(선택 사항으로 만료되지 않도록 설정 가능), 과금 모드 및 그룹(아래 참조)을 설정합니다.
  </Step>

  <Step title="저장하고 KEY 복사">
    저장한 후 token 오른쪽의 복사 아이콘을 클릭하여 전체 `sk-` KEY를 복사합니다.
  </Step>
</Steps>

<img className="block dark:hidden" src="https://mintcdn.com/apiyillc/OMY6ItCc2mC1yzgA/images/key-add-new.png?fit=max&auto=format&n=OMY6ItCc2mC1yzgA&q=85&s=1242d2ab7ec569566ac5667dc41ed32c" alt="새 token 생성" width="1284" height="1158" data-path="images/key-add-new.png" />

<img className="hidden dark:block" src="https://mintcdn.com/apiyillc/OMY6ItCc2mC1yzgA/images/key-add-new.png?fit=max&auto=format&n=OMY6ItCc2mC1yzgA&q=85&s=1242d2ab7ec569566ac5667dc41ed32c" alt="새 token 생성" width="1284" height="1158" data-path="images/key-add-new.png" />

<Tip>
  token을 생성할 때 **허용 모델을 설정할 필요가 없습니다**. 허용 목록 메커니즘이 적용됩니다. 비워 두면 token으로 400개 이상의 모든 모델을 사용할 수 있으며, 설정하면 token 사용이 해당 모델로 제한됩니다. [Token 모델 허용 목록](/ko/faq/token-model-whitelist)을 참조하십시오.
</Tip>
...
다음은 시스템에서 제공하는 그룹 및 설명입니다(콘솔 표시가 기준입니다).

<img className="block dark:hidden" src="https://mintcdn.com/apiyillc/gZdh_-LS6bvRJGUL/images/token-groups-overview-20260527.png?fit=max&auto=format&n=gZdh_-LS6bvRJGUL&q=85&s=5a8d30f9f468b01afda6aeb52ae98912" alt="APIYI 토큰 그룹 개요" width="1200" height="844" data-path="images/token-groups-overview-20260527.png" />

<img className="hidden dark:block" src="https://mintcdn.com/apiyillc/gZdh_-LS6bvRJGUL/images/token-groups-overview-20260527.png?fit=max&auto=format&n=gZdh_-LS6bvRJGUL&q=85&s=5a8d30f9f468b01afda6aeb52ae98912" alt="APIYI 토큰 그룹 개요" width="1200" height="844" data-path="images/token-groups-overview-20260527.png" />

<Note>
  **그룹 요율 배수** 열은 RMB로 책정된 상대적 값이며, **직접적인 USD 할인율이 아닙니다**. 따라서 지나치게 분석할 필요가 없습니다. 사용 중인 모델에 맞는 그룹을 선택하면 됩니다. 요율 배수와 가격 환산을 이해하려면 [모델의 요율 배수란 무엇입니까?](/ko/faq/model-multiplier)를 참조하십시오.
</Note>
