...
[APIYI](https://api.apiyi.com)는 중국 본토에서 직접 연결할 수 있는 GPT Image 2용 게이트웨이로, 하나의 API 키를 공유하는 세 가지 경로를 제공합니다.
...
APIYI는 GPT Image 2에 액세스하는 세 가지 방법을 제공합니다.

| 모델 ID             | 채널            | 과금          | 생성 속도    | 주요 특징                                             |
| ----------------- | ------------- | ----------- | -------- | ------------------------------------------------- |
| `gpt-image-2`     | 공식 릴레이        | token당 과금   | 약 120초   | 공식 OpenAI API와 완전히 호환되며 품질/크기/4K를 지원합니다           |
| `gpt-image-2-all` | 역방향(ChatGPT)  | 이미지당 \$0.03 | 30\~60초  | 중국어 사용에 적합하며, 채팅 엔드포인트를 통해 호출하고 이미지 URL을 직접 반환합니다 |
| `gpt-image-2-vip` | 역방향(Adobe 라인) | 이미지당 \$0.03 | 90\~150초 | 4K를 포함한 30개의 고정 크기 프리셋을 제공합니다                     |

> 이 플러그인은 기본적으로 \*\*`gpt-image-2`(공식 릴레이)\*\*를 사용하며, 공식 OpenAI API와 완전히 호환되고 전체 매개변수 제어를 지원합니다. 더 빠른 생성이 필요한 경우 `gpt-image-2-all` 모드로 전환하십시오(아래 참조).

<Tip>
  [APIYI 콘솔](https://api.apiyi.com/token)에서 `sk-`로 시작하는 API 키를 요청하십시오. 비용을 관리할 수 있도록 일일 쿼터 한도(예: ¥20\~50)를 설정하는 것이 좋습니다.
</Tip>
