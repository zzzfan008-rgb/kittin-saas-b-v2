...
<AccordionGroup>
  <Accordion title="5.0 Pro / 5.0 / 4.5 / 4.0 — 어떤 것을 선택해야 합니까?">
    | 필요 사항                       | 권장 항목                                                                       |
    | --------------------------- | --------------------------------------------------------------------------- |
    | 최신 기능 + 전반적으로 최고            | `seedream-5-0-260128`                                                       |
    | 4K + 뛰어난 텍스트 렌더링(포스터, 광고)   | `seedream-4-5-251128`                                                       |
    | 4K + 최저 가격                  | `seedream-4-0-250828`                                                       |
    | 장시간 실행되는 안정적인 일괄 제작         | `seedream-4-0-250828` (검증됨, 최저 비용, 빠른 prompt 모드)                            |
    | 최고 이미지 품질 / 복잡한 지시사항(전문 작업) | `seedream-5-0-pro-260628` (요청당 \$0.12, 이미지당 약 2분 — 꼭 필요한 경우가 아니면 선택하지 마십시오) |

    전체 비교는 [이전 버전](/ko/api-capabilities/seedream-image/historical-versions)을 참조하십시오.
  </Accordion>

  <Accordion title="이미지 편집에도 generations 엔드포인트를 사용하는 이유는 무엇입니까?">
    Seedream은 생성과 편집을 통합한 아키텍처를 사용하므로 **별도의 `/v1/images/edits` 엔드포인트가 없습니다**. OpenAI의 gpt-image-2가 `/v1/images/edits`로 multipart 업로드를 사용하는 것과 달리, Seedream은 `application/json`을 사용하고 `image` 필드에 이미지 **URL을 배열로** 전달합니다.

    장점은 프로토콜 일관성, 매개변수 재사용, 간편한 모드 전환입니다. 자세한 내용은 [이미지 편집](/ko/api-capabilities/seedream-image/image-edit)을 참조하십시오.
  </Accordion>

  <Accordion title="image 필드에서 base64를 허용합니까?">
    **예**(테스트로 확인되었습니다). 데이터 URI를 사용하십시오: `data:image/<format>;base64,<base64 string>`. 여기서 `<format>`은 소문자로 작성해야 합니다(예: `data:image/jpeg;base64,...`). 동일한 배열에서 URL과 base64 항목을 함께 사용할 수 있습니다. **가능하면 공개 URL을 우선 사용하십시오**. URL은 APIYI 게이트웨이를 거치지 않고 BytePlus가 싱가포르에서 직접 다운로드하지만, base64 본문은 먼저 국경을 넘어 전체를 업로드해야 합니다. 연결이 느린 경우 20MB를 초과하는 본문은 공급자의 600초 요청 본문 타임아웃에 걸릴 수 있으며, 게이트웨이는 base64를 대신 URL로 변환하지 않습니다. 자세한 내용은 이미지 편집 페이지 상단의 경고를 참조하십시오.
  </Accordion>

  <Accordion title="다중 이미지 융합 한도와 일괄 시퀀스 한도는 어떻게 됩니까?">
    * **다중 이미지 융합**(`image` 배열): 4.5 / 5.0-pro는 최대 10개를 명시적으로 지원합니다. 5.0 / 4.0도 다중 이미지를 지원하지만, 문서에 명시적인 상한은 없습니다.
    * **일괄 시퀀스**(`max_images`): 전역 규칙인 **입력 참조 수 + 출력 수 ≤ 15**에 따라 제한됩니다. 융합과 시퀀스를 함께 사용하는 경우 총합을 계산하십시오. **5.0-pro는 일괄 시퀀스를 지원하지 않습니다**(`sequential_image_generation`를 전달하면 400을 반환합니다).
  </Accordion>

  <Accordion title="b64_json에 data:image 접두사가 필요합니까?">
    `response_format`에 따라 다릅니다.

    * `response_format: "url"`(기본값) → `data[0].url`은 임시 서명 URL이므로 `<img src=...>`로 직접 렌더링합니다.
    * `response_format: "b64_json"` → `data[0].b64_json`은 **일반 base64 문자열**입니다(`data:image/...;base64,` 접두사 없음). 디코딩하여 디스크에 저장하거나, 브라우저에서 렌더링할 수 있도록 접두사를 수동으로 추가하십시오.
  </Accordion>

  <Accordion title="스트리밍 출력을 지원합니까?">
    `stream: true`를 통해 5.0 / 4.5 / 4.0에서 지원됩니다. 스트리밍은 긴 prompt와 고해상도 이미지에 특히 유용하며, 프런트엔드에서 부분 결과를 점진적으로 렌더링할 수 있습니다. **`seedream-5-0-pro`는 스트리밍을 지원하지 않습니다** — `stream`를 전달하면 400을 반환합니다.
  </Accordion>

  <Accordion title="요청 제한은 어떻게 됩니까?">
    **기본값은 분당 이미지 500개**(최대 분당 이미지 수)이며, 모든 버전에 통합 적용됩니다. 더 높은 쿼터가 필요한 경우 영업팀에 문의하십시오.
  </Accordion>

  <Accordion title="실패한 요청에도 과금됩니까?">
    **아닙니다**. BytePlus에는 기본 제공 조정 기능이 있습니다. 조정 거부 및 매개변수 오류는 `400` / `403`을 반환하며 **과금되지 않습니다**. 그 밖에 과금되지 않는 오류로는 `401`(유효하지 않은 token), `429`(요청 제한 초과)가 있습니다. **유효한 응답이 포함된 성공적인 생성(`200`)만 과금됩니다**.
  </Accordion>

  <Accordion title="공식 OpenAI SDK를 사용할 수 있습니까?">
    예, 코드를 변경하지 않고 사용할 수 있습니다. `base_url`을 `https://api.apiyi.com/v1`로 지정하고, 확장 매개변수(`image` / `sequential_image_generation` / `watermark` 등)를 `extra_body`을 통해 전달하십시오.
  ...
  <Accordion title="생성된 이미지의 소유자는 누구입니까?">
    생성된 이미지는 상업적 및 비상업적 용도로 사용할 수 있습니다. 자세한 내용은 BytePlus 서비스 약관을 참조하십시오.
  </Accordion>

  <Accordion title="투명한 배경을 지원합니까?">
    `seedream-5-0` / `seedream-5-0-pro`는 `png` 출력을 지원하며, prompt에 “transparent background, alpha channel”을 지정하면 투명한 배경을 생성할 수 있습니다. `seedream-4-5` / `4-0`는 `jpeg`만 출력하며 **투명도를 지원하지 않습니다** — 배경을 직접 제거하는 후처리를 수행하십시오.
  </Accordion>

  <Accordion title="실행 중인 생성을 취소할 수 있습니까?">
    **아닙니다**. `/v1/images/generations`은 동기식입니다. 제출된 요청은 완료될 때까지 실행됩니다. 클라이언트의 연결이 끊어져도 서버는 계속 완료하고 과금합니다. 클라이언트 타임아웃을 설정하고 연결을 끊으면 비용이 절약된다고 가정하지 마십시오.
  </Accordion>
</AccordionGroup>
