...
네 모델은 모두 **정확히 동일한 방식으로 호출**하며, 해상도 제한, 길이 제한, 참조 제한, 속도 및 가격에서 차이가 있습니다. 스크립트는 `--model`에 따라 이러한 차이를 처리합니다:

| 모델 (`--model`) | 모델 ID                             | 최대 해상도    | 속도 (720p/5초, 측정값)    | 그룹                                        | 720p/5초 정가                       | 적합한 용도                                            |
| -------------- | --------------------------------- | --------- | -------------------- | ----------------------------------------- | -------------------------------- | ------------------------------------------------- |
| `mini` (기본값)   | `doubao-seedance-2-0-mini-260615` | 720p      | **가장 빠름, 약 87–170초** | `SeeDance2` 0.18x<br />또는 `SD2Mini` 0.10x | \$0.4508<br />할인 적용 **\$0.2504** | 고빈도 에이전트 사용, 대량 사용, 빠른 미리 보기                      |
| `fast`         | `doubao-seedance-2-0-fast-260128` | 720p      | 약 100–290초           | `SeeDance2` 0.18x<br />또는 `SD2Fast` 0.15x | \$0.7253<br />할인 적용 **\$0.6044** | 품질/비용 균형                                          |
| `std`          | `doubao-seedance-2-0-260128`      | **1080p** | 약 100–290초           | `SeeDance2` 0.18x                         | \$0.9074                         | 1080p, 최고 품질                                      |
| `sd25`         | `doubao-seedance-2-5-260628`      | **1080p** | 약 150초               | `SeeDance2` 0.18x                         | **\$1.3721**                     | **최대 30초, 참조 이미지 30개, mov 출력** — `std` 가격의 약 1.5배 |

<Tip>
  경험칙: **일상적인 사용 및 에이전트 시나리오에서는 기본 `mini`을 유지**하고, **1080p 또는 최고 품질이 필요하면** → `--model std`, **30초 클립, 참조 이미지 30개 또는 mov 출력이 필요하면** → `--model sd25`이 유일한 옵션입니다(`std` 가격의 약 1.5배이며, 2.0 제품군과 동일한 그룹). 모든 종횡비는 동일한 해상도 티어 내에서 비용이 같고, 길이는 초당 선형 과금되며, 프레임 레이트는 24fps로 고정됩니다. 할인 그룹 `SD2Mini` / `SD2Fast`은 \*\*2026년 10월 7일 23:59 (UTC+8)\*\*까지 운영되며, 이후에도 그룹은 온라인 상태로 유지되고 요율 배수는 0.18x로 돌아갑니다 — 자세한 내용은 [개요 페이지의 그룹 안내](/ko/api-capabilities/seedance2/overview)를 참조하십시오.
</Tip>
