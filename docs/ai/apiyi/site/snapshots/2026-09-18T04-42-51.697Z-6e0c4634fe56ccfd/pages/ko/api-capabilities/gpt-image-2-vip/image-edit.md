...
| 필드       | 유형  | 필수  | 설명                                                                                                                                                             |
| -------- | --- | --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model`  | 텍스트 | 예   | `gpt-image-2.5-vip` (= `gpt-image-2.5-sunburst-vip`) / `gpt-image-2.5-flare-vip` / 이전 세대 `gpt-image-2-vip`, 동일한 가격 및 호출 형식입니다. 세 가지 모두에서 `mask`은 전체 이미지 재생성입니다 |
| `prompt` | 텍스트 | 예   | 자연어 편집/융합 설명                                                                                                                                                   |
| `image`  | 파일  | 예   | 참조 이미지입니다. 반복해서 지정할 수 있습니다(배열 필드)                                                                                                                              |
| `size`   | 텍스트 | 아니요 | 출력 크기: `auto`(기본값 — **프롬프트에서 편집 대상으로 지정한 참조 이미지의 크기를 따르며, 반드시 첫 번째 이미지인 것은 아닙니다**) 또는 30개 크기 중 하나입니다. 형식은 `WIDTHxHEIGHT`(소문자 `x`)입니다                           |

<Tip>
  **다중 턴 반복**: 이전 출력을 다음 호출의 `image`로 다시 입력하고, 새로운 편집 지침을 지정하여 점진적으로 개선합니다. 각 라운드에서 자체 `size`를 지정할 수 있습니다.
</Tip>
