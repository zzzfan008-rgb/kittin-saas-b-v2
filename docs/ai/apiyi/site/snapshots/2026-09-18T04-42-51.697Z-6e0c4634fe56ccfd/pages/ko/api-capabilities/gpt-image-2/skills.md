...
# GPT-Image-2.5 / 2 시리즈 에이전트 스킬

> gpt-image-2.5-flare / gpt-image-2.5-sunburst / gpt-image-2(공식)와 gpt-image-2.5-all / gpt-image-2-all / gpt-image-2-vip(리버스)를 바로 사용할 수 있는 하나의 에이전트 스킬로 패키징합니다. Codex, OpenClaw, hermes-agent, Claude Code 또는 모든 코딩 에이전트에 추가하고 --model로 채널을 전환하여 텍스트-이미지 변환, 다중 이미지 융합 및 인페인팅을 수행할 수 있습니다.

<Note>
  이 페이지에서는 **바로 사용할 수 있는 에이전트 스킬**을 제공합니다. 하나의 스크립트로 6개 모델(**gpt-image-2.5-flare / gpt-image-2.5-sunburst / gpt-image-2 (공식)** 및 **gpt-image-2.5-all / gpt-image-2-all / gpt-image-2-vip**(리버스))을 지원합니다. 모두 동일한 OpenAI 이미지 API를 사용하며, `--model`만 다릅니다. 이미 사용 중인 코딩 에이전트에 추가하면 단일 prompt로 이미지를 생성할 수 있으며, 파일은 2개뿐입니다.
</Note>
...
6개 모델은 모두 **동일하게 호출**되며, 소스 채널, 가격/속도 및 적용되는 파라미터만 다릅니다. 공식 모델 3개(2.5-flare / 2.5-sunburst / 2)는 가격과 파라미터를 공유하며, 리버스 모델 3개도 마찬가지입니다. 스크립트는 이러한 차이를 `--model`로 자동 처리합니다:

| 모델 (`--model`)                                           | 채널                  | 가격                        | 속도                                   | `size`             | `quality` / `mask`                                                   | 적합한 용도                                   |
| -------------------------------------------------------- | ------------------- | ------------------------- | ------------------------------------ | ------------------ | -------------------------------------------------------------------- | ---------------------------------------- |
| `gpt-image-2.5-flare` (기본값)                              | 공식 패스스루             | token 기반 약 \$0.03–0.2/img | **가장 빠른 공식 모델** (1K low 기준 측정 약 10초) | ✅ 모든 프리셋           | ✅ 예, `xhigh` / `max` 포함                                              | 일상적인 텍스트-이미지 기본값, 품질 단계, 투명 배경           |
| `gpt-image-2.5-sunburst`                                 | 공식 패스스루             | token 기반, flare와 동일       | flare보다 느림                           | ✅ 모든 프리셋           | ✅ 예, `xhigh` / `max` 포함                                              | 편집 / 다중 이미지 융합 / 마스크 인페인팅, 최고 수준의 편집 정밀도 |
| `gpt-image-2`                                            | 공식 패스스루             | token 기반, flare와 동일       | 약 100–120초                           | ✅ 모든 프리셋           | ✅ 예(최대 `high`)                                                       | 이전 세대, 기존 통합                             |
| `gpt-image-2.5-all`                                      | 리버스 (ChatGPT 웹 2.5) | 정액 \$0.03/img             | 약 30–90초                             | ❌ prompt에 작성       | ❌                                                                    | 대량 처리, 속도, 2.5 품질, prompt 텍스트를 통한 크기 지정  |
| `gpt-image-2-all`                                        | 리버스 (ChatGPT 라인)    | 정액 \$0.03/img             | **가장 빠름 약 30–60초**                   | ❌ prompt에 작성       | ❌                                                                    | 대량 처리, 속도, prompt 텍스트를 통한 크기 지정          |
| `gpt-image-2-vip`                                        | 리버스 (Adobe 라인)      | 정액 \$0.03/img             | 약 90–150초                            | ✅ **4K**를 포함한 30단계 | ❌                                                                    | 고정 출력 크기 / 저렴한 가격의 4K                    |
| `gpt-image-2.5-flare-vip` / `gpt-image-2.5-sunburst-vip` | 리버스 (Adobe 라인, 2.5) | 정액 \$0.03/img             | 약 20–140초                            | ✅ **4K**를 포함한 30단계 | `quality` 6개 단계 모두(`xhigh` / `max` 포함), 투명 배경 ✅; `mask`는 전체 이미지 전용 ❌ | 정액 가격의 2.5 품질, 고정 가능한 크기                 |

<Tip>
  빠른 규칙: **일상적인 텍스트-이미지** → `gpt-image-2.5-flare`; **편집 / 마스크 인페인팅** → `gpt-image-2.5-sunburst`; **빠르고 저렴한 옵션** → `gpt-image-2.5-all` / `gpt-image-2-all`; **고정 크기/4K 및 저렴한 옵션** → `gpt-image-2-vip`.
</Tip>
...
`gpt-image-2/SKILL.md`에 아래의 전체 내용을 작성합니다(`description`은 “무엇을 하며 언제 사용하는지”를 명시하며, Agent가 이를 사용하여 자동으로 트리거합니다).
...
```bash
# 텍스트를 이미지로 변환(기본값: gpt-image-2.5-flare 공식 모델, 품질 지정)
python3 ${CLAUDE_SKILL_DIR}/scripts/gpt_image.py "해변 바에서 선글라스를 쓴 주황색 고양이, 영화 같은 분위기" -o cat.png --size 1536x1024 --quality high

# 가장 빠르고 저렴한 방법: 'all' 역방향 사용(프롬프트에 크기를 입력하고 size는 전달하지 않음)
python3 ${CLAUDE_SKILL_DIR}/scripts/gpt_image.py "평면 일러스트 축제 포스터, 세로 2:3" -o poster.png --model gpt-image-2-all

# 고정된 4K가 필요한 경우: 'vip' 역방향 사용
python3 ${CLAUDE_SKILL_DIR}/scripts/gpt_image.py "도시의 야경을 내려다본 항공 뷰" -o city.png --model gpt-image-2-vip --size 3840x2160

# 여러 이미지 융합(최대 16개, -i 반복)
python3 ${CLAUDE_SKILL_DIR}/scripts/gpt_image.py "이미지 1의 인물을 이미지 2의 장면에 배치하고 이미지 3의 색상을 유지" -i person.png -i scene.png -i style.png -o fused.png

# 인페인팅(마스크, 공식 모델만 지원; gpt-image-2.5-sunburst 권장)
python3 ${CLAUDE_SKILL_DIR}/scripts/gpt_image.py "마스크 영역을 둥근 창문으로 교체" -i room.png --mask mask.png -o edited.png

# 여러 개를 한 번에 생성(최대 5개, 동시 실행)
...
<Tip>
  `name`은 소문자와 하이픈으로만 구성되어야 합니다. 슬래시 명령을 지원하는 Agent에서는 디렉터리 이름이 명령이 되며, `gpt-image-2`은 `/gpt-image-2`이 됩니다. `${CLAUDE_SKILL_DIR}`은 Claude Code에서 제공하는 스킬 디렉터리 변수입니다. 다른 Agent에서는 스크립트의 실제 경로를 사용하면 됩니다.
</Tip>

## scripts/gpt\_image.py

APIYI(`base_url="https://api.apiyi.com/v1"`)를 가리키도록 설정한 OpenAI SDK를 사용하여 `gpt-image-2/scripts/gpt_image.py`를 생성합니다:
...
기본 채널을 변경하려면(매번 `--model`을 전달하지 않도록), `gpt-image-2/.env`에 다음 줄을 추가합니다:
...
<Info>
  이 스크립트는 base64 및 URL 응답을 **모두** 자동으로 처리합니다(역방향 모델의 `b64_json`에는 `data:image;base64,` 접두사가 포함되며, 스크립트가 이를 제거합니다). **URL 출력을 반드시 요구하는 경우에만** APIYI 콘솔에서 token의 과금 그룹을 `image2_OSS`로 전환하면 됩니다. 일반 생성에는 필요하지 않습니다.
</Info>
...
| 입력 내용                             | 스킬 동작                                                   |
| --------------------------------- | ------------------------------------------------------- |
| "영화 같은 고양이를 그려 줘"                 | 기본 gpt-image-2.5-flare, png 1개                          |
| "빠르고 저렴하게 포스터를 만들어 줘"             | 에이전트가 `--model gpt-image-2-all`을 추가합니다                  |
| "4K 도시 야경을 렌더링해 줘"                | 에이전트가 `--model gpt-image-2-vip --size 3840x2160`을 추가합니다 |
| "person.png의 인물을 scene.png에 넣어 줘" | `-i person.png -i scene.png`를 실행하여 이미지를 융합합니다           |
