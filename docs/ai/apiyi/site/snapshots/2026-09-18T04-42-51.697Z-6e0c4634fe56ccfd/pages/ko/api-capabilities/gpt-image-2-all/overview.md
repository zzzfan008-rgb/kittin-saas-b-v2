...
# GPT-Image-2.5-All 이미지 생성/편집

> GPT 이미지 생성 리버스 엔지니어링 모델 gpt-image-2-all(ChatGPT 웹 라인). 호출당 이미지 1장당 고정 $0.03 요금, 생성 시간 약 30~60초. 텍스트-이미지 생성, 다중 이미지 융합 편집, 자연어 편집, 높은 텍스트 렌더링 충실도, 중국어 prompt 친화성을 지원합니다.
...
**gpt-image-2.5-all** 및 **gpt-image-2-all**은 APIYI의 **GPT 이미지 생성 리버스 엔지니어링 모델**(ChatGPT 웹 라인)입니다. ChatGPT 웹 앱 전체가 Images 2.5로 전환되었으므로 `gpt-image-2-all`은 이미 2.5 이미지를 생성하며, 새 `gpt-image-2.5-all` 이름은 해당 업그레이드를 명확히 나타낼 뿐입니다 — **두 이름은 동일한 가격과 동작을 공유하며 서로 바꿔 사용할 수 있습니다**. 새 프로젝트에서는 `gpt-image-2.5-all`를 사용해야 합니다. 호출당 **\$0.03/이미지**라는 매우 경쟁력 있는 가격으로 **약 30\~60초** 만에 이미지를 생성하며, **텍스트-이미지 변환 / 단일 이미지 편집 / 다중 이미지 융합 / 자연어 편집**을 지원합니다. 텍스트 렌더링 충실도가 높고 중국어 prompt를 기본 지원합니다.

<Note>
  **🎨 주요 특징**: 이미지당 \$0.03의 정액 요금을 제공하는 안정적인 리버스 엔지니어링 채널입니다. 크기/품질/n 매개변수에 대해 걱정할 필요 없이 prompt에서 크기와 스타일을 설명하면 됩니다. OpenAI Images API 표준 엔드포인트인 `/v1/images/generations`(텍스트-이미지 변환) 및 `/v1/images/edits`(이미지 편집)를 사용합니다.

  **출력 크기 또는 4K를 고정해야 합니까?** 자매 모델인 [`gpt-image-2-vip`](/ko/api-capabilities/gpt-image-2-vip/overview)로 전환하십시오 — 호출 형식은 동일하며 `size` 필드 하나만 추가하면 됩니다.
</Note>

<CardGroup cols={2}>
  <Card title="텍스트-이미지 변환 API" icon="wand-sparkles" href="/ko/api-capabilities/gpt-image-2-all/text-to-image">
    `/v1/images/generations` — 텍스트 prompt에서 이미지를 생성합니다.
  </Card>

  <Card title="이미지 편집 API" icon="image" href="/ko/api-capabilities/gpt-image-2-all/image-edit">
    `/v1/images/edits` — 편집/융합 지침과 함께 multipart 업로드를 수행합니다.
  </Card>
</CardGroup>
...
| 모델                  | 과금  | 가격                          | 출력                           |
| ------------------- | --- | --------------------------- | ---------------------------- |
| `gpt-image-2-all`   | 호출당 | **\$0.03 / 이미지**            | 호출당 이미지 1개                   |
| `gpt-image-2.5-all` | 호출당 | **\$0.03 / 이미지** (-all과 동일) | 호출당 이미지 1개, GPT-Image 2.5 기반 |

<Info>
  **과금 참고사항**:

  * 고정 가격이며 해상도, 품질 또는 prompt 길이에 따른 등급이 없습니다.
  * 실패한 요청에는 과금되지 않습니다(인증 실패, 매개변수 검증 오류).
  * 이미지 N개가 필요한 경우 API를 병렬로 N회 호출합니다.
</Info>

<Tip>
  **동일 가격의 자매 모델**: [`gpt-image-2-vip`](/ko/api-capabilities/gpt-image-2-vip/overview) (Adobe 역방향 라인(Firefly)) — 이미지당 동일한 \$0.03이며, 명시적 크기 30개(4K 포함)를 지원하고 호출 형식도 동일합니다. 출력 크기를 고정해야 하는 경우 전환하여 사용합니다.
</Tip>

## 그룹 설정

`gpt-image-2-all`은 `Default` 그룹에 있습니다 — **추가 그룹이 필요하지 않습니다**. 현재 리버스 채널은 안정적인 공급을 유지하고 있으므로, 공식 릴레이 `gpt-image-2`과 같은 엔터프라이즈 그룹 대체 경로는 없습니다.

| 모델                  | 그룹           | 비고                                                                  |
| ------------------- | ------------ | ------------------------------------------------------------------- |
| `gpt-image-2-all`   | `Default`    | ChatGPT-web 리버스, 이미지당 고정 \$0.03, 약 30–60초                           |
| `gpt-image-2.5-all` | `Default`    | 동일한 제품군의 GPT-Image 2.5 버전, 이미지당 고정 \$0.03, -all과 동일한 그룹             |
| `gpt-image-2-all`   | `image2_OSS` | **1x 요율 배수(추가 요금 없음)**, 결정적 URL 출력 — 기본 그룹에 부하가 걸려도 base64로 대체되지 않음 |

### 결정적인 URL 출력이 필요함 → `image2_OSS` 그룹으로 전환

2026년 7월 기본 그룹에서 측정한 결과, `gpt-image-2-all`(및 `gpt-image-2-vip`)은 `response_format`이 생략되면 `b64_json`을 반환합니다. 이미지 URL을 받으려면 `response_format: "url"`을 명시적으로 전달해야 합니다. 기본 그룹의 출력 형식은 **보장되지 않습니다** — 역사적으로 `url`이 기본값이었으며, 부하가 걸리면 `b64_json`로 대체되었고 채널 버전에 따라 변경된 이력이 있습니다.

비즈니스가 **URL 출력에 의존하는 경우**(URL을 데이터베이스에 바로 기록하거나, 프런트엔드에서 URL로 렌더링하거나, base64를 허용하지 않는 경우) 토큰의 그룹을 \*\*`image2_OSS`\*\*으로 전환하십시오 — 이 그룹은 **결정적인 URL 출력**을 위해 특별히 구성되었으며, \*\*1x 요율 배수(추가 요금 없음)\*\*가 적용되고 리버스 모델 `gpt-image-2-all` 및 `gpt-image-2-vip` 모두에서 사용할 수 있습니다. 응답에 항상 이미지 URL이 포함되며 base64로 대체되지 않음을 보장합니다.
...
<Frame caption="Token creation: set billing mode to &#x22;pay-as-you-go first&#x22; and pick the image2_OSS group (1x) — use it when you need deterministic URL output">
  <img src="https://mintcdn.com/apiyillc/eNGQJU-a_dFb12gU/images/image2-oss-token-setup-20260525.png?fit=max&auto=format&n=eNGQJU-a_dFb12gU&q=85&s=727f61464cc759006a59e8de6ceccd32" alt="토큰 생성 화면: 먼저 종량제 과금 모드를 선택하고, 그룹은 image2_OSS(1x 요율 배수)로 설정합니다. 이미지 URL을 출력하는 그룹으로, gpt-image-2-all 및 gpt-image-2-vip에 적합합니다." width="1278" height="846" data-path="images/image2-oss-token-setup-20260525.png" />
</Frame>

<Tip>
  **고급 설정(`gpt-image-2-vip` 및 공식 릴레이 `gpt-image-2`도 함께 사용하는 경우)**: 토큰에서 세 모델을 모두 사용할 수 있다면 토큰의 그룹 우선순위를 다음과 같이 설정하십시오.

  * **첫 번째 우선순위**: `image2Enterprise` (1.2x 엔터프라이즈 그룹, 공식 릴레이 전용 안정적 처리 경로)
  * **기본 대체 그룹**: `Default` (두 리버스 모델이 모두 이곳에 있으며 모델별로 라우팅됨)

  결과적으로 공식 릴레이 `gpt-image-2`은 안정성을 위해 엔터프라이즈 처리 경로를 사용하고, 두 리버스 모델은 기본 그룹에 유지됩니다 — 하나의 토큰으로 세 모델을 모두 처리하며 서로 간섭하지 않습니다.
</Tip>
...
| **채널 유형**      | 공식 리버스 엔지니어링(ChatGPT 웹)                                                                    |
| **가격**         | \$0.03 / 이미지, 호출당                                                                          |
| **생성 시간**      | 약 30–60초                                                                                   |
| **출력 해상도**     | 명시적인 크기 매개변수 없음; 적응형(prompt에 설명)                                                           |
| **기본 응답 형식**   | `b64_json` (raw base64, **`data:` 접두사 없음**, 2026-07 검증 완료; 항상 `response_format`을 명시적으로 전달) |
| **선택적 형식**     | `url` (R2 CDN 가속 링크, **유효 기간 약 1일**, `response_format: "url"`을 명시적으로 지정해야 함)               |
| **중국어 prompt** | ✅ 기본 지원                                                                                    |
| **기능**         | 텍스트-이미지 변환, 단일 이미지 편집, 다중 이미지 융합, 자연어 편집                                                   |

<Warning>
  이 모델은 **적응형 출력 크기**를 사용하며 공식 `gpt-image-2` API와 동일하지 않습니다. 출력 크기를 엄격하게 고정하거나 4K를 사용하려면 [`gpt-image-2-vip`](/ko/api-capabilities/gpt-image-2-vip/overview)(Adobe 리버스 라인(Firefly), 4K를 포함한 30개의 명시적 크기)을 사용하십시오. 공식 API와 완전히 동일한 호환성이 필요하면 [`gpt-image-2`](/ko/api-capabilities/gpt-image-2/overview)를 사용하십시오.
</Warning>

<Warning>
  **⏰ 이미지 URL 유효 기간: 약 1일(기본값)**

  `url`-모드 응답의 `url` 필드는 R2 CDN 링크이며 **약 24시간 후 만료됩니다**. 그 이후의 요청은 404를 반환합니다. 장기 보관이 필요한 이미지(제품 사진, 사용자 작품, 기록 데이터 등)는 생성 후 **가능한 한 빨리 다운로드하여 자체 스토리지에 저장하십시오**.

  일반적인 방법은 다음 두 가지입니다.

  * **서버 측 다운로드**: 응답을 받은 즉시 `requests` / `fetch`을 사용하여 이미지를 가져온 후 S3 / OSS / R2 / 로컬 디스크에 저장합니다
  * **`b64_json` 응답 형식 사용**: 이미지를 base64 데이터로 직접 받아 추가적인 원본 간 다운로드를 생략할 수 있으며, 프런트엔드 렌더링이나 파일에 바로 기록하는 데 적합합니다
</Warning>
