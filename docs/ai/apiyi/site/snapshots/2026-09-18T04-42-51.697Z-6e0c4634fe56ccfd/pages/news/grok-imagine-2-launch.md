...
<Warning>
  **2026-09-16 (UTC+8) 更新：本系列已改为定向开放。** Grok Imagine 2 现独立放在 **`Grok_imagine` 专属分组**，**不在 `Default` 默认分组**，需申请开通后才能调用——本文发布时（2026-08-12）的「默认分组即可调用」口径已失效。

  原因是该系列的内容安全策略与平台其它模型差异较大、部分类别不作过滤，为避免合规风险我们收窄了开放范围：累计消费满 \$1,000 的存量客户联系客服说明用途即可开通；其他客户请通过[企业微信客服](https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec)提交申请，说明使用场景与内容管控措施。申请流程见 [Grok Imagine 2 概览 · 分组介绍](/api-capabilities/grok-imagine-image/overview#分组介绍)。本文其余的参数、计费与实测结论不受影响。
</Warning>

## 核心要点

* **两个模型上线**：`grok-imagine-image`（**\$0.02/张**）与 `grok-imagine-image-quality`（**\$0.045/张**），需开通 `Grok_imagine` 专属分组（**2026-09-16 起定向开放**，见文首提示）
...
**分组**：`Grok_imagine` 专属分组（1.0x 倍率），**默认不对外开放，需申请开通**（2026-09-16 起，见文首提示）。开通后把令牌分组切到 `Grok_imagine`，「计费模式」选 `按量优先`（Pay-as-you-go Priority）即可。
