...
> Qwen3.7-Flash 模型詳情：輸入 $0.16 / 輸出 $0.64 每 1M tokens，1,000,000 上下文、65,536 最大輸出，2 個分組可用。

上一代低價多模態推理款，1M 上下文、65K 最大輸出、256K 思考預算，主打多模態 Agent 與視覺編碼。
...
以下示例通過 OpenAI Chat Completions（`/v1/chat/completions`）呼叫 `qwen3.7-flash`。base\_url 換成 `https://api.apiyi.com/v1` 即可，其餘用法與官方一致。
...
<CardGroup cols={2}>
  <Card title="Qwen3.8-Flash" icon="git-compare" href="/zh-Hant/models/qwen3-8-flash">
    同系列模型詳情
  </Card>
