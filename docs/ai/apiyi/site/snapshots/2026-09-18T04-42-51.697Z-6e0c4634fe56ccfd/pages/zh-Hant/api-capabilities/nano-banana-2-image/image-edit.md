...
> Nano Banana 2 圖片編輯 API 參考與線上除錯 — 輸入圖片 + 指令生成新圖片
...
<Tip>
  **場景說明**：本頁用於「圖片編輯」，必須上傳一張待編輯的圖片（base64 編碼）+ 編輯指令。如果只想根據文本生成新圖片，請使用 [文生圖介面](/zh-Hant/api-capabilities/nano-banana-2-image/text-to-image)。
</Tip>
...
<Warning>
  ...
  * 如要在瀏覽器裡試 Playground，**用極小的參考圖（\< 50KB）** 並把 `imageSize` 設為最小檔（如 `512` / `1K`）。
</Warning>

<Warning>
  **⚠️ `parts` 陣列結構（重要，多圖編輯必看）**

  每個 `part` **只能是 `text` 或 `inlineData` 中的一個**，二者不能同時出現在同一個 part 裡。這與谷歌官方 `gemini-3.1-flash-image-preview` 的契約一致。
...
<Warning>
  ...
  **建議**：測試用小圖（\< 200KB），避免 base64 字串過長導致瀏覽器卡頓。頻繁測試圖片編輯更推薦用下方程式碼示例直接在本地執行。
</Warning>
...
| `generationConfig.imageConfig.imageSize`          | string  | 否  | `512` / `1K` / `2K` / `4K`，預設 `1K`                                                       |
| `generationConfig.thinkingConfig.thinkingLevel`   | string  | 否  | `minimal`（快速）/ `High`（深度推理）                                                              |
| `generationConfig.thinkingConfig.includeThoughts` | boolean | 否  | 是否返回思維過程文本                                                                               |

## 多輪對話式編輯

Nano Banana 2（`gemini-3.1-flash-image-preview`）支援**真正的對話式多輪編輯**：把模型每一輪產出的圖片，作為 **`role: "model"` 的 `inlineData`** 追加回 `contents`，再發下一條 user 指令。模型會基於**完整對話歷史**繼續修改並**累積效果**（例如先改沙發顏色、再加配飾，前一步的改動會保留）。

<Info>
  這與"逆向"影像模型不同：原生 Gemini 格式會真正讀取對話歷史裡 `model` 角色的圖片。要保持跨輪一致性、逐步精調，推薦用下面這種**對話歷史回填**的寫法。
</Info>
...
<Note>
  ...
  * **對話歷史回填（上面，推薦）**：`contents` 保留 user/model 交替歷史，跨輪累積修改、畫面更穩。
  * **重新喂圖（簡化版）**：每輪只發單條 user 訊息（`text` + 上一張圖的 `inlineData`），做單步編輯也可以，但不攜帶更早的上下文。
</Note>


## OpenAPI

````yaml api-reference/nano-banana-2-edit-openapi.yaml POST /v1beta/models/gemini-3.1-flash-image-preview:generateContent
openapi: 3.1.0
info:
  title: Nano Banana 2 图片编辑 API
  description: |
    Google 最新图像生成模型 Nano Banana 2（gemini-3.1-flash-image-preview）— 图片编辑接口。
...
paths:
  /v1beta/models/gemini-3.1-flash-image-preview:generateContent:
    post:
      ...
      description: >
        使用 Nano Banana 2 模型，根据文本指令对输入图片进行编辑。支持多轮对话式编辑。
        ...
        - 如需「文生图」请使用
        [文生图接口](/api-capabilities/nano-banana-2-image/text-to-image)
      operationId: editNanoBanana2Image
      requestBody:
        required: true
        content:
          application/json:
...
components:
  schemas:
    ...
    ThinkingConfig:
      type: object
      description: 思维模式配置（Nano Banana 2 独有）。启用后模型会先推理分析再生成图片，适合复杂提示词
      properties:
        thinkingLevel:
          type: string
          description: 思维深度。minimal = 快速生成；High = 深度推理，更精准但稍慢
          enum:
            - minimal
            - High
          default: minimal
        includeThoughts:
          type: boolean
          description: 是否在响应中返回思维过程文本。注意：无论是否开启，思维 token 都会计费
          default: false
    TextPart:
      type: object
