...
> GLM-5.3-Flash 模型詳情：輸入 $0.15 / 輸出 $0.5 每 1M tokens，1,000,000 上下文、128,000 最大輸出，2 個分組可用。

GLM-5 系列首個原生多模態，320B 總引數、18B 啟用，Terminal-Bench 2.1 84.3 逼近 Claude Opus 4.8，成本約為旗艦的十分之一；MIT 開源。
...
以下示例通過 OpenAI Chat Completions（`/v1/chat/completions`）呼叫 `glm-5.3-flash`。base\_url 換成 `https://api.apiyi.com/v1` 即可，其餘用法與官方一致。
...
<CardGroup cols={2}>
  <Card title="上線公告" icon="megaphone" href="/news/glm-5-3-launch">
    GLM-5.3-Flash 的釋出背景、實測表現與遷移建議
  </Card>

  <Card title="GLM-5.3" icon="git-compare" href="/zh-Hant/models/glm-5-3">
    同系列模型詳情
  </Card>
