...
<Tip>
  簡單記：**日常與 Agent 場景就用預設 `mini`**；**要 1080p 或最高品質** → `--model std`；**要 30 秒長片、30 張參考圖或 mov 輸出** → 只能 `--model sd25`（約為 `std` 的 1.5 倍價，分組與 2.0 系相同）。同檔位全比例同價，時長按秒線性計費，幀率固定 24fps。特價分組 `SD2Mini` / `SD2Fast` 截至 **2026 年 10 月 7 日 23:59 (UTC+8)**，到期分組不下線、倍率恢復 0.18x，詳見 [概覽頁的分組說明](/zh-Hant/api-capabilities/seedance2/overview)。
</Tip>
