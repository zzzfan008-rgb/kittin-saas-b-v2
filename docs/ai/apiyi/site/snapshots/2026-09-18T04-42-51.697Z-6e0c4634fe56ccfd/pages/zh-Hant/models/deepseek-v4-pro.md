...
> DeepSeek V4 Pro 模型詳情：輸入 $1.32 / 輸出 $3.96 每 1M tokens，1,048,576 上下文，3 個分組可用。

1.6T 總參 / 49B 啟用的 MoE 旗艦，Agentic Coding 開源 SOTA，SWE-Verified 80.6，思考強度可調到 max。
...
以下示例通過 OpenAI Chat Completions（`/v1/chat/completions`）呼叫 `deepseek-v4-pro`。base\_url 換成 `https://api.apiyi.com/v1` 即可，其餘用法與官方一致。
...
<CardGroup cols={2}>
  <Card title="上線公告" icon="megaphone" href="/news/deepseek-v4-launch">
    DeepSeek V4 Pro 的釋出背景、實測表現與遷移建議
  </Card>

  <Card title="DeepSeek V4 Flash" icon="git-compare" href="/zh-Hant/models/deepseek-v4-flash">
    同系列模型詳情
  </Card>
