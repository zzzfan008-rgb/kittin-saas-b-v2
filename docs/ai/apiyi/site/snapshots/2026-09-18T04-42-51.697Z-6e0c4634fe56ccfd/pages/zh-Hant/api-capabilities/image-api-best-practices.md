...
| -------------------------------------------------------------------------------------------- | ------------------------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------- |
| [GPT-Image-2.5 / 2（官轉，flare / sunburst / 2）](/zh-Hant/api-capabilities/gpt-image-2/overview) | `/v1/images/generations`、`/v1/images/edits` | **360 秒**（gpt-image-2 high + 2K/4K 實測 3-5 分鐘；2.5 更快但 `xhigh` / `max` 同樣建議留足） | 純 b64\_json（**無 `data:` 字首**）                                              | ❌ 不支援（傳 `response_format` 直接 400；`image2_OSS` 分組暫不支援官轉） |
...
IMAGE_TIMEOUTS = {
    ...
    "gpt-image-2.5-flare": 360,              # 2.5 同價同參數，超時經驗沿用
