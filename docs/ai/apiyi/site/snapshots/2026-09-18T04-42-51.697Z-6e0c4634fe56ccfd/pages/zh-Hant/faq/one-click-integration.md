...
<CardGroup cols={2}>
  ...
  <Card title="AI 開發者套件" icon="blocks" href="/zh-Hant/developer-kit">
    技能包、命令列、給程式設計 Agent 的契約與模型登錄檔，三條路各有一段提示詞。
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="命令列零安裝">
    不想寫程式碼就在終端跑 `npx apiyi@latest check`（Node 18 以上）：它會檢查 Key、測節點、列模型，再用 `npx apiyi@latest chat "你好" -m gpt-5.4-mini` 發第一條訊息。命令表見 [AI 開發者套件](/zh-Hant/developer-kit#cli-命令列)。
  </Step>
</Steps>
