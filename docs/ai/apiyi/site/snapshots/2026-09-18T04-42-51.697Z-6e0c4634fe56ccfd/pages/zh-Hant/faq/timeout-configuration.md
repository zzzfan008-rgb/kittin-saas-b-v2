...
<AccordionGroup>
  ...
  <Accordion title="② 推理型 / 長輸出文本模型：非流式撐不住，改用流式">
    普通文本模型通常幾秒內就返回，容易讓人誤以為"文本呼叫不用管 timeout"。但**推理型（reasoning / thinking）模型、以及一次產出上萬字的長輸出場景是例外**：

    * `gemini-3.1-pro-preview`
    * `gpt-5.6-sol`
    * `gpt-5.5-pro`（更貴也更慢）
    * `claude-opus-5` 等開啟了深度思考 / 高 effort 的模型
    * 漫劇指令碼、文學創作、長翻譯、大段程式碼等長輸出任務

    這類呼叫會先長時間推理、再逐 token 產出長文，**總耗時達到十幾分鍾是常態**。

    **關鍵：非流式撐不住，但流式配合合適的 read timeout 可以解決。** 非流式要把整段生成完才一次性回寫，你的 read timeout 得和整段生成競速，稍有波動就斷。改用流式後，首位元組幾秒內到達、之後每隔幾十秒必有資料事件（實測思考階段最大靜默約 42 秒，且期間有 keepalive ping），你的 read timeout 只需覆蓋「事件間隔」而不是整段總時長。

    **實踐建議**：

    * **長輸出優先用流式**（`stream=True`），read timeout 按事件間隔設 **90-120 秒**即可，不要設成覆蓋整段生成的巨大值。
    * 若堅持非流式，把 timeout 設到 **300-600 秒**並接受不穩定；生成越久越容易在拿到結果前先斷。
    * 把思考檔位（`reasoning_effort` / `thinking`）與預期耗時對應起來——檔位越高，需要留的時間越多。

    長輸出場景的完整做法（流式程式碼、max\_tokens、stop\_reason、重試）見 [長文輸出實戰建議](/zh-Hant/api-capabilities/long-form-output-practices)。
  </Accordion>
