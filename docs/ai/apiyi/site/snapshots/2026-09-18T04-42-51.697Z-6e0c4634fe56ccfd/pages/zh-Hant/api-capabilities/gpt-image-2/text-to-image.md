...
> gpt-image-2.5-flare / gpt-image-2.5-sunburst / gpt-image-2 文生圖 API 參考與線上除錯 — 任意合法尺寸（含 4K），按 token 計費，三款同價同參數
...
<Warning>
  ...
  * `input_fidelity` —— 三款模型均強制啟用高保真，傳了會 400 報錯（2.5 實測 2026-09-09：`does not support the 'input_fidelity' parameter`；從 1.5 遷移時直接刪掉這一行）
...
| 引數                   | 型別     | 必填 | 預設     | 說明                                                                                                                                                                                      |
| -------------------- | ------ | -- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model`              | string | 是  | —      | `gpt-image-2.5-flare`（速度優先，日常首選）/ `gpt-image-2.5-sunburst`（畫質與編輯精度優先）/ `gpt-image-2`（上一代，仍可用）；生產環境可鎖日期快照 `gpt-image-2.5-flare-2026-09-08` / `gpt-image-2.5-sunburst-2026-09-08`。三款同價同參數 |
| `prompt`             | string | 是  | —      | 提示詞，支援中英文；最長 32,000 字元（原廠上限，按字元不按 token），長資料先提煉再傳，見 [長提示詞篇](/zh-Hant/api-capabilities/image-long-prompt)                                                                                |
| `size`               | string | 否  | `auto` | 輸出尺寸，預設或滿足約束的自定義尺寸                                                                                                                                                                      |
| `quality`            | string | 否  | `auto` | `low` / `medium` / `high` / `xhigh` / `max` / `auto`；`xhigh` / `max` 是 2.5 新增檔位，`gpt-image-2` 只到 `high`                                                                                 |
...
<Warning>
  **`quality` 不要傳舊版 DALL·E 的 `standard` / `hd`。** 只接受 `low` / `medium` / `high` / `xhigh` / `max` / `auto` 六個官方列舉值（`xhigh` / `max` 僅 2.5 兩款接受）。舊值在不同後端渠道下行為不一致：有時直接 400 報錯（`invalid_value`），有時被靜默忽略、按 `auto` 檔跑出結果（費用不可控）。請始終顯式傳官方列舉值之一。
</Warning>
