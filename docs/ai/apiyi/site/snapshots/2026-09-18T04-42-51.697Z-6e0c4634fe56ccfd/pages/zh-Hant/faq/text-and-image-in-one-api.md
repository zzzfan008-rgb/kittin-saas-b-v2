...
<Info>
  **三句話講完：**
  ...
  3. **其餘場景都是編排**：對話模型 + 獨立出圖介面（Images API）兩個 API 協作。OpenAI Responses 的原生 `image_generation` 工具在 API易 **不建議使用**——只能按次固定收費、計費不合理，且穩定性無法保證。
</Info>
...
| **C. Responses 原生工具** | `gpt-5.5` + `tools: [{"type": "image_generation"}]` | ✅ 有                        | **API易 不建議**：按次固定收費、穩定性無法保證，請走 A |
...
<AccordionGroup>
  ...
  <Accordion title="C. Responses 原生 image_generation 工具 —— API易 不建議使用">
    這條路是調 `POST /v1/responses`，模型用 `gpt-5.5`，請求裡掛上原生出圖工具：
    ...
    和正常的文本輸出並列。形態上它是 OpenAI 側最接近「會畫畫的聊天模型」的一條路，**但在 API易 不建議使用**。

    <Warning>
      **為什麼不建議**：Responses 裡的出圖工具在 API易 **只能按次計費**——每次出圖固定收一筆約 \$0.20 的工具呼叫費，
      無法像路線 A 那樣按實際用量計費，收費方式不合理；並且受資源供給限制，這條鏈路的**穩定性我們無法保證**。

      出圖請一律走路線 A 的 Images API（`/v1/images/generations` / `/v1/images/edits`），按量計費。
      想要「Agent 自己決定畫不畫」的效果，按下面「邊聊邊出圖」的編排方式來搭即可，效果一樣、計費清楚。
    </Warning>
  </Accordion>
...
<AccordionGroup>
  <Accordion title="誤區一：多模態模型 = 能生成圖片">
    **不成立。** 多模態在 API 語境裡預設指**輸入側**能力。`gpt-5.5` 能看懂你發的設計稿，
    但它自己吐不出一張圖——想讓它出圖，得另外調出圖介面（路線 A）；Responses 原生出圖工具（路線 C）在 API易 不建議使用。
  </Accordion>
