...
> Gemini 3.8 Flash 模型詳情：輸入 $0.75 / 輸出 $3.75 每 1M tokens，2 個分組可用。

與 3.7 Flash 同價的新一版 Flash，150 例實測能力逐項對齊；官方規格尚未公佈。思考檔位只剩 low/medium/high，minimal 已移除。
...
以下示例通過 OpenAI Chat Completions（`/v1/chat/completions`）呼叫 `gemini-3.8-flash`。base\_url 換成 `https://api.apiyi.com/v1` 即可，其餘用法與官方一致。
...
<CardGroup cols={2}>
  <Card title="上線公告" icon="megaphone" href="/news/gemini-3-8-flash-launch">
    Gemini 3.8 Flash 的釋出背景、實測表現與遷移建議
  </Card>

  <Card title="Gemini 3.8 Flash 概覽" icon="book-open" href="/zh-Hant/api-capabilities/gemini-3-8-flash/overview">
    引數、呼叫方式與最佳實踐
  </Card>
  ...
  <Card title="Gemini 3.7 Flash" icon="git-compare" href="/zh-Hant/models/gemini-3-7-flash">
    同系列模型詳情
  </Card>
