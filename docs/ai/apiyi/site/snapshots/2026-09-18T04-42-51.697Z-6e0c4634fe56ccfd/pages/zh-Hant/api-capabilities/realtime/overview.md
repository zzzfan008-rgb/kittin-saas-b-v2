...
> 四個即時語音模型，一條 wss 端點，一把 API易 令牌：雙向流式音訊、可打斷、兩種自動斷句、工具呼叫與圖片輸入。四款模型均已上架，預設分組直接呼叫；含兩套協議的完整欄位對照與零成本文本自測方案。
...
<Warning>
  **接入狀態（2026-09-14 (UTC+8) 更新）**：四款模型**均已正式上架，令牌勾選預設分組（default）即可直接呼叫，無需申請**，VIP / SVIP 分組同樣可用。歡迎測試、體驗與對接；文件有缺漏或與實測不符，歡迎直接反饋給我們。上游協議與行為仍可能調整，本頁「已知限制與規避」一節列出的差異均為實測結論、且會隨上游變化更新；請在客戶端實現重連與降級後再投產。需要更高併發額度的客戶，歡迎通過[企業微信客服](https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec)聯絡，也可郵件 [hi@apiyi.com](mailto:hi@apiyi.com) / [feedback@apiyi.com](mailto:feedback@apiyi.com)。
</Warning>

<Note>
  **🎤 核心亮點**：單連線雙向流式音訊、**可隨時打斷**、`server_vad` 與 `semantic_vad` 兩種自動斷句、Function Calling 全鏈路（含結果回注）、圖片輸入、`usage` 按模態分列。四個模型的上述能力均已逐項實測通過（2026-08-24 首測、2026-09-14 複測 (UTC+8)）。
</Note>
...
<CardGroup cols={2}>
  <Card title="令牌與分組管理" icon="key-round" href="/zh-Hant/api-capabilities/token-management">
    四款模型在預設分組下直接可用；建立令牌、勾選分組與設定額度看這裡。
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="實測延遲與併發有據可查" icon="gauge">
    40 併發下握手 p50 約 1 秒、首個文本增量 p50 約 1 秒、120 路會話 120 路成功，測試口徑與日期都寫在「技術規格」裡。
  </Card>

  <Card title="接入問題直連技術支援" icon="handshake">
    企業微信直連，接入問題、併發擴容、上游行為變化都可以直接問到人。
  </Card>
</CardGroup>
...
| 模型                            | 協議家族           | 開放狀態   | 預設音色    | 輸入取樣率    | Prompt 快取 | 定位                                  |
| ----------------------------- | -------------- | ------ | ------- | -------- | --------- | ----------------------------------- |
| `gpt-realtime-2.1`            | Realtime GA 協議 | ✅ 預設分組 | `marin` | ≥ 24 kHz | ✅ 支援      | 旗艦，多語種與推理能力最強，支援 `reasoning.effort` |
| `gpt-realtime-2.1-mini`       | Realtime GA 協議 | ✅ 預設分組 | `marin` | ≥ 24 kHz | ✅ 支援      | 高性價比，日常對話夠用                         |
| `qwen3.5-omni-plus-realtime`  | 阿里雲百鍊協議        | ✅ 預設分組 | `Tina`  | 16 kHz   | ⏸ 暫不支援    | 中文場景旗艦                              |
| `qwen3.5-omni-flash-realtime` | 阿里雲百鍊協議        | ✅ 預設分組 | `Tina`  | 16 kHz   | ⏸ 暫不支援    | 中文場景高性價比                            |
...
下表為**各廠商的官方定價口徑**，單位為每 100 萬 tokens 美元，站內按同價逐 token 計費（2026-09-14 (UTC+8) 已逐模態對賬）。**站內實際扣費以[控制台呼叫日誌](/zh-Hant/api-capabilities/log-query)為準**；疊加[充值加贈活動](/zh-Hant/faq/recharge-promotions)後實付更低。

### Realtime GA 協議

| 模型                      | 文本輸入  | 文本輸出  | 快取讀（官方）            | 圖片輸入  | 音訊輸入 | 音訊輸出 |
| ----------------------- | ----- | ----- | ------------------ | ----- | ---- | ---- |
| `gpt-realtime-2.1`      | \$4   | \$24  | \$0.4（站內暫按 \$4）    | \$5   | \$32 | \$64 |
| `gpt-realtime-2.1-mini` | \$0.6 | \$2.4 | \$0.06（站內暫按 \$0.6） | \$0.8 | \$10 | \$20 |
...
<Note>
  **計費說明**：文本、音訊、圖片三類 token 按上表官方價逐筆計費，打斷（`response.cancel`）的一輪按實際已產出的 token 計，空會話不計費。**快取讀目前暫不打折**：命中的 `cached_tokens` 會在 `usage` 裡如實回顯，但站內暫按對應的文本輸入價計費，計費鏈路修復後自動按官方快取價生效並在[更新日誌](/changelog)公告。平臺會隨官方政策與供給能力動態調整價格，該能力以**保障供給、服務客戶**為主，並非盈利型定價。
</Note>
...
| 分組                | 覆蓋模型 | 說明                           |
| ----------------- | ---- | ---------------------------- |
| **default（預設分組）** | 四款全部 | 令牌勾選預設分組即可，無需申請；分組倍率 1，與官方同價 |
| VIP / SVIP        | 四款全部 | 已掛載，扣費按令牌所在分組的倍率計            |

<Note>
  四款模型共用同一條端點與同一把令牌，切換分組只需在[令牌管理](/zh-Hant/api-capabilities/token-management)裡改勾選，程式碼無需改動。
</Note>
...
| **圖片輸入** | 支援（兩家族寫法不同，見下）                                                                                     |
| **會話時長** | Realtime GA 協議：會話帶 `expires_at`，實測兩次分別約建連後 30 分鐘與 60 分鐘，以 `session.created` 回顯為準；百鍊協議：實測空閒 300 秒斷開 |

### 實測延遲與併發

2026-09-14 (UTC+8) 經 `api.apiyi.com` 公網路徑實測，`gpt-realtime-2.1` 與 `-mini` 各 20 路、40 路兩檔併發，單輪純文本問答：

| 指標           | 實測值（40 路 × 2 模型）                      |
| ------------ | ------------------------------------- |
| WebSocket 握手 | p50 0.96–1.03 秒，最大 1.40 秒             |
| 首個文本增量       | p50 0.99–1.08 秒，最大 1.67 秒             |
| 單輪問答整輪       | p50 1.30–1.44 秒，最大 2.11 秒             |
| 會話成功率        | 100%（20 路 + 40 路兩檔共 120 路，全部成功，零 429） |
| 空閒保持         | 靜默 5 分鐘後仍可對話，ping/pong 約 200 ms       |

<Warning>
  上述數字是特定時點、特定併發下的實測值，不構成效能承諾，**不提供可用率 SLA**，請在客戶端實現重連與降級。
</Warning>
...
以下各條均為實測結論，且都會打到客戶端程式碼，接入前請先看一遍。
...
| `POST /v1/realtime/client_secrets`（臨時金鑰）與 `POST /v1/realtime/calls`（WebRTC）返回 404 | 全部                  | 站內只支援 WebSocket 直連，WebRTC / SIP / ephemeral token 不可用。瀏覽器與移動端場景請走後端中繼，由後端持有令牌建立 WebSocket |
| 自定義音色（`audio.output.voice` 傳 `{id: …}` 物件）返回上游 500                                | Realtime GA 協議      | 只用內建音色名（`marin` / `cedar` / `alloy` 等字串）                                                  |

<Warning>
  上述行為可能隨上游變化而調整，本頁會持續更新。遇到本表之外的異常，歡迎通過[企業微信客服](https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec)或 [feedback@apiyi.com](mailto:feedback@apiyi.com) 反饋，附上時間點與 `session.id` 便於我們定位。
</Warning>
...
| 握手返回 503 且提示無可用渠道                         | 令牌所在分組沒有該模型（四款都在預設分組），或模型名拼錯 | 核對令牌分組與 `model` 引數                              |
...
<AccordionGroup>
  ...
  <Accordion title="握手就失敗 / 連不上，怎麼排查？">
    按順序查五件事：① 協議是不是寫成了 `https://`，應該是 `wss://`；② 端點有沒有帶 `?model=<模型名>`；③ `Authorization: Bearer <令牌>` 請求頭有沒有帶上；④ 令牌分組是否包含該模型（四款都在預設分組；不匹配會返回 503 提示無可用渠道）；⑤ 中間有沒有反向代理吃掉了 `Upgrade` 頭——自建閘道轉發時這一點很常見。
  </Accordion>
  ...
  <Accordion title="有 Prompt 快取嗎？怎麼確認命中？">
    Realtime GA 協議那兩個模型支援，且是自動生效的——實測同一會話內第二輪即命中，`usage` 的 `input_token_details.cached_tokens` 會有值（字首 ≥ 1024 token、以 128 為粒度）。**注意站內目前對命中的快取部分暫按文本輸入全價計費**，修復後會在更新日誌公告。阿里雲百鍊協議那兩個模型目前未觀察到快取命中。
  </Accordion>

  <Accordion title="支援 WebRTC / SIP / 臨時金鑰（client_secrets）嗎？">
    **不支援。** `POST /v1/realtime/client_secrets` 與 `POST /v1/realtime/calls` 在站內都返回 404，SIP 同樣不可用，只有 `wss://api.apiyi.com/v1/realtime` 這一條 WebSocket 端點。瀏覽器或移動端場景請寫一個後端中繼：後端持有令牌並建立 WebSocket，前端只與你自己的服務通訊。
  </Accordion>

  <Accordion title="gpt-realtime-2.1 的 reasoning.effort、noise_reduction 這些 GA 新欄位能用嗎？">
    能，實測全部原樣透傳並在 `session.updated` 裡回顯：`reasoning.effort`（`minimal` / `low` / `medium` / `high` / `xhigh`，兩款都接受）、`audio.input.noise_reduction`、`audio.input.turn_detection.idle_timeout_ms`、`audio.input.transcription.model`（含 `gpt-realtime-whisper`）、`truncation`、`tracing`、`max_output_tokens`、`parallel_tool_calls`。欄位語義以 OpenAI 官方參考為準，閘道不做改寫。
  </Accordion>

  <Accordion title="成本怎麼估？文本和音訊是分開算的嗎？">
    `response.done` 事件的 `usage` 欄位按模態分列返回（文本 / 音訊 / 圖片，輸入輸出各一組），可以據此歸因；控制台呼叫日誌的詳情裡也帶同一份分模態 `usage`，每完成一輪 `response.done` 落一條記錄。音訊檔位顯著高於文本，所以聯調期建議跑純文本。**準確扣費請以[控制台呼叫日誌](/zh-Hant/api-capabilities/log-query)為準。**
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="企業微信客服" icon="headphones" href="https://work.weixin.qq.com/kfid/kfc9adfd5810ece25ec">
    接入問題、併發擴容、文件缺漏，直接找到人。
  </Card>
</CardGroup>

<Info>
  四款 Realtime 模型均已正式上架（預設分組）。本頁實測結論來自 2026-08-24 首測與 2026-09-14 複測 (UTC+8)，會隨上游變化持續更新。有接入計劃、遇到本頁未覆蓋的問題、或需要更高併發額度，歡迎聯絡 [hi@apiyi.com](mailto:hi@apiyi.com) / [feedback@apiyi.com](mailto:feedback@apiyi.com)。
</Info>
