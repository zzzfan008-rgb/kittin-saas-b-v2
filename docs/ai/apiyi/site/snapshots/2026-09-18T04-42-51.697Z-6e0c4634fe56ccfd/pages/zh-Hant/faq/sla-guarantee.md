...
<CardGroup cols={2}>
  <Card title="大用量影像客戶異常計費補償" icon="image">
    針對 **Nano Banana Pro / 2**、**GPT-Image-2.5 / 2 系列** 大用量客戶（月消耗 \$10,000 以上），對明顯異常的計費（如超時卻計費）核對並補償。
  </Card>
...
針對 **Nano Banana Pro / 2**、**GPT-Image-2.5 / 2 系列** 等影像模型的大用量客戶（**月消耗 \$10,000（約 1 萬美金）以上**可參與），我們對**明顯異常的計費**提供補償保障。
...
<AccordionGroup>
  ...
  <Accordion title="大用量影像客戶的異常計費補償怎麼參與？">
    該保障面向 **Nano Banana Pro / 2、GPT-Image-2.5 / 2 系列等影像模型的大用量客戶**，**月消耗 \$10,000 以上**可參與，針對明顯異常的計費（例如請求超時卻仍按出圖計費）進行核對補償。**這不是"300 秒內必出圖否則賠付"的承諾**，而是對異常計費損失的補償。具體請聯絡客服開通與對接。
  </Accordion>
