...
> Gemini 3.6 Flash 模型詳情：輸入 $1.5 / 輸出 $7.5 每 1M tokens，1,048,576 上下文、65,536 最大輸出，3 個分組可用。

多模態旗艦 Flash，預設開思考（四檔可控），搜尋 grounding、程式碼執行、URL context 全放通。
...
以下示例通過 OpenAI Chat Completions（`/v1/chat/completions`）呼叫 `gemini-3.6-flash`。base\_url 換成 `https://api.apiyi.com/v1` 即可，其餘用法與官方一致。
...
<CardGroup cols={2}>
  <Card title="上線公告" icon="megaphone" href="/news/gemini-3-6-flash-lite-launch">
    Gemini 3.6 Flash 的釋出背景、實測表現與遷移建議
  </Card>

  <Card title="Gemini 3.6 Flash 概覽" icon="book-open" href="/zh-Hant/api-capabilities/gemini-3-6-flash/overview">
    引數、呼叫方式與最佳實踐
  </Card>
