...
> Gemini 3.5 Flash 模型詳情：輸入 $1.5 / 輸出 $9 每 1M tokens，1,000,000 上下文、64,000 最大輸出，3 個分組可用。

Terminal-Bench 2.1 76.2%，思維鏈帶加密推理上下文可跨呼叫保留；不支援 Computer Use。
...
以下示例通過 OpenAI Chat Completions（`/v1/chat/completions`）呼叫 `gemini-3.5-flash`。base\_url 換成 `https://api.apiyi.com/v1` 即可，其餘用法與官方一致。
...
<CardGroup cols={2}>
  <Card title="上線公告" icon="megaphone" href="/news/gemini-3-5-flash-launch">
    Gemini 3.5 Flash 的釋出背景、實測表現與遷移建議
  </Card>
