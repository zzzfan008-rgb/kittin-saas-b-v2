...
<Tip>
  上記の `undici` のインポートは別途インストールする必要があります（`npm i undici`）。これは組み込みの `fetch` を動作させますが、そのモジュール名では公開されておらず、インストールしたコピーも `setGlobalDispatcher` を通じて組み込みの `fetch` にアクセスします。

  `maxRetries`、`connectTimeout`、およびリクエスト全体のタイムアウトは、それぞれ異なるレイヤーに存在します。誤ったものを設定することは、Node 側で最もよくある落とし穴です。接続リセット、`UND_ERR_CONNECT_TIMEOUT`、またはプロキシ経由で大きなレスポンスが途中で切れる問題については、[画像 API の接続切断に関するトラブルシューティング](/ja/api-capabilities/image-connection-drops)を参照してください。
</Tip>
...
| フィールド             | タイプ  | 必須  | 説明                                                                                                                                                                                                          |
| ----------------- | ---- | --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model`           | テキスト | はい  | `gpt-image-2.5-all` または `gpt-image-2-all`（価格と動作は同じです）                                                                                                                                                       |
| `prompt`          | テキスト | はい  | 自然言語による編集・融合の指示                                                                                                                                                                                             |
| `image`           | ファイル | はい  | 参照画像。繰り返し指定できます                                                                                                                                                                                             |
| `size`            | テキスト | いいえ | **このフィールドは効果がなく、どのような値を送信しても暗黙的に無視されます。** 出力のアスペクト比は、**プロンプトで編集対象として指定された参照画像**に従います（複数画像の場合、必ずしも最初の画像とは限りません）。サイズを厳密に固定するには、[`gpt-image-2-vip`](/ja/api-capabilities/gpt-image-2-vip/image-edit) を使用してください |
| `response_format` | テキスト | いいえ | `b64_json`（デフォルト）または `url`                                                                                                                                                                                  |

<Tip>
  **マルチターンの反復**：前回の出力画像を `image` 入力として、新しい指示とともに再度入力し、結果を反復的に改善します。
</Tip>
