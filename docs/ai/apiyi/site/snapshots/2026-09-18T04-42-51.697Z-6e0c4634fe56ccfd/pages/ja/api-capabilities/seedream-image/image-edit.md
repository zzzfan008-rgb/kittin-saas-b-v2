...
<Note>
  **`extra_body`について（重要 — 追加のネスト層だと誤解しないでください）**
  ...
  `image`、`sequential_image_generation`、`watermark`は、OpenAI SDK の `images.generate()` の標準パラメータではないため、Python SDK で送信するには **`extra_body` 内に配置する必要があります**。

  ただし、`extra_body`は SDK のパラメータコンテナにすぎず、そのフィールドは **最上位レベルにフラット化されて統合されます**。リクエストボディでは、`model`および`prompt`と**同じレベル**になります。実際に送信される JSON は、以下の cURL 例と同一です（`image`は最上位レベルに配置されます）。リクエスト内に実際の `"extra_body": {...}` のネストは**ありません**。

  OpenAI SDK を使用せず、JSON を直接（requests / fetch などで）構築する場合は、**`extra_body`を記述しないでください**。`image`やその他のフィールドは、`model`と同じレベルに配置してください。
</Note>
...
<Warning>
  **参照画像は base64 ではなく公開 URL として渡してください（2026-09-11 以降、強調）**

  `image`配列のエントリが URL の場合、リクエストボディは数 KB にすぎず、BytePlus は**シンガポールリージョンから画像を直接ダウンロードします**（確認済み：取得元の IP は BytePlus に属しており、APIYI ゲートウェイは関与していません）。
  エントリが `data:image/...;base64,...`文字列の場合、高解像度の参照画像を数枚指定するだけで、ボディのサイズは簡単に 20〜30 MB に達します。そのボディ全体をまず APIYI ゲートウェイにアップロードしてからシンガポールへ転送する必要があります。国境をまたぐアップロードが遅い場合、リクエストボディの読み取りに対するプロバイダーの 600 秒の受信制限を超過し、`400 Error when parsing request`で失敗します。
  リクエスト全体が失敗し、URL モードにはフォールバックしません。これは、ゲートウェイが base64 をダウンロード可能なリンクに変換してくれないためです。

  * ✅ 画像を自分のオブジェクトストレージまたは画像ホストに保存し、認証なしの通常の GET で取得できる公開 URL を渡してください。各画像は約 10 MB 未満にしてください
  * ⚠️ base64 が避けられない場合は、先に圧縮してください：長辺を 2048 px 以内にし、品質 0.9 で再エンコードし、複数画像の場合は合計 6 MB 未満にしてください
  * ❌ 20 MB を超える base64 ボディを送信したり、プライベートネットワークのアドレスやログインが必要なリンクを渡したりしないでください（取得に失敗すると BytePlus は `InvalidParameter: Error while downloading`を返します）
</Warning>
