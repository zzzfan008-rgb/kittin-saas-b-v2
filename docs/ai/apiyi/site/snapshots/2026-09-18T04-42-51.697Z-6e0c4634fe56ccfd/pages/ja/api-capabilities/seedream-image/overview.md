...
<AccordionGroup>
  <Accordion title="5.0 Pro / 5.0 / 4.5 / 4.0 — どれを選ぶべきですか？">
    | 必要なもの                         | 推奨                                                                     |
    | ----------------------------- | ---------------------------------------------------------------------- |
    | 最新機能 + 総合的な最高性能               | `seedream-5-0-260128`                                                  |
    | 4K + 優れたテキスト描画（ポスター、広告）       | `seedream-4-5-251128`                                                  |
    | 4K + 最もお得な価格                  | `seedream-4-0-250828`                                                  |
    | 長時間実行する安定したバッチ制作              | `seedream-4-0-250828`（実績があり、最も安価で、高速なpromptモード）                        |
    | 最高の画像品質 / 複雑な指示（プロフェッショナルな作業） | `seedream-5-0-pro-260628`（\$0.12/リクエスト、1画像あたり約2分 — 必要な場合を除きスキップしてください） |

    完全な比較については、[過去のバージョン](/ja/api-capabilities/seedream-image/historical-versions)を参照してください。
  </Accordion>

  <Accordion title="画像編集でもgenerationsエンドポイントを使用するのはなぜですか？">
    Seedreamは生成と編集を統合したアーキテクチャを使用しているため、**個別の`/v1/images/edits`エンドポイントはありません**。OpenAIのgpt-image-2（`/v1/images/edits`へのmultipartアップロード）とは異なり、Seedreamは`application/json`を使用し、`image`フィールドに**URLを配列として**渡します。

    メリットは、プロトコルの一貫性、パラメータの再利用、モード切り替えの容易さです。詳細については、[画像編集](/ja/api-capabilities/seedream-image/image-edit)を参照してください。
  </Accordion>

  <Accordion title="imageフィールドはbase64を受け付けますか？">
    **はい**（テストで確認済みです）。data URIを使用してください：`data:image/<format>;base64,<base64 string>`。`<format>`は小文字にします（例：`data:image/jpeg;base64,...`）。同じ配列内でURLとbase64のエントリを混在させることもできます。**可能な限り公開URLを優先してください**：URLはAPIYIゲートウェイを経由せず、BytePlusによってシンガポールから直接ダウンロードされます。一方、base64のbodyは、最初に全体を国境越しにアップロードする必要があります。このリンクの速度が遅い場合、20 MBを超えるbodyはプロバイダーの600秒のリクエストbodyタイムアウトに達する可能性があり、ゲートウェイがbase64をURLに変換することもありません。詳細については、画像編集ページ上部の警告を参照してください。
  </Accordion>

  <Accordion title="複数画像の融合上限は？バッチシーケンスの上限は？">
    * **複数画像の融合**（`image`配列）：4.5 / 5.0-proは最大10件を明示的にサポートしています。5.0 / 4.0も複数画像をサポートしていますが、明示的な上限はドキュメントに記載されていません。
    * **バッチシーケンス**（`max_images`）：グローバルルール **入力参照数 + 出力数 ≤ 15** によって制限されます。融合とシーケンスを組み合わせる場合は、合計数を数えてください。**5.0-proはバッチシーケンスをサポートしていません**（`sequential_image_generation`を渡すと400が返されます）。
  </Accordion>

  <Accordion title="b64_jsonにはdata:imageプレフィックスが必要ですか？">
    `response_format`によって異なります。

    * `response_format: "url"`（デフォルト） → `data[0].url`は一時的な署名付きURLです。`<img src=...>`で直接レンダリングしてください
    * `response_format: "b64_json"` → `data[0].b64_json`は**通常のbase64文字列**です（`data:image/...;base64,`プレフィックスなし）。デコードしてディスクに書き込むか、ブラウザーでレンダリングするために手動でプレフィックスを付加してください。
  </Accordion>

  <Accordion title="ストリーミング出力はサポートされていますか？">
    5.0 / 4.5 / 4.0では`stream: true`経由でサポートされています。ストリーミングは、長いpromptや高解像度画像で特に便利です。フロントエンドで部分的な結果を段階的にレンダリングできます。**`seedream-5-0-pro`はストリーミングをサポートしていません**。`stream`を渡すと400が返されます。
  </Accordion>

  <Accordion title="レート制限は？">
    **デフォルトでは毎分500画像**（1分あたりの最大画像数）で、バージョン間で統一されています。より高いクォータについては営業担当にお問い合わせください。
  </Accordion>

  <Accordion title="失敗したリクエストにも課金されますか？">
    **いいえ**。BytePlusにはモデレーション機能が組み込まれています。モデレーションによる拒否およびパラメータエラーでは`400` / `403`が返され、**課金されません**。その他の課金されないエラーには、`401`（無効なtoken）や`429`（レート制限）が含まれます。**有効なレスポンスを伴う生成の成功（`200`）のみが課金対象です**。
  </Accordion>

  <Accordion title="公式OpenAI SDKを使用できますか？">
    はい、コードを変更せずに使用できます。`base_url`を`https://api.apiyi.com/v1`に指定し、拡張パラメータ（`image` / `sequential_image_generation` / `watermark`など）を`extra_body`経由で渡します。
  ...
  <Accordion title="生成された画像の所有者は誰ですか？">
    生成された画像は、商用および非商用の目的で使用できます。詳細については、BytePlusの利用規約を参照してください。
  </Accordion>

  <Accordion title="透明な背景はサポートされていますか？">
    `seedream-5-0` / `seedream-5-0-pro`は`png`出力をサポートしており、promptで「透明な背景、アルファチャンネル」と指定すると透明な背景を生成できます。`seedream-4-5` / `4-0`は`jpeg`のみを出力し、**透明性をサポートしていません**。背景の削除はご自身で後処理してください。
  </Accordion>

  <Accordion title="実行中の生成をキャンセルできますか？">
    **いいえ**。`/v1/images/generations`は同期処理です。送信後、リクエストは完了まで実行されます。クライアントが切断しても、サーバーは処理を完了し、課金も行われます。クライアントのタイムアウトを設定し、切断すればコストを節約できるという前提は避けてください。
  </Accordion>
</AccordionGroup>
