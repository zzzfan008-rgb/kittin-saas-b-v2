...
<AccordionGroup>
  <Accordion title="502発生中のリクエストは課金されますか？">
    **いいえ。** 502は、リクエストが実際にはバックエンドサービスに到達しなかったことを意味します。モデルの利用は発生していないため、**課金記録には何も表示されません**。

    これは便利な診断方法でもあります。失敗したリクエストに[呼び出しログ](/ja/faq/call-logs)上の課金エントリがない場合、そのリクエストはサーバー側で処理されておらず、安全に再送できます。
  </Accordion>

  <Accordion title="502はタイムアウト、429、524とどう異なりますか？">
    * **`502`**: バックエンドサービスが一時的に利用できません（コンテナの再起動中）。30〜60秒待って再試行してください。課金されません。
    * **タイムアウト / 接続切断**: クライアントのタイムアウトが短すぎます。サーバーでは引き続きリクエストが実行され、通常どおり課金される可能性があります。[APIタイムアウトを回避する方法](/ja/faq/timeout-configuration)を参照してください。
    * **`429`**: 同時実行数またはレート制限に達しています。サービスの可用性とは無関係です。[API同時実行数の制限](/ja/faq/api-concurrency)を参照してください。
    * **`524`**: CDNエンドポイント（`api-cf.apiyi.com`）で、およそ100秒を超えるリクエストを実行しています。エンドポイントを切り替えてください。

    これらにはまったく異なる対応が必要です。**直接再試行すべきなのは502/503のみです**。
  </Accordion>

  <Accordion title="WebコンソールとAPIが同時に失敗するのはなぜですか？">
    WebコンソールとAPIは同じサービスによって支えられています。コンテナの再起動中は両方とも**同時に利用できなくなり、同時に復旧します**。そのため、「Webサイトも停止している」という状況は、クライアント設定の問題ではなく、一時的なプラットフォーム側の事象であることを正確に示しています。
  </Accordion>

  <Accordion title="これは頻繁に発生しますか？">
    いいえ。定常的に発生するものではありません。一時的な502は通常、急激なトラフィックスパイクに関連しており、散発的に発生します。

    **2026年8月現在、バックエンドサーバーのアップグレードとスケールアップを進めています**。これにより、このような一時的な502の発生頻度は大幅に低減されます。プラットフォーム側のインシデントが発生した場合は、[ライブステータスフィード](/en/live)で直ちにステータス更新と復旧状況を公開します。
  </Accordion>

  <Accordion title="プラットフォームの問題か、自身のネットワークの問題かを見分けるにはどうすればよいですか？">
    次の2つをすばやく確認してください。

    1. **Webコンソールを開く**: `api.apiyi.com` が502を返し、コンソールも読み込めない場合は、ほぼ確実に一時的なプラットフォーム側の再起動です。1分ほど待ってください。
    2. **ネットワークを切り替える**: モバイルデータ通信（別のキャリア）でコンソールの読み込みを試すか、以下のコマンドを実行してください。そこで動作する場合、問題はローカルネットワークまたはプロキシにあります。
    ...
    別のネットワークでも5分以上すべてが502を返し続ける場合は、サポートにお問い合わせください。

    また、Webコンソールは正常に動作する一方で、スクリプトでは本文が空の断続的な502が発生し、それらのリクエストが呼び出しログにない場合、502はローカルのプロキシソフトウェアに起因している可能性が高いです。[スクリプトで502が発生するが呼び出しログに何もない場合](/ja/faq/proxy-empty-502)を参照してください。
  </Accordion>
</AccordionGroup>
