...
| 機能                          | 公式Webアプリ        | 直接のAPI呼び出し                                                  |
| --------------------------- | --------------- | ----------------------------------------------------------- |
| モデルの重み                      | 同一              | 同一                                                          |
| システムprompt                  | ベンダーにより注入（非公開）  | なし — 自分で記述                                                  |
| Web検索                       | 組み込み、必要に応じて自動実行 | toolを有効化するか、検索を自分で連携                                        |
| 数学計算 / コード実行                | 組み込みサンドボックス     | tool callingを自分で実装                                          |
| ファイルおよび画像の解析                | 組み込み            | 自分でアップロードまたはbase64エンコード                                     |
| 会話メモリ                       | 自動的に保存          | ステートレス — 履歴を自分で渡す（[APIにはメモリがありますか？](/ja/faq/api-memory)を参照） |
| コンテキストオーバーフロー               | 自動的に圧縮          | 自分で切り詰めるか要約                                                 |
| パラメータ（temperature、thinking） | ベンダーが調整         | デフォルト値 — 自分で揃える                                             |
| 出力形式                        | フロントエンドでレンダリング  | プレーンテキスト / JSON                                             |
