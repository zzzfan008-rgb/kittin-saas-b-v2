...
<Info>
  **三句话讲完：**
  ...
  3. **其余场景都是编排**：对话模型 + 独立出图接口（Images API）两个 API 协作。OpenAI Responses 的原生 `image_generation` 工具在 API易 **不建议使用**——只能按次固定收费、计费不合理，且稳定性无法保证。
</Info>
...
| **C. Responses 原生工具** | `gpt-5.5` + `tools: [{"type": "image_generation"}]` | ✅ 有                        | **API易 不建议**：按次固定收费、稳定性无法保证，请走 A |
...
<AccordionGroup>
  ...
  <Accordion title="C. Responses 原生 image_generation 工具 —— API易 不建议使用">
    这条路是调 `POST /v1/responses`，模型用 `gpt-5.5`，请求里挂上原生出图工具：
    ...
    和正常的文本输出并列。形态上它是 OpenAI 侧最接近「会画画的聊天模型」的一条路，**但在 API易 不建议使用**。

    <Warning>
      **为什么不建议**：Responses 里的出图工具在 API易 **只能按次计费**——每次出图固定收一笔约 \$0.20 的工具调用费，
      无法像路线 A 那样按实际用量计费，收费方式不合理；并且受资源供给限制，这条链路的**稳定性我们无法保证**。

      出图请一律走路线 A 的 Images API（`/v1/images/generations` / `/v1/images/edits`），按量计费。
      想要「Agent 自己决定画不画」的效果，按下面「边聊边出图」的编排方式来搭即可，效果一样、计费清楚。
    </Warning>
  </Accordion>
...
<AccordionGroup>
  <Accordion title="误区一：多模态模型 = 能生成图片">
    **不成立。** 多模态在 API 语境里默认指**输入侧**能力。`gpt-5.5` 能看懂你发的设计稿，
    但它自己吐不出一张图——想让它出图，得另外调出图接口（路线 A）；Responses 原生出图工具（路线 C）在 API易 不建议使用。
  </Accordion>
