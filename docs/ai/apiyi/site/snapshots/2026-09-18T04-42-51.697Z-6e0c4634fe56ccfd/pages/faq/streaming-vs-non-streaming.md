...
| 推理型 / 长输出模型        | **推荐流式** | 思考阶段有静默但有 keepalive，read timeout 按事件间隔设即可，见下方误区①                                           |
...
<AccordionGroup>
  <Accordion title="误区①：开了流式就不用管 timeout 了">
    **要分清两件事。** 流式不缩短总生成时间——从首字到最后一个 token 的总时长该多久还是多久，这点没变。

    但流式确实能解决「拿不到结果」：只要把客户端的 read timeout 设成覆盖**两次事件之间的间隔**（推理型模型思考阶段实测最大静默约 42 秒、且期间有 keepalive ping，设 90-120 秒即可），就不会误触发。反倒是**纯非流式**要让一个超时值兜住整段十几分钟的生成，才是真正撑不住的那种。

    所以正确做法是：长输出用流式 + read timeout 按事件间隔设。分场景取值见 [如何避免接口超时](/faq/timeout-configuration)，完整做法见 [长文输出实战建议](/api-capabilities/long-form-output-practices)。
  </Accordion>
