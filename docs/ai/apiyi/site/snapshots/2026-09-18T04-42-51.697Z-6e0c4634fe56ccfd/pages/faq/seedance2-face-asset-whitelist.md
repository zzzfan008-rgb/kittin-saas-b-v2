...
# Seedance 2.0 / 2.5 人脸素材为什么被拦截？

> 说明 Seedance 2.0 与 2.5 的真人脸、虚拟人脸限制，以及素材入库和真人认证的正确使用流程。

## 简短回答

Seedance 2.0 与 2.5 通道自带的“虚拟人脸白名单”是**平台通道侧的上游能力**，不需要也无法在您的 API易 令牌或账号上手动开启。但是，这不代表含人脸图片可以直接通过公网 URL 或 Base64 作为首帧或参考图提交。

如果 AI 生成的虚拟模特图被识别为“可能含真人”，请先将图片上传到 [Seedance 2.0 / 2.5 素材库](https://icover.ai/zh/seedance-official/asset-library)，等待入库状态变为“可用”，取得 `asset://xxx` 素材 ID，再用该素材 ID 创建视频任务。
...
## Seedance 2.0 和 2.5 的人脸规则一样吗？

就人脸素材审核而言，当前文档对 Seedance 2.0 和 2.5 给出的处理方式一致：两代都不能直接提交写实真人脸作为首帧或参考图。这里的“规则一致”仅针对人脸素材审核，不代表两代模型的全部能力和参数完全相同。

* AI 生成的写实虚拟人像也可能被识别为疑似真人
* 虚拟人像应先走素材入库，取得 `asset://xxx` 后再引用
* 真人人脸需要由本人完成活体认证，并使用对应的授权真人素材

<Note>
  `SeeDance2` 是模型调用分组，不是可以绕过人脸审核的放行开关。Seedance 2.5 与 2.0 系列四个模型**同走 `SeeDance2` 分组**（0.18x），一把令牌即可同时调用两代模型，无需单独勾选其它分组。
</Note>
...
调用 Seedance 2.5 或 2.0 系列时，都需要确认令牌已勾选 `SeeDance2` 分组。分组只决定模型调用入口和计费倍率，不会改变人脸审核规则。
...
<AccordionGroup>
  <Accordion title="Seedance 2.5 是否放宽了真人脸限制？">
    没有。就人脸素材审核而言，2.5 与 2.0 的处理方式一致，直接提交写实真人脸仍可能被上游内容安全机制拦截。
  </Accordion>

  <Accordion title="SeeDance2 分组是人脸白名单吗？">
    不是。`SeeDance2` 是 2.5 与 2.0 系列共用的模型调用分组，负责模型路由和计费。即使令牌已勾选该分组，仍需按素材类型完成素材入库或真人认证。
  </Accordion>
</AccordionGroup>

## Seedance 2.0 / 2.5 定向开放说明

Seedance 系列采用原厂直转并受平台保护政策约束，目前主要面向有明确业务需求的定向客户，因此文档中心可能不展示公开导航入口。页面仍可通过以下地址直接访问：

<CardGroup cols={2}>
  <Card title="Seedance 2.0 / 2.5 总览" icon="film" href="/api-capabilities/seedance2/overview">
    查看模型能力、输入限制、计费与常见问题。
  </Card>
...
* [Seedance 2.0 / 2.5 在线测试](https://icover.ai/zh/seedance-official)
