...
返回的图就是带 alpha 通道的透明底 PNG，不需要任何后处理抠图。文生图、图片编辑两条路都支持。
...
| 模型                                                               | 支持方式                                                                              | 稳定性                                      |
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------- | ---------------------------------------- |
| `gpt-image-2.5-flare` / `gpt-image-2.5-sunburst` / `gpt-image-2` | **参数控制** —— `background: "transparent"` + `output_format` 为 `png` 或 `webp`（三款同参数） | ✅ 稳定，推荐（前提是提示词别描述场景，见下文）                 |
| `gpt-image-1.5` / `gpt-image-1`                                  | 参数控制，同上                                                                           | ✅ 稳定（老模型，新项目建议直接用 `gpt-image-2.5-flare`） |
...
`background: "transparent"` 只保证输出带 alpha 通道、并让模型倾向去背；**提示词或参考图一旦在描述一个完整场景，模型会按语义把场景画出来**，透明参数拦不住。这是「透明背景时好时坏」最常见的原因，与画质档位无关。

2026-09-11 用 `gpt-image-2.5-sunburst` 实测 30 次（medium / high 各半，文生图 + 参考图编辑）：

| 提示词写法                                                                                       | 真透明                                |
| ------------------------------------------------------------------------------------------- | ---------------------------------- |
| 只描述主体（苹果、耳机、挥刀的武士），写明 isolated / no background                                              | 16 / 16，medium 与 high 一样稳          |
| 写了场景：「秋天枫叶林里挥刀的武士」                                                                          | 0 / 3，**整张画满秋林**，medium 和 high 都一样 |
| 同一场景改写：几片枫叶飘在武士身边 + `isolated character on a transparent background, no scenery, no ground` | 2 / 2                              |
| 参考图自带全景，提示词只描述场景、不提去背                                                                       | 1 / 2，抛硬币                          |
| 参考图自带全景，提示词写明 `remove the scenery entirely, keep only the subject`                          | 8 / 8                              |

写法上记三条：

* ❌ 别写地面、天空、房间、森林这类**环境词**；要氛围就把元素挂在主体上（「几片飘落的枫叶」而不是「枫叶林」）
* ✅ 句尾固定加 `isolated subject on a transparent background, no scenery, no ground, no shadow`
* ✅ 参考图带背景时，编辑提示词里明确 `remove the background entirely, keep only the character`

把 `quality` 从 `medium` 提到 `high` **不会**让透明更稳，只会多 4 倍 token（439 → 1,756）。
...
<AccordionGroup>
  ...
  <Accordion title="出图确实是白底，不是透明的">
    ...
    三件都对还是白底或整张背景，几乎一定是**提示词（或参考图）描述了场景**：模型按语义把环境画出来了，参数压不过。见上文「提示词别描述场景」。
  </Accordion>
  ...
  <Accordion title="提示词里已经写了 transparent background，为什么还要传参数">
    提示词只是"请求模型这么画"，模型可能画一个看起来像透明的灰白棋盘格，那仍然是不透明像素。真正的 alpha 通道只有 `background: "transparent"` 参数能保证。反过来也一样：参数保证的是通道，压不过提示词里的场景描述，两边要配合。
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="官转 vs 官逆对比" icon="git-compare" href="/api-capabilities/gpt-image-2/vs-gpt-image-2-all">
    `gpt-image-2.5-flare` / `sunburst` / `gpt-image-2` / `-all` / `-vip` 怎么选
  </Card>
