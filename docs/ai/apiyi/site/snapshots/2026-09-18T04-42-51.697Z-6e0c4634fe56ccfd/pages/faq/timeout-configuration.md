...
<AccordionGroup>
  ...
  <Accordion title="② 推理型 / 长输出文本模型：非流式撑不住，改用流式">
    普通文本模型通常几秒内就返回，容易让人误以为"文本调用不用管 timeout"。但**推理型（reasoning / thinking）模型、以及一次产出上万字的长输出场景是例外**：

    * `gemini-3.1-pro-preview`
    * `gpt-5.6-sol`
    * `gpt-5.5-pro`（更贵也更慢）
    * `claude-opus-5` 等开启了深度思考 / 高 effort 的模型
    * 漫剧脚本、文学创作、长翻译、大段代码等长输出任务

    这类调用会先长时间推理、再逐 token 产出长文，**总耗时达到十几分钟是常态**。

    **关键：非流式撑不住，但流式配合合适的 read timeout 可以解决。** 非流式要把整段生成完才一次性回写，你的 read timeout 得和整段生成竞速，稍有波动就断。改用流式后，首字节几秒内到达、之后每隔几十秒必有数据事件（实测思考阶段最大静默约 42 秒，且期间有 keepalive ping），你的 read timeout 只需覆盖「事件间隔」而不是整段总时长。

    **实践建议**：

    * **长输出优先用流式**（`stream=True`），read timeout 按事件间隔设 **90-120 秒**即可，不要设成覆盖整段生成的巨大值。
    * 若坚持非流式，把 timeout 设到 **300-600 秒**并接受不稳定；生成越久越容易在拿到结果前先断。
    * 把思考档位（`reasoning_effort` / `thinking`）与预期耗时对应起来——档位越高，需要留的时间越多。

    长输出场景的完整做法（流式代码、max\_tokens、stop\_reason、重试）见 [长文输出实战建议](/api-capabilities/long-form-output-practices)。
  </Accordion>
