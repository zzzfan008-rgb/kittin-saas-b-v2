...
<Warning>
  **`input_fidelity="high"`를 전달하지 마십시오** — `gpt-image-2`는 입력 이미지를 기본적으로 항상 고충실도로 처리합니다. API는 이 매개변수를 조정하도록 허용하지 않습니다. 이를 전달하면 400 오류가 반환됩니다. 그냥 생략하십시오.
</Warning>

## cURL 예시

이미지 편집 엔드포인트에는 `multipart/form-data`이 필요합니다 — 이미지와 마스크를 일반 JSON 필드로는 제출할 수 없습니다:
...
<Warning>
  `-F`를 사용할 때는 **수동으로 `-H "Content-Type: multipart/form-data"`를 설정하지 마십시오**. curl이 `boundary`를 자동으로 생성해야 하며, 헤더를 수동으로 설정하면 boundary가 사라져 서버가 파일을 파싱할 수 없습니다.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Image 편집 API 참고 문서" icon="image" href="/ko/api-capabilities/gpt-image-2/image-edit">
