...
<Tip>
  위의 `undici` import는 별도로 설치해야 합니다(`npm i undici`) — 내장된 `fetch`를 구동하지만 해당 모듈 이름으로는 노출되지 않으며, 설치한 복사본도 `setGlobalDispatcher`를 통해 내장된 `fetch`에 계속 연결됩니다.

  `maxRetries`, `connectTimeout` 및 전체 요청 타임아웃은 모두 서로 다른 계층에 있으며, 잘못된 항목을 설정하는 것이 Node 측에서 가장 흔한 함정입니다. 연결 재설정, `UND_ERR_CONNECT_TIMEOUT`, 또는 프록시 뒤에서 잘린 대용량 응답에 대해서는 [Image API 연결 끊김 문제 해결](/ko/api-capabilities/image-connection-drops)를 참조하세요.
</Tip>
...
| ----------------- | ---- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model`           | text | Yes      | 고정: `gpt-image-2-all`                                                                                                                                                                                         |
| `prompt`          | text | Yes      | 자연어 편집/융합 지시문                                                                                                                                                                                                 |
| `image`           | file | Yes      | 참조 이미지입니다. 반복 지정할 수 있습니다.                                                                                                                                                                                     |
| `size`            | text | No       | **이 필드는 효과가 없으며, 어떤 값을 보내도 조용히 무시됩니다.** 출력 종횡비는 prompt가 편집 대상으로 지목한 참조 이미지를 따릅니다(여러 이미지 시나리오에서 반드시 첫 번째 이미지는 아닙니다). 엄격한 크기 고정을 위해 [`gpt-image-2-vip`](/ko/api-capabilities/gpt-image-2-vip/image-edit)를 사용합니다 |
| `response_format` | text | No       | `b64_json`(기본값) 또는 `url`                                                                                                                                                                                      |
