...
<Prompt description="코딩 에이전트가 APIYI를 스스로 통합하게 하십시오. Codex, Claude Code, Cursor 및 유사한 도구에 복사하여 붙여넣으십시오." icon="bot" actions={["copy"]}>
  APIYI를 이 프로젝트에 통합하십시오.

  1. 먼저 통합 지식을 불러오십시오: `npx skills add https://docs.apiyi.com`를 실행하여 APIYI 스킬을 설치하십시오. 해당 명령이 작동하지 않으면 [https://docs.apiyi.com/skill.md를](https://docs.apiyi.com/skill.md를) 전체로 가져와 읽으십시오 — 내용은 동일합니다.
  2. 더 구체적인 내용이 필요하면 [https://docs.apiyi.com/llms.txt에서](https://docs.apiyi.com/llms.txt에서) 해당 페이지를 찾으십시오. 문서 URL에 `.md`를 덧붙이면 일반 Markdown 버전을 얻을 수 있으며, HTML을 스크래핑하는 것보다 훨씬 더 적은 token을 사용합니다.
  3. API 키를 요청하십시오(저는 [https://api.apiyi.com/token에서](https://api.apiyi.com/token에서) 복사해 오겠습니다) 그리고 `APIYI_API_KEY` 환경 변수에 넣으십시오. **하드코딩하지 말고 git에 커밋하지 마십시오.**
  4. 이 프로젝트의 기존 스택에서 최소한으로 실행 가능한 예제를 작성하십시오. 시작 모델은 `gpt-5.4-mini`입니다. base URL은 SDK에 따라 다릅니다: OpenAI SDK는 `https://api.apiyi.com/v1`를 사용하고, Anthropic SDK는 /v1 없이 루트 도메인 `https://api.apiyi.com`를 사용하며, Google GenAI SDK는 api\_version을 v1beta로 설정한 루트 도메인을 사용합니다.
  5. 실제로 실행하고 응답을 보여주십시오. 동작하면 호출 비용이 얼마인지와 실제 사용에서는 어떤 모델로 전환할지 알려주십시오.
</Prompt>

### 수행 내용

<Steps>
  <Step title="스킬을 설치하거나 skill.md를 직접 읽으십시오">
    `https://docs.apiyi.com/skill.md`는 기계용으로 작성된 통합 참고 자료입니다: 엔드포인트, 인증, 모델 명명 규칙, 알려진 함정 및 검증 체크리스트가 포함됩니다. 이를 읽으면 에이전트가 APIYI를 통합하는 데 필요한 모든 것을 얻습니다.
  </Step>

  <Step title="필요한 페이지를 찾아보십시오">
    `llms.txt`는 이 사이트의 모든 페이지 색인입니다. 에이전트는 관련된 페이지를 선택하고 `.md`를 덧붙여 일반 텍스트로 읽습니다.
  </Step>

  <Step title="코드를 작성하고 실제로 실행하십시오">
    문서의 Python을 그대로 복사하지 않고, 프로젝트의 기존 언어와 종속성에 맞춰 예제를 작성합니다. 호출이 성공해야 비로소 완료됩니다.
  </Step>
</Steps>

### AI에 전달할 수 있는 네 가지 진입점

| 진입점              | URL                               | 사용 시기                                        |
| ---------------- | --------------------------------- | -------------------------------------------- |
| **스킬**           | `https://docs.apiyi.com/skill.md` | 에이전트에게 완전한 통합 참고 자료를 한 번에 전달합니다. 여기서 시작하십시오. |
| **페이지 색인**       | `https://docs.apiyi.com/llms.txt` | 에이전트가 필요한 페이지를 결정하게 하십시오                     |
| **단일 페이지를 텍스트로** | 문서 URL에 `.md`를 덧붙이십시오             | 한 페이지만 필요할 때 — HTML보다 저렴합니다                  |
| **MCP 서버**       | `https://docs.apiyi.com/mcp`      | 이 문서 사이트를 MCP 서버로 연결하여 에이전트가 실시간으로 검색하게 하십시오 |

<Tip>
  예를 들어, 이 페이지의 일반 텍스트 버전은 `https://docs.apiyi.com/en/getting-started.md`입니다. 이 사이트의 모든 페이지는 이 접미사를 지원합니다.
</Tip>

### 어떤 페이지든 바로 AI에 보내기

모든 문서 페이지의 오른쪽 상단에는 **페이지 복사** 버튼이 있습니다. 그 옆의 화살표를 열면 더 많은 옵션이 표시됩니다:

<img src="https://mintcdn.com/apiyillc/pSJvB-WdRHZF62ww/images/contextual-menu-copy-page.png?fit=max&auto=format&n=pSJvB-WdRHZF62ww&q=85&s=34ae33d4f4555c0b9436364f12bab88b" alt="문서 페이지 오른쪽 상단의 페이지 복사 메뉴로, 페이지를 복사하고, Markdown으로 보기를 하며, ChatGPT, Claude, Perplexity 또는 Google AI Studio에서 여는 옵션이 있습니다" width="648" height="694" data-path="images/contextual-menu-copy-page.png" />

| 옵션                        | 수행 내용                                                 |
| ------------------------- | ----------------------------------------------------- |
| **페이지 복사**                | 현재 페이지를 Markdown으로 클립보드에 복사하여, 어떤 AI에도 바로 붙여넣을 수 있습니다 |
| **Markdown으로 보기**         | 일반 텍스트 버전을 브라우저에서 엽니다 — 확인하거나 링크를 공유할 때 유용합니다         |
| **ChatGPT에서 열기**          | 이 페이지를 컨텍스트로 포함해 ChatGPT로 이동합니다                       |
| **Claude에서 열기**           | Claude도 마찬가지입니다                                       |
| **Perplexity에서 열기**       | Perplexity도 마찬가지입니다                                   |
| **Google AI Studio에서 열기** | Google AI Studio도 마찬가지입니다                             |

특정 모델에서 문제가 생기면 가장 빠른 방법은 해당 모델의 페이지를 열고 **페이지 복사**를 눌러 오류와 함께 AI에 보내는 것입니다.
