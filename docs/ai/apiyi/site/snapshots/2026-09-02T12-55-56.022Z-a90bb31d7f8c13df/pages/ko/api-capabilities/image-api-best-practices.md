...
| -------------------------------------------------------------------------- | -------------------------------------------- | ----------------------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| [GPT-Image-2 (Official)](/ko/api-capabilities/gpt-image-2/overview)        | `/v1/images/generations`, `/v1/images/edits` | **360초** (고해상도 + 2K/4K는 3\~5분 측정)         | Raw b64\_json (**`data:` 접두사 없음**)                                                  | ❌ 지원하지 않음 (`response_format`는 400을 반환하며, `image2_OSS` 그룹은 아직 공식 채널을 포함하지 않습니다) |
| [GPT-Image-2-All (Reverse)](/ko/api-capabilities/gpt-image-2-all/overview) | 위와 동일                                        | **300초**                                  | 기본 `b64_json` (**`data:` 접두사 없음**, 2026-07 검증); 명시적 `response_format: "url"`로 전환 가능 | ✅ 명시적 `url`: R2 CDN, 약 24시간; URL에 의존한다면 `image2_OSS` 그룹을 사용하십시오                |
| [GPT-Image-2-VIP](/ko/api-capabilities/gpt-image-2-vip/overview)           | 위와 동일                                        | **300초**                                  | All과 동일(기본 `b64_json`, 접두사 없음, 2026-07 검증)                                          | ✅ All과 동일(명시적 `url` 또는 `image2_OSS` 그룹)                                        |
| [Nano Banana Pro](/ko/api-capabilities/nano-banana-image/overview)         | 네이티브 Gemini `:generateContent`               | 1K/2K **300초**, 4K **600초**, 멀티 이미지 5분 이상 | `inlineData.data`의 Raw base64                                                       | `NB_OSS` 베타 그룹(아래 참조)                                                          |
| [Nano Banana 2](/ko/api-capabilities/nano-banana-2-image/overview)         | 위와 동일                                        | **360초**                                  | 위와 동일                                                                               | `NB_OSS` 적용 범위: 지원팀에 문의하십시오                                                    |
| [Nano Banana Lite](/ko/api-capabilities/nano-banana-lite-image/overview)   | 위와 동일                                        | **300초**(\~4초가 일반적이며, 피크 혼잡을 위한 여유가 있습니다) | 위와 동일                                                                               | `NB_OSS` 적용 범위: 지원팀에 문의하십시오                                                    |
| [FLUX](/ko/api-capabilities/flux/overview)                                 | `/v1/images/generations`                     | **60-120초**, flex 모델은 180초                | `data[0].url`만 (**URL은 업스트림 기본값입니다**)                                               | ⚠️ 약 **10분** 동안만 유효하며, CORS 없음 — 서버 측에서 즉시 재호스팅하십시오                            |
| [Seedream](/ko/api-capabilities/seedream-image/overview)                   | `/v1/images/generations`(통합 생성/편집 엔드포인트)     | **60초**(4K + hd는 약 30\~60초)               | 기본 `url`(**URL은 업스트림 기본값입니다**); 선택적 `b64_json`(Raw base64, 접두사 없음)                  | ✅ BytePlus TOS, 약 24시간                                                         |

<Tip>
  `response_format`는 **적용 범위가 좁습니다**: GPT-Image-2-All / VIP와 Seedream만 이를 허용하며, 공식 GPT-Image-2 채널은 이를 전달하면 400 `unknown_parameter`을 반환합니다. 지원되는 경우에는 기본값에 의존하지 말고 **항상 명시적으로 전달하십시오** — 기본값은 과거에 그룹과 부하 조건에 따라 달라진 적이 있습니다.
</Tip>
...
* 2K/4K 해상도의 `high` 품질 GPT-Image-2는 처음부터 끝까지 **3-5분**이 걸립니다;
...
| 실패                           | 과금됨?                  | 권장 사항                                          |
| ---------------------------- | --------------------- | ---------------------------------------------- |
| 429 / 503 (요청 제한, 상위 서버 과부하) | 과금되지 않음               | 지수 백오프로 재시도하십시오(예: 5초, 15초, 45초)               |
| 클라이언트 타임아웃 / 조기 연결 종료        | **과금됨**               | 먼저 타임아웃을 늘리십시오. 꼭 재시도해야 한다면 시도 횟수를 신중하게 제한하십시오 |
| 400 / 403 (매개변수 또는 권한 오류)    | 과금되지 않음               | 다시 보내기 전에 요청을 수정하십시오. 무작정 재시도는 무의미합니다          |
| 콘텐츠 모더레이션 차단                 | **모델에 따라 다름** (아래 참고) | prompt를 수정하십시오. 그대로 다시 보내면 대부분 다시 차단됩니다        |

<Info>
  **모더레이션 차단 과금은 모델에 따라 다릅니다**: token 과금 모델(공식 GPT-Image-2 등)은 모더레이션이 트리거되면 보통 400 오류를 반환하며 — **과금되지 않습니다**. 오직 **이미지별 과금되는 Nano Banana Pro**만 Google 측의 “HTTP 200이지만 생성 실패” 차단에 걸리며, 해당 호출은 **과금됩니다** — APIYI는 이러한 사용자 과실이 아닌 실패를 [실패한 생성 크레딧 환급 계획](/ko/api-capabilities/nano-banana-pro-guarantee)으로 보상하며, 이미지별 집계를 기준으로 크레딧을 환급합니다.
</Info>
