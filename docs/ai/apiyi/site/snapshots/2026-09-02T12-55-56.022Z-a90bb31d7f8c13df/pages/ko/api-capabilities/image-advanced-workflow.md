...
## 5. 파이프라인 설계를 바꾸게 될 사실

이것들을 미리 알면 재작업을 한 번 줄일 수 있습니다:

| 사실                                                                                                                                                            | 파이프라인에 미치는 영향                                                                                                                                                      |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **이 제품군 전반에서 Seed는 사실상 사용할 수 없습니다**: Nano Banana나 GPT-Image 모델 중 어느 것도 seed를 노출하지 않으며, Seedream 4.x / 5.x의 seed는 효과가 없는 것으로 측정되었고, Grok Imagine은 이를 지원하지 않습니다 | 시드에 의존해 재현성을 설계하지 마십시오. **유일하게 신뢰할 수 있는 재현 방법은 성공한 전체 요청을 보관하는 것**입니다 — prompt, 참조 이미지, 모든 파라미터를 그대로 기록하고 동일하게 재실행하십시오                                             |
| **서버 측 `n` 파라미터는 대부분의 이미지 모델에서 효과가 없습니다**                                                                                                                     | 여러 후보는 클라이언트 측 동시 실행 수에서 나와야 합니다. 채널별 동시 실행 수를 부하 테스트하고 지수 백오프를 추가하십시오                                                                                             |
| **모든 이미지 API는 동기식이며, task ID가 없고, 연결이 끊겨도 과금됩니다**                                                                                                             | 파이프라인은 자체 작업 큐가 필요합니다. [Build Your Own Async Queue](/ko/api-capabilities/image-async-queue)와 [Image API 핵심](/ko/api-capabilities/image-api-best-practices)을 참조하십시오 |
| **업스케일링, 배경 제거, 얼굴 복원 엔드포인트가 없습니다**                                                                                                                           | 출력 크기는 생성 시점에 확정하십시오. 투명 배경이 필요하면 `background: "transparent"`을 `gpt-image-2`에 전달하십시오(컷아웃 엔드포인트가 아니라 투명도 위에서 직접 생성합니다)                                              |
| **이미지 없이 HTTP 200이 반환되면 대개 모더레이션 차단을 의미합니다**                                                                                                                  | 선택 로직은 “이미지가 반환되지 않음”과 “이미지는 반환되었지만 품질이 나쁨”을 구분해야 합니다. 전자의 경우 [Gemini Image Error Handling](/ko/api-capabilities/gemini-image-error-handling)을 참조하십시오              |
| **반환되는 모든 이미지 URL은 임시 링크입니다**                                                                                                                                 | 받는 즉시 재호스팅하십시오. 상위 URL을 데이터베이스에 절대 저장하지 마십시오                                                                                                                       |
