...
| --------------------------- | -------------------- | ------------------------ |
| 모델 가중치                      | 동일                   | 동일                       |
| System prompt               | 벤더가 주입함(공개되지 않음)     | 없음 — 직접 작성해야 합니다         |
| 웹 검색                        | 내장되어 있으며 자동으로 트리거됩니다 | 도구를 활성화하거나 검색을 직접 연결합니다  |
| 수학 / 코드 실행                  | 내장 샌드박스              | 도구 호출을 직접 구현합니다          |
| 파일 및 이미지 파싱                 | 내장됨                  | 직접 업로드하거나 Base64로 인코딩합니다 |
| 대화 메모리                      | 자동으로 저장됩니다           | 무상태 — 히스토리를 직접 전달해야 합니다  |
| 컨텍스트 윈도우 초과                 | 자동으로 압축됩니다           | 직접 잘라내거나 요약해야 합니다        |
| 파라미터(temperature, thinking) | 벤더가 조정함              | 기본값 — 직접 맞춰야 합니다         |
| 출력 형식                       | 프런트엔드가 렌더링함          | 일반 텍스트 / JSON            |
