...
> Package Nano Banana Pro (gemini-3-pro-image) as a ready-to-use Agent Skill. Drop it into Codex, OpenClaw, hermes-agent, Claude Code, or any coding Agent and call APIYI for text-to-image and image editing with a single prompt.
...
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "A shiba inu wearing an astronaut helmet, cinematic lighting" -o dog.png --size 2K --aspect 16:9

# Image editing (one image)
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "Replace the background with a cyberpunk night city" -i input.jpg -o edited.png

# Multi-image compositing (repeat -i)
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "Composite these two people into one office group photo" -i a.png -i b.png -o merged.png

# Several variants of the same prompt at once (max 5, generated concurrently)
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "Chinese ink landscape illustration" -o landscape.png -n 3 --aspect 16:9
```
...
<Info>
  Nano Banana Pro has strict content-safety controls. If the script reports a `finishReason` other than `STOP` or returns a rejection text, adjust your content accordingly and do not repeatedly retry the same violating prompt. See the [error handling guide on the generation reference](/en/api-capabilities/nano-banana-image/overview).
</Info>

## Related docs

* [Nano Banana Pro Image Generation overview](/en/api-capabilities/nano-banana-image/overview)
* [Text-to-Image API reference](/en/api-capabilities/nano-banana-image/text-to-image)
* [Image Editing API reference](/en/api-capabilities/nano-banana-image/image-edit)
