...
> Grok 4.5 on APIYI: $2 input / $6 output per 1M tokens, 500,000 context, available in 4 billing groups.

Co-trained with Cursor: tops agentic tool-calling benchmarks with roughly 4.2× the output-token efficiency of Opus 4.8.
...
* Above 204,800 tokens: \$4/1M input
...
The example below calls `grok-4.5` through OpenAI Chat Completions (`/v1/chat/completions`). Point base\_url at `https://api.apiyi.com/v1` — everything else matches the official API.
...
<CardGroup cols={2}>
  <Card title="Launch announcement" icon="megaphone" href="/en/news/grok-4-5-launch">
    Background, benchmarks and migration notes for Grok 4.5
  </Card>
