...
> GPT-5.6 Sol on APIYI: $5 input / $30 output per 1M tokens, 1,000,000 context, available in 4 billing groups.

The flagship tier of the GPT-5.6 family: 88.8% on Terminal-Bench 2.1 and 90.4% on BrowseComp; the `gpt-5.6` alias points here.
...
* Above 272,000 tokens: \$10/1M input
...
| `CodexReverse`   | 0.7×       | 70% of list price |
...
The example below calls `gpt-5.6-sol` through OpenAI Chat Completions (`/v1/chat/completions`). Point base\_url at `https://api.apiyi.com/v1` — everything else matches the official API.
...
<CardGroup cols={2}>
  <Card title="Launch announcement" icon="megaphone" href="/en/news/gpt-5-6-launch">
    Background, benchmarks and migration notes for GPT-5.6 Sol
  </Card>
