...
> GPT-5.6 Luna on APIYI: $0.2 input / $1.2 output per 1M tokens, 1,000,000 context, available in 3 billing groups.

The lightweight tier of the GPT-5.6 family, built for high concurrency and cost-sensitive workloads; weaker beyond 256K context, so chunk long documents.
...
* Above 272,000 tokens: \$0.4/1M input
...
The example below calls `gpt-5.6-luna` through OpenAI Chat Completions (`/v1/chat/completions`). Point base\_url at `https://api.apiyi.com/v1` — everything else matches the official API.
...
<CardGroup cols={2}>
  <Card title="Launch announcement" icon="megaphone" href="/en/news/gpt-5-6-launch">
    Background, benchmarks and migration notes for GPT-5.6 Luna
  </Card>
  ...
  <Card title="GPT-5.6 Terra" icon="git-compare" href="/en/models/gpt-5-6-terra">
    Details for a model in the same family
  </Card>
