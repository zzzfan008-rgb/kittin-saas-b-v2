...
> 把 Nano Banana 2（gemini-3.1-flash-image-preview）封装成开箱即用的 Agent Skill，丢进 Codex、OpenClaw、hermes-agent、Claude Code 等任意编码 Agent，一句话即可调用 API易 平台完成文生图与图片编辑。

<Note>
  本页提供一个**开箱即用的 Agent 技能（Skill）**：把它放进你正在用的编码 Agent，就能用自然语言（或显式命令）直接调用 API易 平台的 **Nano Banana 2**（`gemini-3.1-flash-image-preview`）完成生图与改图。整套东西就两个文件，复制即用。
</Note>

<Tip>
  如果你想要的是 Nano Banana **Pro**（`gemini-3-pro-image`，极致画质）的技能，见 [Nano Banana Pro Agent 技能](/api-capabilities/nano-banana-image/skills)。本页是 **Nano Banana 2**（Pro 级画质 + Flash 级速度，性价比更高）。
</Tip>
...
<CardGroup cols={3}>
  <Card title="文生图" icon="wand-sparkles">
    只给提示词 → 生成全新图片，支持 **14 种宽高比**与 512/1K/2K/4K 分辨率。
  </Card>
...
相比 Pro，Nano Banana 2 多了 `1:4 / 4:1 / 1:8 / 8:1` 四种**超长/超宽比例**和独有的 **512px** 低分辨率档，按次仅 \$0.055/张、按量低至约 \$0.025/张，更适合走量。
...
在 `nano-banana-2/.env` 里写上你的 **API易 API Key**（在 `api.apiyi.com` 控制台创建）：
...
<Tip>
  `name` 必须是小写字母 + 连字符，且**不能含 `claude` / `anthropic`** 等保留词。在支持斜杠命令的 Agent 里，目录名就是命令名——叫 `nano-banana-2` 即 `/nano-banana-2`。`${CLAUDE_SKILL_DIR}` 是 Claude Code 提供的技能目录变量；其他 Agent 直接用脚本的实际路径即可。
</Tip>
...
<Tip>
  **怎么控制比例和清晰度**：用 `--aspect` 选宽高比（14 选 1，含 Pro 没有的 `1:4 / 4:1 / 1:8 / 8:1` 超长超宽），用 `--size` 选分辨率（`512` / `1K` / `2K` / `4K`，`512` 是 Nano Banana 2 独有的省钱低分档）。直接说"竖屏 9:16""出 4K""来个超宽横幅"，Agent 也会自动带上这两个参数。
</Tip>
...
<Info>
  Nano Banana 2 有严格的内容安全管控。若脚本提示 `finishReason` 非 `STOP` 或返回的是拒绝说明文本，请按提示调整内容，不要对同一违规提示词反复重试。
</Info>

## 相关文档

* [Nano Banana 2 图片生成总览](/api-capabilities/nano-banana-2-image/overview)
