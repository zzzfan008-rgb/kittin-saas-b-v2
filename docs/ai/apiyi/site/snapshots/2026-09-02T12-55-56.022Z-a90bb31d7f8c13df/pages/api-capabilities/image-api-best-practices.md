...
| --------------------------------------------------------------------- | ------------------------------------------- | ---------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------- |
| [GPT-Image-2（官转）](/api-capabilities/gpt-image-2/overview)             | `/v1/images/generations`、`/v1/images/edits` | **360 秒**（high + 2K/4K 实测 3-5 分钟）        | 纯 b64\_json（**无 `data:` 前缀**）                                              | ❌ 不支持（传 `response_format` 直接 400；`image2_OSS` 分组暂不支持官转） |
