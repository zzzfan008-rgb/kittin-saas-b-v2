...
> Nano Banana 2 Lite 文生图 API 参考与在线调试 — 输入文本描述生成图片

<Info>
  右侧的交互式 Playground 支持下拉选择参数（宽高比、响应类型等）。请在 **Authorization** 中填入你的 API Key（格式：`Bearer sk-xxx`），即可一键发送请求测试。
</Info>

<Tip>
  **场景说明**：本页用于「文本生成图片」。只需输入提示词即可，无需上传任何图片。如需根据现有图片做编辑，请使用 [图片编辑接口](/api-capabilities/nano-banana-lite-image/image-edit)。
</Tip>
...
<Warning>
  ...
  * Nano Banana 2 Lite 专注 1K 画布，响应体量适中，但仍建议本地运行以稳妥保存图片。
</Warning>
...
<Tip>
  详细的参数文档、可选值和默认值请查看右侧 Playground 中的字段说明，所有 enum 类型字段（如 `aspectRatio`）都支持下拉选择，无需手动输入。
</Tip>

<Info>
  **从 Nano Banana 2 迁移**：只需把模型名从 `gemini-3.1-flash-image` 改为 `gemini-3.1-flash-lite-image`，其它参数保持不变。注意 Lite 仅支持 `1K` 分辨率，如传入 `2K/4K` 请改回 `1K`。
</Info>
