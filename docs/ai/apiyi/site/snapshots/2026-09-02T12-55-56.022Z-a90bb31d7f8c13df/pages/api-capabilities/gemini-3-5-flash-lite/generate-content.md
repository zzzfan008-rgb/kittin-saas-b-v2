...
# Gemini 3.5 Flash-Lite 原生 API 参考

> Gemini 3.5 Flash-Lite 原生 generateContent API 参考与在线调试：官方格式直连，默认零思考极速响应，搜索 grounding 等工具支持。

<Info>
  右侧 Playground 可直接调试：在 **x-goog-api-key** 填 `sk-your-api-key`（API易令牌，无需 Google Key），点击发送即可看到响应——默认零思考，速度很快。
</Info>

<Tip>
  模型**默认不输出思考**；需要深度推理传 `thinkingConfig: {"thinkingLevel": "high"}`（实测仅 high 档稳定触发）。搜索 grounding、代码执行、URL context、Maps 等工具仅本原生端点支持。模型能力、定价详见 [Gemini 3.5 Flash-Lite 概览](/api-capabilities/gemini-3-5-flash-lite/overview)。
</Tip>
...
| `generationConfig.thinkingConfig.thinkingLevel`        | string |    | 默认不思考；`high` 实测触发约 1000 思考 tokens                                                          |
| `generationConfig.thinkingConfig.includeThoughts`      | bool   |    | 回显思考过程（配合 high 档）                                                                          |
