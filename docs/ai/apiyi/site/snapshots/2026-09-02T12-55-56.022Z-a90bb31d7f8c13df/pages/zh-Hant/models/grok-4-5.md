...
> Grok 4.5 模型詳情：輸入 $2 / 輸出 $6 每 1M tokens，500,000 上下文，4 個分組可用。

與 Cursor 聯合訓練，Agentic 工具呼叫單項登頂，輸出 token 效率約 Opus 4.8 的 4.2 倍。
...
以下示例通過 OpenAI Chat Completions（`/v1/chat/completions`）呼叫 `grok-4.5`。base\_url 換成 `https://api.apiyi.com/v1` 即可，其餘用法與官方一致。
...
<CardGroup cols={2}>
  <Card title="上線公告" icon="megaphone" href="/news/grok-4-5-launch">
    Grok 4.5 的釋出背景、實測表現與遷移建議
  </Card>
