...
<Tip>
  **同價姐妹模型**：[`gpt-image-2-vip`](/zh-Hant/api-capabilities/gpt-image-2-vip/overview)（逆向 Codex 線路）同樣 \$0.03/張，支援 30 檔常見 size（含 4K），呼叫方式與本模型完全一致——需要鎖定輸出尺寸時切過去即可。
</Tip>
...
<Warning>
  本模型為**自適應輸出尺寸**，不等同於官轉 `gpt-image-2` API。需要嚴格鎖定輸出尺寸或 4K 時，請改用 [`gpt-image-2-vip`](/zh-Hant/api-capabilities/gpt-image-2-vip/overview)（逆向 Codex 線路，30 檔 size 含 4K）；需要官方完全一致字段時，請使用 [`gpt-image-2`](/zh-Hant/api-capabilities/gpt-image-2/overview)（官轉）。
</Warning>
