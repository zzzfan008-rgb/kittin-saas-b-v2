...
> Seed 2.1 Turbo 模型詳情：輸入 $0.5 / 輸出 $2.5 每 1M tokens，256,000 上下文，3 個分組可用。

高頻生產型文本模型，雙端點 15/15 實測通過，思考開關可控 + 隱式/顯式雙層快取。

## 規格

| 屬性        | 值                            |
| --------- | ---------------------------- |
| **模型 ID** | `dola-seed-2-1-turbo-260628` |
| **廠商**    | 字節跳動                         |
| **廠商釋出日** | 2026-06-23                   |
| **上線日期**  | 2026-07-21                   |
| **知識截止**  | 未公開                          |
...
以下示例通過 OpenAI Chat Completions（`/v1/chat/completions`）呼叫 `dola-seed-2-1-turbo-260628`。base\_url 換成 `https://api.apiyi.com/v1` 即可，其餘用法與官方一致。
...
<CardGroup cols={2}>
  <Card title="上線公告" icon="megaphone" href="/news/dola-seed-2-1-turbo-launch">
    Seed 2.1 Turbo 的釋出背景、實測表現與遷移建議
  </Card>

  <Card title="概覽" icon="book-open" href="/zh-Hant/api-capabilities/dola-seed-2-1-turbo/overview">
    引數、呼叫方式與最佳實踐
  </Card>
