...
> Grok 4.3 模型詳情：輸入 $1.25 / 輸出 $2.5 每 1M tokens，1,000,000 上下文，4 個分組可用。

推理常開不可關閉，1M 上下文，輸出速度約 159 tokens/s。
...
* 0 – 204,800 tokens 輸入 \$1.25/1M
* 超過 204,800 tokens 輸入 \$2.5/1M
...
以下示例通過 OpenAI Chat Completions（`/v1/chat/completions`）呼叫 `grok-4.3`。base\_url 換成 `https://api.apiyi.com/v1` 即可，其餘用法與官方一致。
...
<CardGroup cols={2}>
  <Card title="上線公告" icon="megaphone" href="/news/grok-4-3-launch">
    Grok 4.3 的釋出背景、實測表現與遷移建議
  </Card>
  ...
  <Card title="Grok 4.6" icon="git-compare" href="/zh-Hant/models/grok-4-6">
    同系列模型詳情
  </Card>
