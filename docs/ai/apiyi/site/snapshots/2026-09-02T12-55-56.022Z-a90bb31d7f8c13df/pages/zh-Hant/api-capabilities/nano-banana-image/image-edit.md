...
> Nano Banana Pro 圖片編輯 API 參考與線上除錯 — 輸入圖片 + 指令生成新圖片
...
<Tip>
  **場景說明**：本頁用於「圖片編輯」，必須上傳一張待編輯的圖片（base64 編碼）+ 編輯指令。如果只想根據文本生成新圖片，請使用 [文生圖介面](/zh-Hant/api-capabilities/nano-banana-image/text-to-image)。
</Tip>
...
<Warning>
  ...
  * 如要在瀏覽器裡試 Playground，**用極小的參考圖（\< 50KB）** 並把 `imageSize` 設為最小檔（如 `1K`）。
</Warning>

<Warning>
  **⚠️ `parts` 陣列結構（重要，多圖編輯必看）**

  每個 `part` **只能是 `text` 或 `inlineData` 中的一個**，二者不能同時出現在同一個 part 裡。這與谷歌官方 `gemini-3-pro-image-preview` 的契約一致。
...
<Tip>
  Nano Banana Pro 支援多圖編輯：可以同時輸入多張圖片和編輯指令，實現多圖合成、風格遷移等高階編輯功能。
</Tip>


## OpenAPI

````yaml api-reference/nano-banana-pro-edit-openapi.yaml POST /v1beta/models/gemini-3-pro-image-preview:generateContent
openapi: 3.1.0
info:
  title: Nano Banana Pro 图片编辑 API
  description: |
    Google 图像生成模型 Nano Banana Pro（gemini-3-pro-image-preview）— 图片编辑接口。
...
paths:
  /v1beta/models/gemini-3-pro-image-preview:generateContent:
    post:
      ...
      description: |
        使用 Nano Banana Pro 模型，根据文本指令对输入图片进行编辑。

        - 必须提供输入图片（`inlineData`，base64 编码）
        - 文本（`text`）描述编辑指令
        - 支持 4K 高清编辑、局部精细编辑、文本渲染
        - 如需「文生图」请使用 [文生图接口](/api-capabilities/nano-banana-image/text-to-image)
      operationId: editNanoBananaProImage
      requestBody:
        required: true
        content:
          application/json:
