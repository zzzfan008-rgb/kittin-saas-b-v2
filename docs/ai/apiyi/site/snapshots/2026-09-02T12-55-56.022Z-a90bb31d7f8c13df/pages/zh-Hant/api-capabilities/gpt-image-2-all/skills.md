...
> 在 Agent 裡用 gpt-image-2-all（官逆·ChatGPT 線，最快、flat $0.03/張）出圖——與 gpt-image-2、gpt-image-2-vip 共用同一個 gpt-image-2 技能，只需把 --model 設為 gpt-image-2-all。
...
裝好 [gpt-image-2 系列技能](/zh-Hant/api-capabilities/gpt-image-2/skills) 後，把 `--model` 設為 `gpt-image-2-all` 即可：

```bash theme={null}
python3 gpt-image-2/scripts/gpt_image.py "扁平插畫風節日海報，豎版 2:3" -o poster.png --model gpt-image-2-all
```
