...
> Nano Banana 2 文生圖 API 參考與線上除錯 — 輸入文本描述生成圖片
...
<Tip>
  **場景說明**：本頁用於「文本生成圖片」。只需輸入提示詞即可，無需上傳任何圖片。如需根據現有圖片做編輯，請使用 [圖片編輯介面](/zh-Hant/api-capabilities/nano-banana-2-image/image-edit)。
</Tip>
...
<Warning>
  ...
  * 如要在瀏覽器裡試 Playground，把 `imageSize` 設為最小檔（如 `512` / `1K`），縮小響應體積。
</Warning>
...
| `generationConfig.thinkingConfig.thinkingLevel`   | string  | 否  | `minimal`（快速）/ `High`（深度推理），預設 `minimal` |
| `generationConfig.thinkingConfig.includeThoughts` | boolean | 否  | 是否返回思維過程文本，預設 `false`                    |

<Tip>
  詳細的引數文件、可選值和預設值請檢視右側 Playground 中的欄位說明，所有 enum 型別欄位（如 `aspectRatio`、`imageSize`、`thinkingLevel`）都支援下拉選擇，無需手動輸入。
</Tip>
...
<Warning>
  ...
  * **Image Search Grounding**（圖片搜尋增強，Nano Banana 2 獨有）：通過谷歌圖片搜尋獲取視覺上下文

  這些功能需要額外的谷歌搜尋 API 費用，暫不納入 API易 轉發範圍。
</Warning>


## OpenAPI

````yaml api-reference/nano-banana-2-generate-openapi.yaml POST /v1beta/models/gemini-3.1-flash-image-preview:generateContent
openapi: 3.1.0
info:
  title: Nano Banana 2 文生图 API
  description: |
    Google 最新图像生成模型 Nano Banana 2（gemini-3.1-flash-image-preview）— 文生图接口。
...
paths:
  /v1beta/models/gemini-3.1-flash-image-preview:generateContent:
    post:
      ...
      description: |
        使用 Nano Banana 2 模型，根据文本提示词生成图片。

        - 仅需要输入提示词（`text`）和生成配置
        - 支持 14 种宽高比、4 档分辨率（512px / 1K / 2K / 4K）
        - 如需「图片编辑」请使用 [图片编辑接口](/api-capabilities/nano-banana-2-image/image-edit)
      operationId: generateNanoBanana2TextToImage
      requestBody:
        required: true
        content:
          application/json:
