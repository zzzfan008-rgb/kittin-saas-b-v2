...
> 把 Nano Banana 2 Lite（gemini-3.1-flash-lite-image）封裝成開箱即用的 Agent Skill，丟進 Codex、OpenClaw、hermes-agent、Claude Code 等任意編碼 Agent，一句話即可呼叫 API易 平臺完成文生圖與圖片編輯。

<Note>
  本頁提供一個**開箱即用的 Agent 技能（Skill）**：把它放進你正在用的編碼 Agent，就能用自然語言（或顯式命令）直接呼叫 API易 平臺的 **Nano Banana 2 Lite**（`gemini-3.1-flash-lite-image`）完成生圖與改圖。整套東西就兩個檔案，複製即用。
</Note>

<Tip>
  如果你需要更高畫質或 2K/4K，見 [Nano Banana 2 Agent 技能](/zh-Hant/api-capabilities/nano-banana-2-image/skills)（最高 4K）或 [Nano Banana Pro Agent 技能](/zh-Hant/api-capabilities/nano-banana-image/skills)（極致畫質）。本頁是 **Lite**（最快最省，專注 1K，按次 \$0.025/次）。
</Tip>
...
Nano Banana 2 Lite 主打**速度與成本**：約 4 秒出圖、比 Nano Banana 2 快約 2.7 倍，按量實測約 \$0.018/次（官網 4 折費率）、按次 \$0.025/張，最適合高併發、快速迭代、走量場景。
...
在 `nano-banana-lite/.env` 裡寫上你的 **API易 API Key**（在 `api.apiyi.com` 控制台建立）：
...
# Nano Banana Lite 出圖技能

通過 API易 平臺呼叫 Nano Banana 2 Lite（`gemini-3.1-flash-lite-image`）生成或編輯圖片。約 4 秒出圖、專注 1K 畫布，價效比高、適合走量。
...
```bash
# 文生圖（默認出 1 張）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana_lite.py "一隻戴著宇航頭盔的柴犬，電影感打光" -o dog.png --aspect 16:9

# 超寬橫幅（8:1 / 4:1 等超寬比例）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana_lite.py "國風山水長卷橫幅" -o banner.png --aspect 8:1

# 圖片編輯（傳 1 張）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana_lite.py "把背景換成賽博朋克城市夜景" -i input.jpg -o edited.png

# 多圖合成（傳多張，重複 -i）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana_lite.py "把這兩個人合到同一張辦公室合影裡" -i a.png -i b.png -o merged.png

# 一次出多張同主題變體（最多 5 張，併發生成）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana_lite.py "國風山水插畫" -o shanshui.png -n 3 --aspect 16:9
```
...
- `--size`：解析度僅支援 `1K`（Lite 專注 1K 畫布），預設 `1K`。
...
<Tip>
  `name` 必須是小寫字母 + 連字元，且**不能含 `claude` / `anthropic`** 等保留詞。在支援斜槓命令的 Agent 裡，目錄名就是命令名——叫 `nano-banana-lite` 即 `/nano-banana-lite`。`${CLAUDE_SKILL_DIR}` 是 Claude Code 提供的技能目錄變數；其他 Agent 直接用指令碼的實際路徑即可。
</Tip>
...
```python theme={null}
#!/usr/bin/env python3
"""通過 API易 呼叫 Nano Banana 2 Lite（gemini-3.1-flash-lite-image）生成 / 編輯圖片。純標準庫，零依賴。"""
import argparse
import base64
import json
import os
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Nano Banana Lite 出圖")
    ...
    parser.add_argument("--size", default="1K", help="解析度僅支援 1K（Lite 專注 1K 畫布）")
...
| 你說                                    | 技能行為                            |
| ------------------------------------- | ------------------------------- |
| "用 nano banana lite 畫一張 16:9 的雪山日出海報" | 調指令碼（無 `-i`），出 1 張 png          |
...
* **支援斜槓命令的 Agent**（如 Claude Code）：
  ...
  ```text theme={null}
  /nano-banana-lite 一隻在花園裡打盹的橘貓，油畫風格 --aspect 3:2
  ```

* **任意 Agent / 直接命令它跑指令碼**（最通用）：

  ```text theme={null}
  執行 python3 nano-banana-lite/scripts/nano_banana_lite.py "一隻在花園裡打盹的橘貓，油畫風格" --aspect 3:2
  ```
...
<Info>
  Nano Banana 2 Lite 有嚴格的內容安全管控。若指令碼提示 `finishReason` 非 `STOP` 或返回的是拒絕說明文本，請按提示調整內容，不要對同一違規提示詞反覆重試。
</Info>

## 相關文件

* [Nano Banana Lite 圖片生成總覽](/zh-Hant/api-capabilities/nano-banana-lite-image/overview)
* [文生圖 API 參考](/zh-Hant/api-capabilities/nano-banana-lite-image/text-to-image)
* [圖片編輯 API 參考](/zh-Hant/api-capabilities/nano-banana-lite-image/image-edit)
* [Nano Banana 2 Agent 技能](/zh-Hant/api-capabilities/nano-banana-2-image/skills)
