...
| 推理型模型（thinking / reasoning） | 流式更友好，但**不解決超時** | 思考階段可能長時間不吐 token，見下方誤區①                                                                   |
...
<AccordionGroup>
  <Accordion title="誤區①：開了流式就不會超時了">
    **不成立。** 流式只是讓"第一個 token"來得早，它不縮短總生成時間，也不保證中途一直有資料。

    推理型模型（`gemini-3.1-pro-preview`、`gpt-5.6-sol`、`gpt-5.5-pro` 等）在**思考階段可能長時間不吐任何 token**，客戶端的 read timeout 一樣會被觸發。

    正確做法是按場景分檔設定 timeout，見 [如何避免介面超時](/zh-Hant/faq/timeout-configuration)。
  </Accordion>
