...
> GPT-5.6 Sol 模型詳情：輸入 $5 / 輸出 $30 每 1M tokens，1,000,000 上下文，4 個分組可用。

GPT-5.6 系列旗艦檔，Terminal-Bench 2.1 88.8%、BrowseComp 90.4%，`gpt-5.6` 別名指向它。
...
* 0 – 272,000 tokens 輸入 \$5/1M
* 超過 272,000 tokens 輸入 \$10/1M
...
以下示例通過 OpenAI Chat Completions（`/v1/chat/completions`）呼叫 `gpt-5.6-sol`。base\_url 換成 `https://api.apiyi.com/v1` 即可，其餘用法與官方一致。
...
<CardGroup cols={2}>
  ...
  <Card title="GPT-5.6 Luna" icon="git-compare" href="/zh-Hant/models/gpt-5-6-luna">
    同系列模型詳情
  </Card>
