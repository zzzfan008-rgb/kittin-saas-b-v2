...
# Nano Banana Pro Agent 技能

> 把 Nano Banana Pro（gemini-3-pro-image）封裝成開箱即用的 Agent Skill，丟進 Codex、OpenClaw、hermes-agent、Claude Code 等任意編碼 Agent，一句話即可呼叫 API易 平臺完成文生圖與圖片編輯。

<Note>
  本頁提供一個**開箱即用的 Agent 技能（Skill）**：把它放進你正在用的編碼 Agent，就能用自然語言（或顯式命令）直接呼叫 API易 平臺的 **Nano Banana Pro**（`gemini-3-pro-image`）完成生圖與改圖。整套東西就兩個檔案，複製即用。
</Note>
...
<CardGroup cols={3}>
  <Card title="文生圖" icon="wand-sparkles">
    只給提示詞 → 生成全新圖片，支援 10 種寬高比與 1K/2K/4K 解析度。
  </Card>
...
在 `nano-banana-pro/.env` 裡寫上你的 **API易 API Key**（在 `api.apiyi.com` 控制台建立）：
...
* **支援技能自動發現的 Agent**（如 Claude Code）：把整個 `nano-banana-pro/` 目錄放進它的技能目錄——個人級 `~/.claude/skills/`，或專案級 `.claude/skills/`（隨倉庫共享）。
...
新建 `nano-banana-pro/SKILL.md`，完整內容如下（`description` 寫清「做什麼 + 何時用」，Agent 會據此自動觸發）：
...
# Nano Banana Pro 出圖技能

通過 API易 平臺呼叫 Nano Banana Pro（`gemini-3-pro-image`）生成或編輯圖片。
...
```bash
# 文生圖（默認出 1 張）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "一隻戴著宇航頭盔的柴犬，電影感打光" -o dog.png --size 2K --aspect 16:9

# 圖片編輯（傳 1 張）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "把背景換成賽博朋克城市夜景" -i input.jpg -o edited.png

# 多圖合成（傳多張，重複 -i）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "把這兩個人合到同一張辦公室合影裡" -i a.png -i b.png -o merged.png

# 一次出多張同主題變體（最多 5 張，併發生成）
python3 ${CLAUDE_SKILL_DIR}/scripts/nano_banana.py "國風山水插畫" -o shanshui.png -n 3 --aspect 16:9
```
...
- `--aspect`：寬高比，10 選 1（`1:1` `2:3` `3:2` `3:4` `4:3` `4:5` `5:4` `9:16` `16:9` `21:9`），預設 `1:1`。
- `--size`：解析度 `1K` / `2K` / `4K`，預設 `2K`。
...
<Tip>
  `name` 必須是小寫字母 + 連字元，且**不能含 `claude` / `anthropic`** 等保留詞。在支援斜槓命令的 Agent 裡，目錄名就是命令名——叫 `nano-banana-pro` 即 `/nano-banana-pro`。`${CLAUDE_SKILL_DIR}` 是 Claude Code 提供的技能目錄變數；其他 Agent 直接用指令碼的實際路徑即可。
</Tip>

## scripts/nano\_banana.py

新建 `nano-banana-pro/scripts/nano_banana.py`，使用 Gemini 原生格式（與本站文生圖 / 圖片編輯介面程式碼一致、已驗證可跑）。**純 Python 標準庫，無需 `pip install` 任何東西**：

```python theme={null}
#!/usr/bin/env python3
"""通過 API易 呼叫 Nano Banana Pro（gemini-3-pro-image）生成 / 編輯圖片。純標準庫，零依賴。"""
import argparse
import base64
import json
import os
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Nano Banana Pro 出圖")
    ...
    parser.add_argument("--aspect", default="1:1", help="寬高比，如 16:9")
    parser.add_argument("--size", default="2K", help="解析度 1K / 2K / 4K")
...
<Tip>
  模型名預設 `gemini-3-pro-image`（正式名）。如需用原 `gemini-3-pro-image-preview`，設環境變數 `APIYI_IMAGE_MODEL=gemini-3-pro-image-preview` 即可，兩者價格一致、效果等價。
</Tip>
...
| 你說                               | 技能行為                               |
| -------------------------------- | ---------------------------------- |
| "用 nano banana 畫一張 16:9 的雪山日出海報" | 調指令碼（無 `-i`），出 1 張 png             |
| "把 photo.jpg 的背景虛化，突出人物"         | 調指令碼 `-i photo.jpg`，出 1 張 png      |
| "國風山水來 3 張不同的"                   | 調指令碼 `-n 3`，併發出 3 張 png            |
| "把 a.png 和 b.png 裡的人合成一張合影"      | 調指令碼 `-i a.png -i b.png`，出 1 張 png |
...
* **支援斜槓命令的 Agent**（如 Claude Code）：
  ...
  ```text theme={null}
  /nano-banana-pro 一隻在花園裡打盹的橘貓，油畫風格 --size 4K --aspect 3:2
  ```

* **任意 Agent / 直接命令它跑指令碼**（最通用）：

  ```text theme={null}
  執行 python3 nano-banana-pro/scripts/nano_banana.py "一隻在花園裡打盹的橘貓，油畫風格" --size 4K --aspect 3:2
  ```

<Tip>
  **怎麼控制比例和清晰度**：用 `--aspect` 選寬高比（如 `--aspect 16:9`，10 選 1），用 `--size` 選解析度（`1K` / `2K` / `4K`）。直接說"豎屏 9:16""畫方圖""出 4K"，Agent 也會自動帶上這兩個引數。
</Tip>
...
<Info>
  Nano Banana Pro 有嚴格的內容安全管控。若指令碼提示 `finishReason` 非 `STOP` 或返回的是拒絕說明文本，請按提示調整內容，不要對同一違規提示詞反覆重試。詳見 [生圖介面的錯誤處理指南](/zh-Hant/api-capabilities/nano-banana-image/overview#api-錯誤處理指南)。
</Info>

## 相關文件

* [Nano Banana Pro 圖片生成總覽](/zh-Hant/api-capabilities/nano-banana-image/overview)
* [文生圖 API 參考](/zh-Hant/api-capabilities/nano-banana-image/text-to-image)
* [圖片編輯 API 參考](/zh-Hant/api-capabilities/nano-banana-image/image-edit)
