...
Знание этого заранее избавит вас от повторной переделки:

| Факт                                                                                                                                                                                                                                 | Последствие для пайплайна                                                                                                                                                                                       |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Seed фактически недоступен во всей этой линейке продуктов**: ни одна модель Nano Banana или GPT-Image не предоставляет seed, эффективность seed в Seedream 4.x / 5.x была измерена как нулевая, а Grok Imagine его не поддерживает | Не стройте воспроизводимость на seed. **Единственный надёжный способ воспроизведения — архивировать весь успешно выполненный запрос** — prompt, опорные изображения, каждый параметр — и повторять его дословно |
| **Параметр `n` на стороне сервера не влияет на большинство моделей генерации изображений**                                                                                                                                           | Несколько кандидатов должны появляться за счёт параллельных запросов на стороне клиента; выполните нагрузочное тестирование параллельных запросов по каждому каналу и добавьте экспоненциальный backoff         |
| **Все image API синхронны, не имеют task ID и всё равно тарифицируются при отключении**                                                                                                                                              | Пайплайну нужна собственная очередь задач; см. [Создайте собственную async-очередь](/ru/api-capabilities/image-async-queue) и [Основы Image API](/ru/api-capabilities/image-api-best-practices)                 |
| **Нет эндпоинта для апскейлинга, удаления фона или восстановления лица**                                                                                                                                                             | Задавайте размер вывода на этапе генерации; для прозрачного фона передайте `background: "transparent"` в `gpt-image-2` (это не эндпоинт вырезания — он генерирует сразу на прозрачном фоне)                     |
| **HTTP 200 без изображения обычно означает блокировку модерацией**                                                                                                                                                                   | Логика выбора должна различать «изображение не вернулось» и «изображение вернулось, но оно плохое»; для первого случая см. [Обработка ошибок Gemini Image](/ru/api-capabilities/gemini-image-error-handling)    |
| **Каждый возвращаемый URL изображения — это временная ссылка**                                                                                                                                                                       | Размещайте его у себя сразу после получения; никогда не сохраняйте исходный URL в своей базе данных                                                                                                             |
