...
| -------- | ------ | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model`  | string | Yes                            | Фиксировано на `gpt-image-2-vip`                                                                                                                                                                                              |
| `prompt` | string | Yes                            | Prompt — опишите содержание, стиль, освещение и т. д.                                                                                                                                                                         |
| `size`   | string | **Настоятельно рекомендуется** | Размер вывода: `auto` (модель определяет — **vip остаётся относительно фиксированным для данного prompt**) или один из 30 размеров; формат `WIDTHxHEIGHT` (нижний регистр `x`); если поле не указано, это эквивалентно `auto` |
