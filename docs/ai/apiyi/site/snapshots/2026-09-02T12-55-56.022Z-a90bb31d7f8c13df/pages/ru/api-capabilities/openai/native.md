...
| ------------------------ | ------------------ | ------------------------------------------------------------------------ |
| Веб-поиск                | `web_search`       | Модель автономно ищет в web                                              |
| Поиск файлов             | `file_search`      | Запрос к загруженным vector stores                                       |
| Интерпретатор кода       | `code_interpreter` | Запуск Python в sandbox                                                  |
| Использование компьютера | `computer_use`     | Управление виртуальным рабочим столом                                    |
| Удаленный MCP            | `mcp`              | Подключение к удаленным MCP servers                                      |
| Генерация изображений    | `image_generation` | Встроенная генерация изображений                                         |
| Поиск инструментов       | `tool_search`      | Динамическое получение данных по большим наборам tools (gpt-5.4 и новее) |
...
<Info>
  Встроенные инструменты выполняются на стороне OpenAI; поддержку pass-through для каждого инструмента на канале APIYI следует подтвердить тестированием. Пользовательский function calling полностью поддерживается — см. [Вызов функций](/ru/api-capabilities/openai/function-calling).
</Info>
...
| Модель              | Входные данные (за 1M tokens) | Выходные данные (за 1M tokens) | Примечания                                                         |
| ------------------- | ----------------------------- | ------------------------------ | ------------------------------------------------------------------ |
| `gpt-5.6-sol`       | \$5.00                        | \$30.00                        | Последний флагман, контекст 1M, псевдоним `gpt-5.6` указывает сюда |
| `gpt-5.6-terra`     | \$2.50                        | \$15.00                        | Сбалансированная рабочая модель серии 5.6                          |
| `gpt-5.6-luna`      | \$1.00                        | \$6.00                         | Легковесный вариант 5.6                                            |
| `gpt-5.4`           | \$2.50                        | \$15.00                        | Предыдущая рабочая модель, контекст 1M                             |
| `gpt-5.4-mini`      | \$0.75                        | \$4.50                         | Легковесная, отличное соотношение цены и качества                  |
| `gpt-5.5`           | \$5.00                        | \$30.00                        | Предыдущий флагман, сложное рассуждение                            |
| `gpt-5.2`           | \$1.75                        | \$14.00                        | Предыдущая рабочая модель                                          |
| `gpt-5.1` / `gpt-5` | \$1.25                        | \$10.00                        | Бюджетный вариант                                                  |
| `gpt-5.4-pro`       | \$30.00                       | \$180.00                       | только svip, только responses, для профессионального использования |
| `gpt-5.5-pro`       | \$30.00                       | \$180.00                       | только svip, только responses, для профессионального использования |

Закрепленные версии датирования (например, `gpt-5.4-2026-03-05`) также доступны по той же цене. Полный список: [Models & Pricing](/ru/api-capabilities/model-info).
