...
Все четыре модели **вызываются абсолютно одинаково**; они отличаются максимальным разрешением, максимальной длительностью, ограничениями на количество референсов, скоростью и ценой. Скрипт обрабатывает различия в зависимости от `--model`:

| Модель (`--model`)    | ID модели                         | Макс. разрешение | Скорость (720p/5 с, измеренная) | Группа                                     | Цена по прайс-листу за 720p/5 с       | Лучше всего подходит для                                                                |
| --------------------- | --------------------------------- | ---------------- | ------------------------------- | ------------------------------------------ | ------------------------------------- | --------------------------------------------------------------------------------------- |
| `mini` (по умолчанию) | `doubao-seedance-2-0-mini-260615` | 720p             | **самая высокая, \~87–170 с**   | `SeeDance2` 0.18x<br />или `SD2Mini` 0.10x | \$0.4508<br />со скидкой **\$0.2504** | частого использования агентами, больших объёмов, быстрых предварительных просмотров     |
| `fast`                | `doubao-seedance-2-0-fast-260128` | 720p             | \~100–290 с                     | `SeeDance2` 0.18x<br />или `SD2Fast` 0.15x | \$0.7253<br />со скидкой **\$0.6044** | оптимального баланса качества и стоимости                                               |
| `std`                 | `doubao-seedance-2-0-260128`      | **1080p**        | \~100–290 с                     | `SeeDance2` 0.18x                          | \$0.9074                              | 1080p и максимального качества                                                          |
| `sd25`                | `doubao-seedance-2-5-260628`      | **1080p**        | \~150 с                         | `SeeDance2` 0.18x                          | **\$1.3721**                          | **до 30 с, 30 референсных изображений, вывод в mov** — примерно в 1,5 раза дороже `std` |

<Tip>
  Эмпирическое правило: **для повседневных задач и сценариев с агентами выбирайте модель по умолчанию `mini`**; **нужно разрешение 1080p или максимальное качество** → `--model std`; **нужны клипы длительностью 30 секунд, 30 референсных изображений или вывод в mov** → `--model sd25` — единственный вариант (примерно в 1,5 раза дороже `std`, в той же группе, что и семейство 2.0). Все соотношения сторон имеют одинаковую стоимость в пределах одного уровня разрешения, тарификация длительности выполняется линейно за секунду, а частота кадров фиксирована на уровне 24 кадров/с. Группы со скидкой `SD2Mini` / `SD2Fast` действуют до **7 сентября 2026 года, 23:59 (UTC+8)**; после этого группы останутся доступны, а коэффициент вернётся к значению 0.18x — см. [примечания о группах на странице обзора](/ru/api-capabilities/seedance2/overview).
</Tip>
