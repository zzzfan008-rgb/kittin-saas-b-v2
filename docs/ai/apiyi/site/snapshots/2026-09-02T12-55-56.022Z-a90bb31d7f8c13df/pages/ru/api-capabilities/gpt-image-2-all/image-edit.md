...
<Tip>
  Импорт `undici` выше нужно устанавливать отдельно (`npm i undici`) — он обеспечивает встроенный `fetch`, но не раскрывается под этим именем модуля, а устанавливаемая вами копия по-прежнему обращается к встроенному `fetch` через `setGlobalDispatcher`.

  `maxRetries`, `connectTimeout` и общий timeout запроса находятся на разных уровнях, и настройка не того параметра — самая распространенная ошибка на стороне Node. При сбросах соединения, `UND_ERR_CONNECT_TIMEOUT` или усеченных больших ответах через proxy см. [Устранение неполадок с обрывами соединения Image API](/ru/api-capabilities/image-connection-drops).
</Tip>
...
| ----------------- | ---- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model`           | text | Yes         | Фиксированное значение: `gpt-image-2-all`                                                                                                                                                                                                                                                                                                                                                                        |
| `prompt`          | text | Yes         | Инструкция по редактированию/слиянию на естественном языке                                                                                                                                                                                                                                                                                                                                                       |
| `image`           | file | Yes         | Референсное изображение; может повторяться                                                                                                                                                                                                                                                                                                                                                                       |
| `size`            | text | No          | **Поле не влияет на результат; любое переданное значение молча игнорируется.** Соотношение сторон выходного изображения **соответствует тому референсному изображению, которое prompt называет целевым объектом редактирования** (не обязательно первому в сценариях с несколькими изображениями). Для жёсткой фиксации размера используйте [`gpt-image-2-vip`](/ru/api-capabilities/gpt-image-2-vip/image-edit) |
| `response_format` | text | No          | `b64_json` (по умолчанию) или `url`                                                                                                                                                                                                                                                                                                                                                                              |

<Tip>
  **Итерация в несколько ходов**: Подавайте предыдущее выходное изображение обратно как вход `image` с новыми инструкциями, чтобы итеративно уточнять результат.
</Tip>
