...
# Справочник API HappyHorse по преобразованию изображения в видео

> Справочник API и онлайн-отладка HappyHorse-1.1-i2v Image-to-Video: генерация видео по изображению первого кадра (без поддержки управления по аудио), асинхронный сквозной эндпоинт DashScope.

<Info>
  Вы можете отлаживать напрямую в Playground справа: заполните **Authorization** значением `Bearer sk-your-api-key`, задайте `model` / `input.media` / `parameters`, затем отправьте запрос. Успешная отправка возвращает `task_id`; ниже см. polling и скачивание.
</Info>

<Tip>
  Эта страница описывает create-эндпоинт для `happyhorse-1.1-i2v` (Image-to-Video): передайте изображение первого кадра + prompt, чтобы оживить сцену. Для полного асинхронного процесса см. [Обзор HappyHorse](/ru/api-capabilities/happyhorse/overview).
</Tip>

<Warning>
  * **HappyHorse i2v не поддерживает `driving_audio`** (audio-driven): он принимает только `first_frame`. Для lip-sync / rap используйте [Wan2.7-i2v](/ru/api-capabilities/wan/image-to-video).
  * `input.media` обязательно; иначе upstream вернет `Image-to-video model ... must provide an image`. Медиа `url` должно быть общедоступной https-ссылкой, которую можно получить напрямую через GET.
  * Запрос create отправляется на `/wan/api/v1/...` с `X-DashScope-Async: enable`; не используйте `/v1/videos`.
</Warning>
...
| -------------------------- | ------ | ----------- | ------------ | ------------------------------------------- |
| `model`                    | string | ✓           | —            | Фиксировано на `happyhorse-1.1-i2v`         |
| `input.prompt`             | string | ✓           | —            | Текстовый prompt                            |
| `input.media`              | array  | ✓           | —            | См. таблицу с медиа ниже                    |
| `parameters.resolution`    | string |             | `720P`       | `720P` / `1080P`                            |
| `parameters.duration`      | int    |             | `5`          | Целое число 2–15 секунд                     |
...
| ------------- | ----------- | ---------- | -------------------------------------------------------------------------------------------------------- |
| `first_frame` | ✓           | 1          | Изображение первого кадра; видео начинается с этого изображения (HappyHorse i2v поддерживает только это) |
...
<Warning>
  После отправки **опросите `GET /v1/tasks/{task_id}`** до тех пор, пока `completed`, затем скачайте mp4 с `result_url` (**без заголовка `Authorization`**, срок действия истекает через 24 часа). Полный цикл опроса см. в [Обзор HappyHorse · Поток асинхронных вызовов](/ru/api-capabilities/happyhorse/overview#async-call-flow).
</Warning>
