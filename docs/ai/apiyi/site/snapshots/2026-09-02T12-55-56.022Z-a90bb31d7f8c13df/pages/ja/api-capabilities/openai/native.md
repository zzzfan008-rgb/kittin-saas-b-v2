...
組み込みツールは Responses 専用の機能です。`tools` に宣言すると、OpenAI がサーバー側で実行します:

| ツール              | type               | 説明                                     |
| ---------------- | ------------------ | -------------------------------------- |
| Web search       | `web_search`       | モデルが自律的に Web を検索します                    |
| File search      | `file_search`      | アップロード済みのベクターストアをクエリします                |
| Code interpreter | `code_interpreter` | サンドボックスで Python を実行します                 |
| Computer use     | `computer_use`     | 仮想デスクトップを操作します                         |
| Remote MCP       | `mcp`              | リモート MCP サーバーに接続します                    |
| Image generation | `image_generation` | インラインで画像を生成します                         |
| Tool search      | `tool_search`      | 大規模なツールセット全体を対象に動的取得を行います (gpt-5.4 以降) |
...
<Info>
  組み込みツールは OpenAI 側で実行されます。APIYI チャネルでの各ツールのパススルー対応は、テストで確認してください。カスタム関数呼び出しは完全にサポートされています — [関数呼び出し](/ja/api-capabilities/openai/function-calling) を参照してください。
</Info>
...
| ------------------- | --------------------- | ---------------------- | -------------------------------------------- |
| `gpt-5.6-sol`       | \$5.00                | \$30.00                | 最新のフラッグシップ、1M コンテキスト、`gpt-5.6` エイリアスはここを指します |
| `gpt-5.6-terra`     | \$2.50                | \$15.00                | 5.6 シリーズのバランスの取れた主力モデル                       |
| `gpt-5.6-luna`      | \$1.00                | \$6.00                 | 軽量な 5.6 バリアント                                |
| `gpt-5.4`           | \$2.50                | \$15.00                | 前の主力モデル、1M コンテキスト                            |
| `gpt-5.4-mini`      | \$0.75                | \$4.50                 | 軽量で、コストパフォーマンスに優れています                        |
| `gpt-5.5`           | \$5.00                | \$30.00                | 前のフラッグシップ、複雑な推論                              |
| `gpt-5.2`           | \$1.75                | \$14.00                | 前の主力モデル                                      |
| `gpt-5.1` / `gpt-5` | \$1.25                | \$10.00                | 手頃な価格                                        |
| `gpt-5.4-pro`       | \$30.00               | \$180.00               | svip 限定、responses のみ、業務利用向け                  |
| `gpt-5.5-pro`       | \$30.00               | \$180.00               | svip 限定、responses のみ、業務利用向け                  |

日付固定版（例: `gpt-5.4-2026-03-05`）も同じ価格で利用できます。全リスト: [Models & Pricing](/ja/api-capabilities/model-info)。
