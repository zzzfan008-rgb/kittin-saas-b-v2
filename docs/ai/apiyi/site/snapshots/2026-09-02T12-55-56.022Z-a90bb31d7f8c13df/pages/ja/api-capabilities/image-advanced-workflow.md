...
これらを事前に把握しておくと、やり直しの手戻りを 1 回減らせます。

| 事実                                                                                                                                            | パイプラインへの影響                                                                                                                                                 |
| --------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Seed はこの製品ライン全体で実質的に利用できません**: Nano Banana でも GPT-Image でも seed を公開しているモデルはなく、Seedream 4.x / 5.x の seed も無効だと測定されており、Grok Imagine も対応していません | seed による再現性を前提にしないでください。**唯一信頼できる再現方法は、成功した完全なリクエストをアーカイブすることです** — prompt、参照画像、すべてのパラメータを含めて、そのまま一字一句同じ内容で再実行してください                                       |
| **サーバー側の `n` パラメータは、ほとんどの画像モデルでは効果がありません**                                                                                                    | 複数候補はクライアント側の同時実行数から得る必要があります。チャネルごとの同時実行数を負荷テストし、指数バックオフを追加してください                                                                                         |
| **すべての画像 API は同期型で、タスクIDがなく、切断されても課金されます**                                                                                                    | パイプラインには独自のタスクキューが必要です。[独自の非同期キューを構築する](/ja/api-capabilities/image-async-queue) と [Image API の基本](/ja/api-capabilities/image-api-best-practices) を参照してください |
| **アップスケーリング、背景除去、顔修復のエンドポイントはありません**                                                                                                          | 出力サイズは生成時に決めてください。透過背景にする場合は `background: "transparent"` を `gpt-image-2` に渡します（切り抜きエンドポイントではなく、透過を直接生成します）                                                 |
| **画像が返らない HTTP 200 は、通常モデレーションによるブロックを意味します**                                                                                                 | 選択ロジックは「画像が返ってこなかった」場合と「画像は返ったが不適切だった」場合を区別しなければなりません。前者については [Gemini 画像エラーハンドリング](/ja/api-capabilities/gemini-image-error-handling) を参照してください             |
| **返されるすべての画像 URL は一時リンクです**                                                                                                                   | 受信したら即座に再ホストしてください。上流の URL をデータベースに保存してはいけません                                                                                                              |
