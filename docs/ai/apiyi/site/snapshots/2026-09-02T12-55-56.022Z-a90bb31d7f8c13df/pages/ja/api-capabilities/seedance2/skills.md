...
4つのモデルは**まったく同じ方法で呼び出します**。違いは、解像度上限、長さ上限、参照数の上限、速度、価格です。スクリプトは`--model`ごとに違いを処理します。

| モデル（`--model`） | モデルID                             | 最大解像度     | 速度（720p/5秒、実測）  | グループ                                       | 720p/5秒の定価                     | 最適な用途                                     |
| -------------- | --------------------------------- | --------- | --------------- | ------------------------------------------ | ------------------------------ | ----------------------------------------- |
| `mini`（デフォルト）  | `doubao-seedance-2-0-mini-260615` | 720p      | **最速、約87～170秒** | `SeeDance2` 0.18x<br />または `SD2Mini` 0.10x | \$0.4508<br />割引後 **\$0.2504** | 高頻度のエージェント利用、大量処理、クイックプレビュー               |
| `fast`         | `doubao-seedance-2-0-fast-260128` | 720p      | 約100～290秒       | `SeeDance2` 0.18x<br />または `SD2Fast` 0.15x | \$0.7253<br />割引後 **\$0.6044** | 品質とコストの中間                                 |
| `std`          | `doubao-seedance-2-0-260128`      | **1080p** | 約100～290秒       | `SeeDance2` 0.18x                          | \$0.9074                       | 1080p、最高品質                                |
| `sd25`         | `doubao-seedance-2-5-260628`      | **1080p** | **約150秒**       | `SeeDance2` 0.18x                          | **\$1.3721**                   | **最大30秒、30枚の参照画像、mov出力** — `std`の約1.5倍の価格 |

<Tip>
  目安として、**日常的な用途やエージェントシナリオではデフォルトの`mini`を使用してください**。**1080pまたは最高品質が必要** → `--model std`。**30秒のクリップ、30枚の参照画像、またはmov出力が必要** → `--model sd25`のみが対応します（`std`の約1.5倍の価格で、2.0ファミリーと同じグループです）。同じ解像度帯ではすべてのアスペクト比の料金は同じで、長さに応じて1秒単位で比例課金され、フレームレートは24fpsに固定されています。割引グループ`SD2Mini` / `SD2Fast`は\*\*2026年9月7日 23:59 (UTC+8)\*\*まで有効です。その後もグループはオンラインに残りますが、倍率は0.18xに戻ります。詳しくは[概要ページのグループに関する注記](/ja/api-capabilities/seedance2/overview)をご覧ください。
</Tip>
