...
> GPT-5.6 Sol 模型详情：输入 $5 / 输出 $30 每 1M tokens，1,000,000 上下文，4 个分组可用。

GPT-5.6 系列旗舰档，Terminal-Bench 2.1 88.8%、BrowseComp 90.4%，`gpt-5.6` 别名指向它。
...
* 0 – 272,000 tokens 输入 \$5/1M
* 超过 272,000 tokens 输入 \$10/1M
...
以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `gpt-5.6-sol`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  ...
  <Card title="GPT-5.6 Luna" icon="git-compare" href="/models/gpt-5-6-luna">
    同系列模型详情
  </Card>
