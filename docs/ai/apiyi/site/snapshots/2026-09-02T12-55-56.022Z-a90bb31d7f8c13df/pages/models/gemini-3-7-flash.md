...
> Gemini 3.7 Flash 模型详情：输入 $0.75 / 输出 $3.75 每 1M tokens，1,000,000 上下文、64,000 最大输出，2 个分组可用。

编码与 Agent 大幅进步的新一代 Flash 主力，DeepSWE v1.1 达 65.3%，思考三档可调，1M 上下文。
...
以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `gemini-3.7-flash`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  <Card title="上线公告" icon="megaphone" href="/news/gemini-3-7-flash-launch">
    Gemini 3.7 Flash 的发布背景、实测表现与迁移建议
  </Card>
