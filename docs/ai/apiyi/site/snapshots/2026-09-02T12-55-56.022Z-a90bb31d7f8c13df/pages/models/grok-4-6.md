...
> Grok 4.6 模型详情：输入 $2 / 输出 $6 每 1M tokens，500,000 上下文，4 个分组可用。

沿用 4.5 的 1.5T 参数基座，靠后训练把 Artificial Analysis 智能指数从 56 提到 61，挂牌与 4.5 同价。
...
* 204,801 – 512,000 tokens 输入 \$4/1M
...
以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `grok-4.6`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  <Card title="上线公告" icon="megaphone" href="/news/grok-4-6-launch">
    Grok 4.6 的发布背景、实测表现与迁移建议
  </Card>
  ...
  <Card title="Grok 4.5" icon="git-compare" href="/models/grok-4-5">
    同系列模型详情
  </Card>
...
<Note>本页规格数据人工维护于 `models/data/model-details.json`，价格与端点取自系统实时定价接口，数据更新于 2026-08-31 11:46 (UTC+8)，规格最后核对于 2026-08-13。</Note>
