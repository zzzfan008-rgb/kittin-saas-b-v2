...
> Grok 4.3 模型详情：输入 $1.25 / 输出 $2.5 每 1M tokens，1,000,000 上下文，4 个分组可用。

推理常开不可关闭，1M 上下文，输出速度约 159 tokens/s。
...
* 0 – 204,800 tokens 输入 \$1.25/1M
* 超过 204,800 tokens 输入 \$2.5/1M
...
以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `grok-4.3`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  <Card title="上线公告" icon="megaphone" href="/news/grok-4-3-launch">
    Grok 4.3 的发布背景、实测表现与迁移建议
  </Card>
