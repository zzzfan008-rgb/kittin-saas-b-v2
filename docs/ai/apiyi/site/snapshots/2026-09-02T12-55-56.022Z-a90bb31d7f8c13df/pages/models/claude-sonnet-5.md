...
> Claude Sonnet 5 模型详情：输入 $2 / 输出 $10 每 1M tokens，4 个分组可用。

编程与智能体开发首选，SWE-bench Verified 85.2%，AWS 纯官转、高缓存命中。
...
| 模型 ID                      | 说明                   |
| -------------------------- | -------------------- |
| `claude-sonnet-5`          | 标准调用                 |
| `claude-sonnet-5-thinking` | 强制思考变体，价格与端点与标准版完全相同 |

## 调用示例

以下示例通过 OpenAI Chat Completions（`/v1/chat/completions`）调用 `claude-sonnet-5`。base\_url 换成 `https://api.apiyi.com/v1` 即可，其余用法与官方一致。
...
<CardGroup cols={2}>
  <Card title="上线公告" icon="megaphone" href="/news/claude-sonnet-5-launch">
    Claude Sonnet 5 的发布背景、实测表现与迁移建议
  </Card>
