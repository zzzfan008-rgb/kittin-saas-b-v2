...
> 本文件是状态与证据台账，不重复定义当前项目规则。规则、优先级和冲突
> 处理统一以仓库根目录 [`AGENTS.md`](../AGENTS.md) 为准；此处的分支、SHA、
> PR、CI 与审核记录均按对应日期理解，并在使用前实时核对。
...
| 门禁替换 PR | 已合并 | [PR #22](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/pull/22) 以 `d942885c57fcf97c5f18821ee94c7f3f720e6cac` 为 base、`246a258bdad108c0c49853fb5804e811768c468a` 为 head，已 squash-merge 为 `4701280af93cea8a62df6c2799ba5f1b425829ce`；GitHub Actions 门禁由本地 Codex 门禁替代 |
| 门禁证据收口 | 已合并 | [PR #23](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/pull/23) 从 `4701280af93cea8a62df6c2799ba5f1b425829ce` 合并为 `35c39e6753cbb9872f4ad26bf9455a877d72a6da`；[PR #24](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/pull/24) 从 `35c39e6753cbb9872f4ad26bf9455a877d72a6da` 合并为 `56fcbff1225b974906996ad790ed2c64c9692d57`，校正证据并稳定 Base UI Tab 几何回归 |
| 最新门禁维护 PR | 已合并 | [PR #25](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/pull/25) 以 `56fcbff1225b974906996ad790ed2c64c9692d57` 为 base、`64cc74c5bcbf78597a53da3352f52138614a07ba` 为 head，CodeRabbit 成功后 squash-merge 为已验证检查点 `main@45cad4f4a71655fb410218806cff08855418e58e`；提交 tree 为 `2282aa971085e14703283ed165fde49ed534cda6` |
...
| 最新已合并 main 门禁检查点 | 已完成 | 2026-09-01 对当时干净且与 `origin/main` 一致的 `main@45cad4f4a71655fb410218806cff08855418e58e` 运行 `npm run gate:codex -- --commit 45cad4f4a71655fb410218806cff08855418e58e`：Node.js 22.20.0、完整确定性套件、30/30 桌面 E2E、production smoke 3/3、GitNexus 与 Codex 默认模型精确审查全部通过；0 个 P0-P3 findings。本次文档候选以该提交为 base，候选 head 由 PR 与门禁输出绑定；合并后再记录新的 main SHA |
...
| D 结果迭代与桌面体验 | 已完成 | 显式结果动作、三主题 × 三宽度、统一左侧 Dock、横向缩放、平台快捷键与拖拽稳定性 | [PR #10](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/pull/10)、[PR #14](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/pull/14)、[PR #15](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/pull/15)、[PR #17](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/pull/17)、[PR #19](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/pull/19) | [最新合并后 main CI](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/actions/runs/33313563543) 通过 | 三主题 × 三宽度、键盘/焦点、Results 与最新 UI 回归证据已补齐并经用户视觉确认；本机 Gemma 与 CodeRabbit 通过 |
...
| G 文档与发布准备 | 已完成 | 部署/恢复/安全文档、发布候选 PR、最终审计与用户确认 | [PR #17](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/pull/17) | [PR head CI 33268687680](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/actions/runs/33268687680) 与[合并后 main CI 33269171028](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/actions/runs/33269171028)通过 | [本机 Gemma 精确 SHA 复审记录](https://github.com/zzzfan008-rgb/kittin-saas-b-v2/pull/17#issuecomment-5464184334)为 `APPROVE`，无有效 P0–P3 |
...
- [x] 使用内置浏览器完成登录后的人工视觉验收。

Phase D 证据补录：2026-08-30 在 `main@3dc6a1d` 的已登录本地工作台完成三主题 × 三宽度矩阵，逐张检查画布、节点、Dock、MiniMap 与 Results；另行验证成功、失败、结果未知、查看、对比、下载、继续处理、主题菜单、快捷键说明、Project Center 焦点循环、Escape 焦点恢复和 1024 Dock 互斥。截图与交互记录见 [`PHASE_D_VISUAL_ACCEPTANCE.md`](PHASE_D_VISUAL_ACCEPTANCE.md)。本轮未发现必须修改的 UI，用户已完成视觉确认，并在随后明确授权合并最终发布候选 PR #17。

PR #19 补充证据：2026-08-30 在已登录本地服务上按用户反馈迭代统一左侧 Dock、悬浮节点库、横向缩放滑杆、平台化快捷键和节点拖拽稳定性；用户逐项确认后授权推送与合并。精确候选 `c8d99866d06468868920b684e90da6e367e66995` 的桌面 Playwright 29/29（1024 / 1280 / 1440）、完整 `npm run check`、生产构建、本机 `gemma4:e4b` 三组复审和 CodeRabbit 均通过；合并后 `main@bb89795b632af04e48c3246af6ada33245c7fdd4` 的 CI 33313563543 再次通过。Results 的恢复、失败、未知结果、查看、对比与继续处理边界保持不变。
...
- [x] 发布候选 PR 附摘要、风险/回滚、测试矩阵、视觉证据、包体、GitNexus、CI 与本地精确 SHA 复审。
- [x] 用户明确确认最终合并后，main 合并后 CI 再次成功。
...
| 门禁 | 最新结果 | 证据/备注 |
| --- | --- | --- |
| `npm ci` | 通过 | 2026-09-01 `main@45cad4f` 精确合并后门禁；干净安装完成，未使用真实 AI 配置 |
| `npm test` | 通过 | 2026-09-01 `main@45cad4f` 精确合并后门禁；隔离 PostgreSQL 全套回归，包含包体预算、懒加载边界与 Codex 门禁行为契约；仅使用 dummy/stub AI |
| `npm run test:e2e` | 通过 | 2026-09-01 `main@45cad4f` 精确合并后门禁为 30/30；覆盖 1024、1280、1440 桌面项目、登录、独立黄金路径、初始项目恢复与拖拽/同步提示稳定性；临时 PostgreSQL + dummy/stub AI |
| production browser smoke | 本地通过 | 2026-09-01 `main@45cad4f` 精确合并后门禁的 production smoke 3/3；真实哈希产物、动态 chunk、项目中心与 SPA fallback 均通过 |
| `npm run build` | 通过 | 2026-09-01 `main@45cad4f` 精确合并后门禁；Web + server；初始 JS gzip 为 147,188 / 210,000 bytes |
| `npm audit` | 通过 | 2026-09-01 `main@45cad4f` 精确合并后门禁：`found 0 vulnerabilities` |
...
| GitNexus `detect_changes` | 已执行 | 2026-09-01 `main@45cad4f` 索引已同步；对精确父提交 `56fcbff...45cad4f` 的比较为 3 个文件、2 个变更符号、0 条受影响流程、low risk，并由 30/30 桌面回归和完整隔离测试覆盖 |
...
| 本地模型复审 | 当前 main 精确 SHA 通过 | 2026-09-01 Codex 配置的默认模型对 `56fcbff...45cad4f` 精确差异返回 `pass`、0 个 P0-P3 findings；GitNexus 同步返回 pass；未触发或等待 Codex Cloud |
...
| 2026-09-01 | PR #25 合并检查点证据续接 | PR #23、#24、#25 依次合并；对 PR #25 合并后的 `main@45cad4f` 精确父提交差异完成完整本地门禁，默认 Codex 模型返回 `pass`、0 个 P0-P3。本次文档候选从 `45cad4f` 分出，合并前不预写新的 main SHA；未执行 tag、Release、部署或生产数据操作。 |
