...
| GitNexus | 已重建 | `gitnexus analyze --index-only --pdg`：14,889 symbols、32,308 edges、220 clusters、300 flows；当前索引对应 `f1eb1bc`，且未写入仓库 |
...
| B 文档与持久化边界 | 进行中 | `DocumentSnapshot`、活动文档单一数据源、草稿隔离、持久化节流 | — | 本地 B1–B2 门禁通过 | B1、B2 各三次独立本地审计 APPROVE；Cloud checkpoint 留待 B3 |
...
| `npm run check` | 通过 | 2026-08-25；Phase B2 的 lint、Vite/CSS 构建门禁与隔离 PostgreSQL 全套回归均通过；仅使用 dummy/stub AI |
| `npm run test:e2e` | 通过 | 11/11；1024、1280、1440 桌面项目，临时 PostgreSQL + dummy AI |
| production browser smoke | 待实现 | Phase E |
| `npm run build` | 通过 | 2026-08-25；Phase B2 Web + server；主 JS 734.10 kB / gzip 234.00 kB，既有 >500 kB 警告留待 Phase F |
| `npm audit` | 通过 | `found 0 vulnerabilities` |
| `git diff --check` | 通过 | 未发现空白错误；`dist` / `dist-server` 仍为忽略产物 |
| GitNexus `detect_changes` | 已执行 | 2026-08-25 Phase B2 staged 差异为 critical：48 changed / 41 affected / 22 files。范围为活动文档 selector、UI/命令式消费者迁移与异步素材定向回写；完整 `npm run check`及页签、选择、历史和会话回归覆盖 |
