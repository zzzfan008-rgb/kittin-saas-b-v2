  ...
  return createPortal(
    <div className="fixed inset-0 z-100 flex flex-col bg-[var(--gc-shell)]">
      <header className="flex h-12 shrink-0 items-center gap-3 border-b border-[var(--gc-border)] px-4">
        ...
        <span className="text-label text-neutral-500">GPT Image 2</span>
      ...
      <footer className="flex min-h-16 shrink-0 items-center gap-4 border-t border-[var(--gc-border)] px-4 py-2">
        ...
        <label className="flex min-w-56 items-center gap-2 text-label text-neutral-500">
          笔刷 {brushSize}px
          <input
        ...
        <span className="text-label text-neutral-600">红色是修改中心，不是裁切框 · 新内容可在金色融合区内完整延展</span>
...
function ModeButton({ active, label, onClick, disabled = false }: { active: boolean; label: string; onClick: () => void; disabled?: boolean }) {
  return (
    <button type="button" onClick={onClick} disabled={disabled} className={`rounded-sm px-3 py-1.5 text-label disabled:opacity-40 ${active ? "bg-gold text-[var(--gc-accent-cta-ink)]" : "text-neutral-400 hover:text-neutral-200"}`}>
      {label}
    </button>
  );
}
